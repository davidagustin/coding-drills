...
function computeSteps(): DropWhileStep[] {
  const steps: DropWhileStep[] = [];
  let startIndex = -1;
  ...
  steps.push({
    ...
    result: [],
    i: -1,
    explanation: `Start: Drop elements while predicate (x < 4) is true`,
  });
  ...
  for (let i = 0; i < ARRAY.length; i++) {
    ...
    steps.push({
      ...
      result: [],
      i,
      explanation: `Index ${i}: ${val} < 4 → drop`,
    });
  }

  const result = startIndex >= 0 ? ARRAY.slice(startIndex) : [];
  ...
  if (startIndex >= 0) {
    steps.push({
      ...
      result: [...result],
      i: startIndex,
      explanation: `Index ${startIndex}: ${ARRAY[startIndex]} >= 4 → start keeping from here`,
    });
    ...
    for (let i = startIndex; i < ARRAY.length; i++) {
      steps.push({
        ...
        result: [...result],
        i,
        explanation: `Index ${i}: Add ${ARRAY[i]} to result`,
      });
    }
  }
...
export default function DropWhileViz() {
  ...
  const startIndex = arr.findIndex((val) => !PREDICATE(val));

  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-zinc-900 rounded-xl border border-zinc-800">
      <h2 className="text-2xl font-bold text-white mb-4">Drop While</h2>
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {arr.map((val, idx) => {
              ...
              const isDropped = startIndex >= 0 && idx < startIndex;
              const isKept = startIndex >= 0 && idx >= startIndex;
              return (
                <motion.div
                  key={idx}
                  initial={false}

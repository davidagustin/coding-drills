...
import { FRAMEWORK_CONFIG, type FrameworkId } from '@/lib/frontend-drills';
...
export default function FrontendStudyPage() {
  const params = useParams();
  const framework = (params?.framework as string) || 'react';
  const sources = useMemo(() => getFrameworkSources(framework), [framework]);
  ...
  const handleStart = useCallback(
    ...
    [framework, prioritiseWeak, getWeakCardIds, studyState.ratings],
  );
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <Breadcrumb
            items={[
              { label: 'Home', href: '/' },
              { label: 'Frontend', href: '/frontend-drills' },
              { label: frameworkName, href: `/frontend-drills/${framework}` },
              { label: 'Study Mode' },
            ]}
            className="text-sm"
          />

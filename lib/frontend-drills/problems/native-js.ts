...
// { tagName: "button", textContent: "Click me", attributes: { class: "btn-primary" } }`,
    ...
    sample: `({ tagName: "button", textContent: "Click me", attributes: { class: "btn-primary" } })`,
...
checkPasswordStrength(testPassword)`,
...

  {
    ...
    text: 'Implement a synchronous race function that takes an array of { value, priority } objects and returns the value with the lowest priority number (fastest).',
    setup: 'Competitors with priorities.',
    setupCode: `const competitors = [\n  { value: "slow", priority: 1000 },\n  { value: "fast", priority: 1 },\n  { value: "medium", priority: 500 }\n];\nfunction race(items) {\n  return items.reduce((best, item) => item.priority < best.priority ? item : best).value;\n}`,
    expected: 'fast',
    sample: 'race(competitors)',
    hints: ['Use reduce to find the item with lowest priority', 'Return its value'],
...

  {
    ...
    difficulty: 'medium',
    title: 'Request Queue',
    text: 'Implement a request queue that processes functions sequentially. Each function returns a value. Return the results in order.',
    setup: 'A sequential request queue.',
    setupCode: `function createQueue() {\n  const results = [];\n  const queue = [];\n  return {\n    add(fn) { queue.push(fn); },\n    process() {\n      while (queue.length) {\n        const fn = queue.shift();\n        results.push(fn());\n      }\n    },\n    getResults() { return results; }\n  };\n}\nconst q = createQueue();\nq.add(() => "A");\nq.add(() => "B");\nq.add(() => "C");\nq.process();`,
    expected: ['A', 'B', 'C'],
    sample: 'q.getResults()',
    hints: ['Process queue items one at a time', 'Shift from queue and push result'],
...

  {
    ...
    text: 'Implement an optimistic update pattern: update state immediately, then confirm or rollback based on the result. Return the state log.',
...

  {
    ...
    text: 'Wrap an operation in try/catch and return a result object with { data, error }. Return results for both success and failure cases.',
    setup: 'A safe fetch wrapper.',
    setupCode: `function safeFetch(fn) {\n  try {\n    const data = fn();\n    return { data, error: null };\n  } catch (e) {\n    return { data: null, error: e.message };\n  }\n}\nconst ok = safeFetch(() => "hello");\nconst fail = safeFetch(() => { throw new Error("oops"); });`,
...

  {
    ...
    text: 'Implement a "latest only" pattern that ignores stale responses. The acceptResult method only stores data if the request ID matches the latest. Simulate two requests where the first arrives after the second was started.',
    setup: 'A latest-only result accepter.',
    ...
    sample: '({ r1, r2 })',
    hints: [
      'Increment a request ID for each call',
      'Compare ID when accepting result to detect staleness',
    ],
...

  {
    ...
    text: 'Implement a poll function that calls a function repeatedly until a condition is met. Track the number of polls. Return the final result and poll count.',
    setup: 'A polling mechanism.',
    setupCode: `let pollCount = 0;\nlet serverValue = 0;\nfunction poll(fn, condition, maxPolls) {\n  for (let i = 0; i < maxPolls; i++) {\n    pollCount++;\n    const result = fn();\n    if (condition(result)) return result;\n  }\n  return null;\n}\nserverValue = 0;\nconst result = poll(\n  () => { serverValue++; return serverValue; },\n  v => v >= 3,\n  10\n);`,
    expected: { result: 3, polls: 3 },
    sample: '({ result, polls: pollCount })',
    hints: ['Loop and call fn each iteration', 'Check condition after each result'],
...

  {
    ...
    text: 'Fetch user and posts data from two sources and combine into a single result object.',
    setup: 'Two data source functions.',
    setupCode: `const getUser = () => ({ id: 1, name: "Alice" });\nconst getPosts = () => [{ title: "Post 1" }, { title: "Post 2" }];\nfunction fetchAll() {\n  const user = getUser();\n  const posts = getPosts();\n  return { user, posts };\n}`,
    expected: { user: { id: 1, name: 'Alice' }, posts: [{ title: 'Post 1' }, { title: 'Post 2' }] },
    sample: 'fetchAll()',
    hints: ['Call both data sources', 'Combine results into a single object'],
...

  {
    ...
    text: 'Implement a synchronous allSettled that takes an array of functions, calls each, and returns status objects. Functions that throw are "rejected"; others are "fulfilled".',
    setup: 'A synchronous allSettled implementation.',
    setupCode: `function allSettled(fns) {\n  return fns.map(fn => {\n    try {\n      const value = fn();\n      return { status: "fulfilled", value };\n    } catch (e) {\n      return { status: "rejected", reason: e.message || String(e) };\n    }\n  });\n}\nconst results = allSettled([\n  () => 1,\n  () => { throw new Error("fail"); },\n  () => 3\n]);`,
    expected: [
    ...
    sample: 'results',
    hints: [
      'Wrap each function call in try/catch',
...

  {
    ...
    text: 'Implement a request deduplicator that caches results for the same key and only calls the fetcher once per unique key. Track fetch count.',
    ...
    hints: ['Store results in a Map', 'Return cached result for duplicate keys'],
...

  {
    ...
    text: 'Implement validators that check values against a mock server lookup. Run validators and collect error results.',
    setup: 'Validators with mock server data.',
    ...
    sample: 'result',
    hints: ['Run each validator on its field', 'Collect non-null results as errors'],

import { Method, LanguageId } from './types';
...
export const pythonMethods: Method[] = [
  // ==================== LIST METHODS ====================
  {
    ...
    description: 'Adds an element to the end of the list. Commonly used in Django views to build response data incrementally.',
    arguments: [
    ...
    examples: [
      ...
      { code: 'errors = []; errors.append("Invalid email"); errors', output: "['Invalid email']", explanation: 'Building validation errors list - common Django pattern' }
    ],
  ...
  },
  {
    ...
    description: 'Extends the list by appending elements from the iterable. Useful for merging querysets or aggregating API responses.',
    arguments: [
    ...
    examples: [
      ...
      { code: 'all_users = list(active_users); all_users.extend(inactive_users)', output: "Combined list of users", explanation: 'Merging Django querysets into a single list' }
    ],
  ...
  },
  {
    ...
    description: 'Inserts an element at a specified position. Use sparingly as it has O(n) complexity.',
    arguments: [
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'lst = [1, 3]; lst.insert(1, 2); lst', output: '[1, 2, 3]', explanation: 'Inserts 2 at index 1' },
      { code: 'middleware = ["auth", "logging"]; middleware.insert(0, "cors"); middleware', output: "['cors', 'auth', 'logging']", explanation: 'Adding middleware at beginning - Django middleware ordering' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'index', type: 'int', description: 'Position to remove from', optional: true, defaultValue: '-1' }
    ],
    returns: { type: 'any', description: 'The removed element' },
    timeComplexity: 'O(1) for last element, O(n) otherwise',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'lst = [1, 2, 3]; lst.pop()', output: '3', explanation: 'Removes and returns last element' },
      { code: 'task_queue = ["task1", "task2"]; current = task_queue.pop(0); current', output: "'task1'", explanation: 'Processing tasks FIFO - consider collections.deque for this' }
    ],
    relatedMethods: ['remove', 'del', 'deque.popleft'],
    notes: ['Raises IndexError if list is empty or index out of range', 'Use deque for O(1) popleft operations']
  },
  {
    name: 'remove',
    category: 'lists',
    ...
    arguments: [
      { name: 'element', type: 'any', description: 'Element to remove' }
    ],
    returns: { type: 'None', description: 'Modifies list in place' },
    timeComplexity: 'O(n)',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'lst = [1, 2, 2, 3]; lst.remove(2); lst', output: '[1, 2, 3]', explanation: 'Removes first occurrence of 2' },
      { code: 'permissions = ["read", "write", "admin"]; permissions.remove("admin"); permissions', output: "['read', 'write']", explanation: 'Revoking a permission - Django permission handling' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'stages = ["draft", "review", "published"]; stages.index("review")', output: '1', explanation: 'Finding workflow stage position' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'element', type: 'any', description: 'Element to count' }
    ],
    ...
    examples: [
      ...
      { code: 'responses = ["success", "error", "success"]; responses.count("error")', output: '1', explanation: 'Counting API error responses' }
    ],
  ...
  },
  {
    ...
    description: 'Sorts the list in place. For Django querysets, prefer order_by() for database-level sorting.',
    arguments: [
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'lst = [3, 1, 2]; lst.sort(); lst', output: '[1, 2, 3]', explanation: 'Sorts in ascending order' },
      { code: 'users = [{"name": "Bob", "age": 25}, {"name": "Alice", "age": 30}]; users.sort(key=lambda u: u["age"]); users', output: '[{"name": "Bob", ...}, {"name": "Alice", ...}]', explanation: 'Sorting users by age - common API response sorting' }
    ],
    relatedMethods: ['sorted', 'reverse'],
    notes: ['Modifies list in-place', 'Use sorted() to return a new list', 'Timsort algorithm - stable sort']
  },
  {
    name: 'reverse',
    category: 'lists',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'lst = [1, 2, 3]; lst.reverse(); lst', output: '[3, 2, 1]', explanation: 'Reverses the list' },
      { code: 'history = ["event1", "event2", "event3"]; history.reverse(); history', output: "['event3', 'event2', 'event1']", explanation: 'Showing most recent events first' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'original = [1, 2, 3]; copied = original.copy(); copied', output: '[1, 2, 3]', explanation: 'Creates a shallow copy' },
      { code: 'config = {"debug": True}; settings = [config]; new_settings = settings.copy()', output: 'Shallow copy - nested objects share references', explanation: 'Be careful with mutable nested objects' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'cache = ["item1", "item2"]; cache.clear()', output: '[]', explanation: 'Clearing a cache - Django cache invalidation pattern' }
    ],
  ...
  },
  {
    ...
    description: 'Creates a new list by applying an expression to each item. Pythonic and often more readable than map/filter.',
    arguments: [],
    ...
    examples: [
      ...
      { code: '[user.email for user in User.objects.filter(is_active=True)]', output: "['user1@example.com', ...]", explanation: 'Extracting emails from Django queryset' },
      { code: '[{"id": p.id, "name": p.name} for p in products]', output: '[{"id": 1, "name": "Widget"}, ...]', explanation: 'Building API response data from queryset' }
    ],
    relatedMethods: ['map', 'filter', 'generator expression'],
    notes: ['More Pythonic than map/filter with lambda', 'Use generator expression (parentheses) for memory efficiency']
  },
  {
    name: 'sorted',
    category: 'lists',
    syntax: 'sorted(iterable, key=None, reverse=False)',
    description: 'Returns a new sorted list from the items in iterable. Does not modify the original.',
    arguments: [
    ...
    examples: [
      ...
      { code: 'sorted(users, key=lambda u: u.created_at, reverse=True)', output: 'Users sorted by creation date (newest first)', explanation: 'Sorting Django model instances by field' },
      { code: 'sorted(data, key=itemgetter("price"))', output: 'Data sorted by price', explanation: 'Using operator.itemgetter for cleaner sorting' }
    ],
    relatedMethods: ['list.sort', 'reversed', 'operator.itemgetter'],
    notes: ['Returns new list, does not modify original', 'Stable sort - equal elements maintain relative order']
  },
  {
    name: 'enumerate',
    category: 'lists',
    syntax: 'enumerate(iterable, start=0)',
    description: 'Returns an enumerate object yielding pairs of (index, element). Essential for loops needing indices.',
    arguments: [
      { name: 'iterable', type: 'iterable', description: 'Sequence to enumerate' },
      { name: 'start', type: 'int', description: 'Starting index value', optional: true, defaultValue: '0' }
    ],
    returns: { type: 'enumerate', description: 'Iterator of (index, element) tuples' },
    timeComplexity: 'O(1) to create, O(n) to iterate',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'list(enumerate(["a", "b", "c"]))', output: '[(0, "a"), (1, "b"), (2, "c")]', explanation: 'Index-value pairs' },
      { code: 'for i, item in enumerate(items, start=1): print(f"{i}. {item}")', output: '1. first\n2. second', explanation: 'Human-readable numbering starting at 1' },
      { code: '[{"rank": i, "name": p.name} for i, p in enumerate(top_products, 1)]', output: '[{"rank": 1, "name": "Widget"}, ...]', explanation: 'Building ranked API response' }
    ],
    relatedMethods: ['zip', 'range'],
    notes: ['Avoid range(len(list)) pattern - use enumerate instead', 'Memory efficient - yields values on demand']
  },
  {
    name: 'zip',
    category: 'lists',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'list(zip([1, 2], ["a", "b"]))', output: '[(1, "a"), (2, "b")]', explanation: 'Pairs elements from two lists' },
      { code: 'dict(zip(keys, values))', output: '{"key1": "value1", "key2": "value2"}', explanation: 'Creating dict from parallel lists - common data transformation' },
      { code: 'for field, value in zip(form.fields, request.POST.values()): ...', output: 'Process form data pairs', explanation: 'Parallel iteration pattern in form processing' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'list(map(str.upper, ["a", "b", "c"]))', output: "['A', 'B', 'C']", explanation: 'Converts all strings to uppercase' },
      { code: 'list(map(int, ["1", "2", "3"]))', output: '[1, 2, 3]', explanation: 'Converts strings to integers' },
      { code: 'list(map(lambda u: u.email, users))', output: "['user1@example.com', ...]", explanation: 'Extracting attribute - prefer list comprehension for readability' }
    ],
    relatedMethods: ['filter', 'list comprehension', 'functools.reduce'],
    notes: ['Returns iterator - wrap in list() if needed', 'List comprehension often more readable in Python']
  },
  {
    name: 'filter',
    category: 'lists',
    ...
    arguments: [
      { name: 'function', type: 'callable | None', description: 'Test function (None filters falsy values)' },
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'list(filter(lambda x: x > 0, [-1, 0, 1, 2]))', output: '[1, 2]', explanation: 'Keeps positive numbers' },
      { code: 'list(filter(None, ["", "hello", None, "world"]))', output: "['hello', 'world']", explanation: 'Filters out falsy values' },
      { code: 'list(filter(lambda u: u.is_active, users))', output: 'Active users only', explanation: 'Filtering model instances - prefer queryset filter()' }
    ],
    relatedMethods: ['map', 'list comprehension', 'itertools.filterfalse'],
    notes: ['Returns iterator', 'For Django, prefer queryset.filter() for database-level filtering']
  },
  {
    name: 'any',
    category: 'lists',
    syntax: 'any(iterable)',
    description: 'Returns True if any element in the iterable is truthy. Short-circuits on first True.',
    arguments: [
      { name: 'iterable', type: 'iterable', description: 'Iterable to check' }
    ],
    ...
    examples: [
      ...
      { code: 'any(user.is_admin for user in users)', output: 'True if any admin exists', explanation: 'Check if any user is admin - Django permission checking' },
      { code: 'any(error in response for error in critical_errors)', output: 'True if critical error found', explanation: 'Error detection in API responses' }
    ],
  ...
  },
  {
    ...
    category: 'lists',
    syntax: 'all(iterable)',
    description: 'Returns True if all elements in the iterable are truthy. Short-circuits on first False.',
    arguments: [
      { name: 'iterable', type: 'iterable', description: 'Iterable to check' }
    ],
    ...
    examples: [
      ...
      { code: 'all(field.is_valid() for field in form.fields)', output: 'True if form is valid', explanation: 'Form validation - Django form pattern' },
      { code: 'all(hasattr(obj, attr) for attr in required_attrs)', output: 'True if all attributes exist', explanation: 'Duck typing validation' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'sequence', type: 'sequence', description: 'Sequence to measure' }
    ],
    ...
    examples: [
      ...
      { code: 'len(User.objects.filter(is_active=True))', output: 'Count of active users', explanation: 'Django queryset count - prefer .count() for efficiency' }
    ],
    relatedMethods: ['count', 'size'],
    notes: ['For Django querysets, prefer .count() for database-level counting', 'O(1) for built-in types']
  },
  {
    name: 'max',
    category: 'lists',
    ...
    examples: [
      ...
      { code: 'max(users, key=lambda u: u.score)', output: 'User with highest score', explanation: 'Finding top performer' },
      { code: 'max(orders, key=attrgetter("total"), default=None)', output: 'Highest value order or None', explanation: 'Safe max with default for empty querysets' }
    ],
    relatedMethods: ['min', 'sorted'],
    notes: ['Raises ValueError on empty iterable without default', 'Use default parameter for safety']
  },
  {
    name: 'min',
    category: 'lists',
    ...
    examples: [
      ...
      { code: 'min(products, key=lambda p: p.price)', output: 'Cheapest product', explanation: 'Finding lowest price item' },
      { code: 'min(dates, default=datetime.now())', output: 'Earliest date or now', explanation: 'Safe min with default' }
    ],
  ...
  },
  {
    ...
    arguments: [
      ...
      { name: 'start', type: 'number', description: 'Starting value', optional: true, defaultValue: '0' }
    ],
    ...
    examples: [
      ...
      { code: 'sum(order.total for order in orders)', output: 'Total revenue', explanation: 'Aggregating order totals - prefer Django aggregate()' },
      { code: 'sum(item.quantity for item in cart.items)', output: 'Total items in cart', explanation: 'Cart total calculation' }
    ],
    relatedMethods: ['functools.reduce', 'math.fsum'],
    notes: ['For Django, prefer .aggregate(Sum()) for database-level aggregation', 'Use math.fsum() for floating point precision']
  },
  ...
  // ==================== STRING METHODS ====================
  {
    ...
    description: 'Joins elements of an iterable with the separator string. Efficient for string concatenation.',
    arguments: [
    ...
    examples: [
      ...
      { code: '", ".join(str(id) for id in user_ids)', output: '"1, 2, 3"', explanation: 'Building comma-separated IDs for SQL IN clause' },
      { code: '"/".join(["api", "v1", "users"])', output: '"api/v1/users"', explanation: 'Building URL path segments' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: '"user@domain.com".split("@")', output: "['user', 'domain.com']", explanation: 'Parsing email address' },
      { code: '"Bearer token123".split(" ", 1)', output: "['Bearer', 'token123']", explanation: 'Parsing Authorization header with maxsplit' }
    ],
    relatedMethods: ['join', 'rsplit', 'splitlines', 'partition'],
    notes: ['None sep splits on any whitespace and removes empty strings', 'Use rsplit() to split from the right']
  },
  {
    name: 'strip',
    category: 'strings',
    ...
    examples: [
      ...
      { code: 'request.GET.get("email", "").strip()', output: 'Cleaned email input', explanation: 'Sanitizing form input - Django best practice' },
      { code: 'api_key.strip()', output: 'Clean API key', explanation: 'Cleaning API keys from config files' }
    ],
    relatedMethods: ['lstrip', 'rstrip', 'replace'],
    notes: ['Always strip user input for security', 'chars parameter removes any character in the set']
  },
  {
    name: 'lower',
    category: 'strings',
    ...
    examples: [
      ...
      { code: 'email.lower()', output: 'Normalized email', explanation: 'Case-insensitive email comparison - Django user model pattern' },
      { code: 'search_query.lower()', output: 'Normalized search term', explanation: 'Case-insensitive search preparation' }
    ],
    relatedMethods: ['upper', 'casefold', 'capitalize', 'title'],
    notes: ['Use casefold() for aggressive case-folding (international text)', 'Strings are immutable - returns new string']
  },
  {
    name: 'upper',
    category: 'strings',
  ...
  },
  {
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: '"hello world".replace("world", "Python")', output: '"hello Python"', explanation: 'Simple replacement' },
      { code: 'template.replace("{{name}}", user.name)', output: 'Rendered template', explanation: 'Simple template rendering - for complex cases use Django templates' },
      { code: 'slug.replace(" ", "-").lower()', output: 'url-friendly-slug', explanation: 'Creating URL slugs' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'prefix', type: 'str | tuple', description: 'Prefix to check (can be tuple of prefixes)' },
    ...
    examples: [
      ...
      { code: 'url.startswith(("http://", "https://"))', output: 'True for valid URLs', explanation: 'Validating URL protocols with tuple' },
      { code: 'path.startswith("/api/")', output: 'True for API routes', explanation: 'Django URL routing check' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'suffix', type: 'str | tuple', description: 'Suffix to check (can be tuple of suffixes)' },
      { name: 'start', type: 'int', description: 'Start index', optional: true },
      { name: 'end', type: 'int', description: 'End index', optional: true }
    ],
    ...
    examples: [
      ...
      { code: 'filename.endswith((".jpg", ".png", ".gif"))', output: 'True for image files', explanation: 'File type validation with tuple' },
      { code: 'email.endswith("@company.com")', output: 'True for company emails', explanation: 'Email domain validation' }
    ],
    relatedMethods: ['startswith', 'find', 'rfind'],
    notes: ['Can accept tuple of suffixes to check multiple', 'Useful for file extension validation']
  },
  {
    name: 'find',
    category: 'strings',
    ...
    examples: [
      ...
      { code: 'content_type.find("json")', output: 'Index if JSON content type', explanation: 'Checking Content-Type header' }
    ],
    relatedMethods: ['index', 'rfind', 'in operator'],
    notes: ['Use "in" operator for existence check', 'index() raises ValueError instead of returning -1']
  },
  {
    name: 'format',
    category: 'strings',
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: '"Hello, {}!".format("World")', output: '"Hello, World!"', explanation: 'Positional formatting' },
      { code: '"Hello, {name}!".format(name="Alice")', output: '"Hello, Alice!"', explanation: 'Keyword formatting' },
      { code: '"/api/users/{user_id}/".format(user_id=123)', output: '"/api/users/123/"', explanation: 'URL building pattern' },
      { code: '"{:.2f}".format(3.14159)', output: '"3.14"', explanation: 'Number formatting' }
    ],
    relatedMethods: ['f-strings', '% operator', 'str.format_map'],
    notes: ['Prefer f-strings (f"...") for readability in Python 3.6+', 'Use format for dynamic format strings']
  },
  {
    name: 'f-string',
    category: 'strings',
    syntax: 'f"string {expression}"',
    description: 'Formatted string literals for embedding expressions inside strings. Python 3.6+ feature.',
    arguments: [],
    returns: { type: 'str', description: 'Formatted string' },
    timeComplexity: 'O(n)',
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'name = "World"; f"Hello, {name}!"', output: '"Hello, World!"', explanation: 'Variable interpolation' },
      { code: 'f"User {user.id}: {user.name}"', output: '"User 1: Alice"', explanation: 'Attribute access in f-string' },
      { code: 'f"Total: ${amount:.2f}"', output: '"Total: $19.99"', explanation: 'Formatting numbers in f-string' },
      { code: 'f"{datetime.now():%Y-%m-%d}"', output: '"2024-01-15"', explanation: 'Date formatting in f-string' }
    ],
    relatedMethods: ['format', '% operator'],
    notes: ['Most Pythonic string formatting method', 'Expressions evaluated at runtime', 'Use = for debugging: f"{var=}"'],
    sinceVersion: '3.6'
  },
  {
    name: 'isdigit',
    ...
    examples: [
      ...
      { code: 'user_id.isdigit()', output: 'True for numeric IDs', explanation: 'Validating numeric URL parameters' }
    ],
    relatedMethods: ['isnumeric', 'isdecimal', 'isalpha', 'isalnum'],
    notes: ['Does not handle negative numbers or decimals', 'Use try/except int() for robust validation']
  },
  {
    name: 'isalpha',
    category: 'strings',
    ...
    examples: [
      ...
      { code: 'username.isalpha()', output: 'True for letter-only usernames', explanation: 'Simple username validation' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'token.isalnum()', output: 'Validates alphanumeric tokens', explanation: 'API token validation' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'encoding', type: 'str', description: 'Encoding to use', optional: true, defaultValue: '"utf-8"' },
    ...
    examples: [
      ...
      { code: 'password.encode() # For hashing', output: "b'secret'", explanation: 'Encoding for Django password hashing' },
      { code: 'json_data.encode("utf-8") # For HTTP response', output: 'Bytes for response body', explanation: 'Preparing data for HTTP response' }
    ],
    relatedMethods: ['bytes.decode', 'codecs'],
    notes: ['Essential for hashing, cryptography, and binary protocols', 'UTF-8 is the web standard encoding']
  },
  {
    name: 'zfill',
    category: 'strings',
    ...
    arguments: [
      { name: 'width', type: 'int', description: 'Minimum string width' }
    ],
    ...
    examples: [
      ...
      { code: 'str(order_number).zfill(8)', output: '"00000123"', explanation: 'Formatting order numbers' },
      { code: 'str(hour).zfill(2) + ":" + str(minute).zfill(2)', output: '"09:05"', explanation: 'Time formatting' }
    ],
  ...
  // ==================== DICTIONARY METHODS ====================
  {
    ...
    description: 'Returns the value for key if it exists, otherwise returns default. Essential for safe dictionary access.',
    arguments: [
    ...
    examples: [
      ...
      { code: 'request.GET.get("page", "1")', output: 'Page number or "1"', explanation: 'Django request parameter with default' },
      { code: 'user_data.get("preferences", {})', output: 'Preferences or empty dict', explanation: 'Safe nested access pattern' }
    ],
    relatedMethods: ['setdefault', 'pop', '__getitem__'],
    notes: ['Never raises KeyError', 'Essential for processing user input safely', 'Most common dict method in Django']
  },
  {
    name: 'setdefault',
    category: 'dicts',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'd = {}; d.setdefault("key", [])', output: '[]', explanation: 'Sets and returns default' },
      { code: 'cache.setdefault(user_id, {"visits": 0})', output: 'Existing or new user data', explanation: 'Cache initialization pattern' },
      { code: 'groups.setdefault(category, []).append(item)', output: 'Group-by pattern', explanation: 'Building grouped data structure' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: '{"a": 1, "b": 2}.keys()', output: "dict_keys(['a', 'b'])", explanation: 'View of keys' },
      { code: 'list(config.keys())', output: "['DEBUG', 'DATABASE_URL', ...]", explanation: 'List of configuration keys' },
      { code: '"email" in request.POST.keys()', output: 'True if email submitted', explanation: 'Checking form field presence' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: '{"a": 1, "b": 2}.values()', output: 'dict_values([1, 2])', explanation: 'View of values' },
      { code: 'sum(cart.values())', output: 'Total quantity', explanation: 'Sum of cart quantities' },
      { code: 'any(v is None for v in data.values())', output: 'True if any None value', explanation: 'Checking for missing data' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: '{"a": 1, "b": 2}.items()', output: "dict_items([('a', 1), ('b', 2)])", explanation: 'View of items' },
      { code: 'for key, value in config.items(): ...', output: 'Iterate over key-value pairs', explanation: 'Pythonic dict iteration' },
      { code: '{k: v for k, v in data.items() if v is not None}', output: 'Dict without None values', explanation: 'Filtering dict by values' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'd = {"a": 1}; d.update({"b": 2}); d', output: '{"a": 1, "b": 2}', explanation: 'Merge dicts' },
      { code: 'context.update(extra_context)', output: 'Merged template context', explanation: 'Django template context merging' },
      { code: 'defaults.update(user_settings)', output: 'User settings override defaults', explanation: 'Settings override pattern' }
    ],
  ...
  },
  {
    ...
    description: 'Removes and returns value for key. Raises KeyError if key not found and no default provided.',
    arguments: [
    ...
    examples: [
      ...
      { code: 'session.pop("user_id", None)', output: 'User ID or None', explanation: 'Removing session data safely' },
      { code: 'data.pop("password")  # Remove before logging', output: 'Sanitize sensitive data', explanation: 'Removing sensitive fields before serialization' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: '{x: x**2 for x in range(4)}', output: '{0: 0, 1: 1, 2: 4, 3: 9}', explanation: 'Square mapping' },
      { code: '{k: v for k, v in data.items() if v}', output: 'Dict with truthy values only', explanation: 'Filtering dict' },
      { code: '{user.id: user.name for user in users}', output: '{1: "Alice", 2: "Bob"}', explanation: 'Building ID-to-name mapping from queryset' },
      { code: '{field: getattr(obj, field) for field in fields}', output: 'Serialized object', explanation: 'Manual serialization pattern' }
    ],
    relatedMethods: ['list comprehension', 'dict()'],
    notes: ['Pythonic way to transform and filter dicts', 'More readable than dict() with generator']
  },
  {
    name: 'fromkeys',
    category: 'dicts',
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'dict.fromkeys(["a", "b"], 0)', output: '{"a": 0, "b": 0}', explanation: 'Initialize with same value' },
      { code: 'dict.fromkeys(required_fields, "")', output: 'Empty form data template', explanation: 'Creating form defaults' },
      { code: 'seen = dict.fromkeys(items)', output: 'Preserve order, dedupe', explanation: 'Ordered deduplication (Python 3.7+)' }
    ],
    relatedMethods: ['dict comprehension', 'defaultdict'],
    notes: ['All values share same reference - careful with mutable values', 'Use comprehension for distinct mutable values']
  },
  ...
  // ==================== SET METHODS ====================
  {
    ...
    examples: [
      ...
      { code: 'visited_pages.add(page_id)', output: 'Track unique page visits', explanation: 'Tracking unique items' },
      { code: 'tags.add(tag.lower())', output: 'Building normalized tag set', explanation: 'Deduplicating tags' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'active_sessions.remove(session_id)', output: 'Remove known session', explanation: 'Session management' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'element', type: 'hashable', description: 'Element to discard' }
    ],
    returns: { type: 'None', description: 'Modifies set in place' },
    timeComplexity: 'O(1) average',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 's = {1, 2, 3}; s.discard(5); s', output: '{1, 2, 3}', explanation: 'No error if not found' },
      { code: 'permissions.discard("admin")', output: 'Remove permission if exists', explanation: 'Safe permission removal' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'others', type: 'iterable', description: 'Sets to union with' }
    ],
    ...
    examples: [
      ...
      { code: 'all_permissions = user_perms | group_perms | role_perms', output: 'Combined permissions', explanation: 'Merging permission sets' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'others', type: 'iterable', description: 'Sets to intersect with' }
    ],
    returns: { type: 'set', description: 'New set with common elements' },
    timeComplexity: 'O(min(n, m))',
    spaceComplexity: 'O(min(n, m))',
    examples: [
      { code: '{1, 2, 3}.intersection({2, 3, 4})', output: '{2, 3}', explanation: 'Common elements' },
      { code: '{1, 2, 3} & {2, 3, 4}', output: '{2, 3}', explanation: 'Operator syntax' },
      { code: 'allowed_fields = requested_fields & valid_fields', output: 'Filter to valid fields only', explanation: 'API field validation' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'others', type: 'iterable', description: 'Sets to subtract' }
    ],
    ...
    examples: [
      ...
      { code: 'missing_fields = required_fields - provided_fields', output: 'Find missing required fields', explanation: 'Form validation pattern' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'other', type: 'iterable', description: 'Set to compare against' }
    ],
    ...
    examples: [
      ...
      { code: 'user_perms <= required_perms', output: 'False - missing permissions', explanation: 'Permission subset check' },
      { code: 'requested_fields.issubset(allowed_fields)', output: 'Validate field access', explanation: 'API field access control' }
    ],
  ...
  // ==================== FILE OPERATIONS ====================
  {
    ...
    description: 'Opens a file and returns a file object. Always use with context manager (with statement).',
    arguments: [
      { name: 'file', type: 'str | Path', description: 'Path to file' },
      { name: 'mode', type: 'str', description: 'File mode (r/w/a/b/+)', optional: true, defaultValue: '"r"' },
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'with open("file.txt", "r") as f: content = f.read()', output: 'File contents', explanation: 'Read file with context manager' },
      { code: 'with open("data.json", "w", encoding="utf-8") as f: json.dump(data, f)', output: 'Write JSON file', explanation: 'Django settings/fixture writing' },
      { code: 'with open(log_path, "a") as f: f.write(log_entry)', output: 'Append to log', explanation: 'Log file appending' }
    ],
    relatedMethods: ['pathlib.Path.open', 'io.StringIO'],
    notes: ['Always use with statement for automatic cleanup', 'Specify encoding="utf-8" for text files', 'Use Django FileField for user uploads']
  },
  {
    name: 'read',
    category: 'files',
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'with open("file.txt") as f: content = f.read()', output: 'Entire file contents', explanation: 'Read entire file' },
      { code: 'chunk = f.read(1024)', output: 'First 1024 chars', explanation: 'Read in chunks for large files' }
    ],
  ...
  },
  {
    ...
    examples: [
      { code: 'line = f.readline()', output: 'First line with newline', explanation: 'Read single line' },
      { code: 'header = f.readline().strip()', output: 'First line without newline', explanation: 'Read CSV header' }
    ],
  ...
  },
  {
    ...
    arguments: [
      { name: 'string', type: 'str', description: 'String to write' }
    ],
    ...
    examples: [
      ...
      { code: 'f.write(json.dumps(data))', output: 'Write JSON string', explanation: 'Writing serialized data' }
    ],
  ...
  // ==================== JSON OPERATIONS ====================
  {
    ...
    arguments: [
      ...
      { name: 'default', type: 'function', description: 'Function for non-serializable objects', optional: true }
    ],
    returns: { type: 'str', description: 'JSON string' },
    timeComplexity: 'O(n)',
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'json.dumps({"name": "Alice"})', output: '\'{"name": "Alice"}\'', explanation: 'Simple dict to JSON' },
      { code: 'json.dumps(data, indent=2)', output: 'Pretty-printed JSON', explanation: 'Formatted for readability' },
      { code: 'json.dumps(data, default=str)', output: 'Serialize with str fallback', explanation: 'Handle datetime and other non-serializable types' }
    ],
    relatedMethods: ['json.loads', 'json.dump', 'DjangoJSONEncoder'],
    notes: ['Use DjangoJSONEncoder for datetime, Decimal, UUID support', 'In Django views, prefer JsonResponse']
  },
  {
    name: 'json.loads',
    category: 'json',
    ...
    arguments: [
      { name: 'string', type: 'str', description: 'JSON string to parse' }
    ],
    returns: { type: 'any', description: 'Python object' },
    timeComplexity: 'O(n)',
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'json.loads(\'{"name": "Alice"}\')', output: '{"name": "Alice"}', explanation: 'JSON to dict' },
      { code: 'data = json.loads(request.body)', output: 'Parse request body', explanation: 'Django API endpoint pattern' },
      { code: 'config = json.loads(Path("config.json").read_text())', output: 'Load config file', explanation: 'Loading JSON configuration' }
    ],
  ...
  // ==================== DATETIME OPERATIONS ====================
  {
    ...
    arguments: [
      { name: 'tz', type: 'timezone', description: 'Timezone info', optional: true }
    ],
    returns: { type: 'datetime', description: 'Current datetime' },
    timeComplexity: 'O(1)',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'datetime.now()', output: 'datetime.datetime(2024, 1, 15, 10, 30, 0)', explanation: 'Current local time' },
      { code: 'datetime.now(timezone.utc)', output: 'Current UTC time', explanation: 'Timezone-aware datetime' },
      { code: 'from django.utils import timezone; timezone.now()', output: 'Django timezone-aware now', explanation: 'Django best practice for current time' }
    ],
    relatedMethods: ['datetime.utcnow', 'django.utils.timezone.now', 'datetime.today'],
    notes: ['In Django, always use timezone.now() for timezone awareness', 'Store dates in UTC, display in user timezone']
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'datetime.now().strftime("%Y-%m-%d")', output: '"2024-01-15"', explanation: 'ISO date format' },
      { code: 'dt.strftime("%B %d, %Y")', output: '"January 15, 2024"', explanation: 'Human readable date' },
      { code: 'dt.strftime("%Y-%m-%dT%H:%M:%SZ")', output: '"2024-01-15T10:30:00Z"', explanation: 'ISO 8601 format for APIs' }
    ],
    relatedMethods: ['strptime', 'isoformat'],
    notes: ['Use isoformat() for ISO 8601 standard', 'Common codes: %Y year, %m month, %d day, %H hour, %M minute, %S second']
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'datetime.strptime("2024-01-15", "%Y-%m-%d")', output: 'datetime(2024, 1, 15)', explanation: 'Parse ISO date' },
      { code: 'datetime.strptime("15/01/2024", "%d/%m/%Y")', output: 'datetime(2024, 1, 15)', explanation: 'Parse European date format' },
      { code: 'datetime.strptime(request.POST["date"], "%Y-%m-%d")', output: 'Parse form date input', explanation: 'Form date handling' }
    ],
    relatedMethods: ['strftime', 'fromisoformat'],
    notes: ['Raises ValueError if format does not match', 'Use dateutil.parser for flexible parsing']
  },
  {
    name: 'timedelta',
    category: 'datetime',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'datetime.now() + timedelta(days=7)', output: 'Date one week from now', explanation: 'Date arithmetic' },
      { code: 'timedelta(hours=24)', output: 'One day duration', explanation: 'Token expiration period' },
      { code: 'expires_at = timezone.now() + timedelta(hours=1)', output: 'Session expiration', explanation: 'Django session/token expiration' }
    ],
    relatedMethods: ['datetime', 'relativedelta'],
    notes: ['Use dateutil.relativedelta for month/year arithmetic', 'timedelta only handles days and smaller units']
  },
  ...
  // ==================== EXCEPTION HANDLING ====================
  {
    ...
    examples: [
      { code: 'try:\n    result = int(value)\nexcept ValueError:\n    result = 0', output: 'Safe integer conversion', explanation: 'Handle conversion errors' },
      { code: 'try:\n    user = User.objects.get(pk=pk)\nexcept User.DoesNotExist:\n    return HttpResponse(status=404)', output: '404 response', explanation: 'Django model lookup with 404' },
      { code: 'try:\n    data = json.loads(body)\nexcept json.JSONDecodeError:\n    return JsonResponse({"error": "Invalid JSON"}, status=400)', output: 'API error handling', explanation: 'JSON parsing in API views' }
    ],
    relatedMethods: ['raise', 'finally', 'else clause'],
    notes: ['Catch specific exceptions, not bare except', 'Use get_object_or_404() shortcut in Django', 'Log exceptions for debugging']
  },
  {
    name: 'raise',
    category: 'exceptions',
    ...
    arguments: [
      { name: 'exception', type: 'Exception', description: 'Exception to raise' }
    ],
    returns: { type: 'NoReturn', description: 'Never returns' },
    examples: [
      { code: 'raise ValueError("Invalid input")', output: 'Raises ValueError', explanation: 'Raise with message' },
      { code: 'if not user.is_authenticated:\n    raise PermissionDenied("Login required")', output: 'Django permission error', explanation: 'Django permission checking' },
      { code: 'raise Http404("Page not found")', output: 'Django 404', explanation: 'Django 404 response' }
    ],
    relatedMethods: ['try-except', 'assert'],
    notes: ['Use built-in exceptions when appropriate', 'Create custom exceptions for domain-specific errors']
  },
  ...
  // ==================== COLLECTIONS MODULE ====================
  {
    ...
    arguments: [
      { name: 'default_factory', type: 'callable', description: 'Function to create default values' }
    ],
    returns: { type: 'defaultdict', description: 'Dictionary with default values' },
    timeComplexity: 'O(1) for access',
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'd = defaultdict(list); d["key"].append(1)', output: '{"key": [1]}', explanation: 'Auto-create lists' },
      { code: 'counts = defaultdict(int); counts["a"] += 1', output: '{"a": 1}', explanation: 'Counting pattern' },
      { code: 'groups = defaultdict(list)\nfor item in items:\n    groups[item.category].append(item)', output: 'Group items by category', explanation: 'Grouping pattern - like Django annotate' }
    ],
    relatedMethods: ['dict.setdefault', 'Counter'],
    notes: ['Cleaner than setdefault() for complex defaults', 'from collections import defaultdict']
  },
  {
    name: 'Counter',
    category: 'collections',
    ...
    arguments: [
      { name: 'iterable', type: 'iterable', description: 'Items to count' }
    ],
    returns: { type: 'Counter', description: 'Counter object' },
    timeComplexity: 'O(n)',
    spaceComplexity: 'O(k) unique items',
    examples: [
      { code: 'Counter(["a", "b", "a", "c", "a"])', output: "Counter({'a': 3, 'b': 1, 'c': 1})", explanation: 'Count occurrences' },
      { code: 'Counter(word.lower() for word in text.split())', output: 'Word frequency', explanation: 'Word frequency analysis' },
      { code: 'Counter(log.level for log in logs).most_common(3)', output: 'Top 3 log levels', explanation: 'Log analysis' }
    ],
    relatedMethods: ['most_common', 'defaultdict'],
    notes: ['most_common(n) returns n most frequent', 'Supports arithmetic operations between Counters']
  },
  {
    name: 'OrderedDict',
    category: 'collections',
    syntax: 'OrderedDict(items)',
    description: 'Dict that remembers insertion order. Note: regular dict maintains order in Python 3.7+.',
    arguments: [
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'OrderedDict([("a", 1), ("b", 2)])', output: "OrderedDict([('a', 1), ('b', 2)])", explanation: 'Ordered dict' },
    ...
    notes: ['Regular dict preserves order in Python 3.7+', 'OrderedDict has move_to_end() and popitem(last=False)']
  },
  {
    name: 'namedtuple',
    category: 'collections',
    ...
    examples: [
      { code: 'Point = namedtuple("Point", ["x", "y"])\np = Point(1, 2)\np.x', output: '1', explanation: 'Access by name' },
      { code: 'User = namedtuple("User", "id name email")\nuser = User(1, "Alice", "alice@example.com")', output: 'Lightweight data class', explanation: 'Data transfer object pattern' },
      { code: 'APIResponse = namedtuple("APIResponse", ["data", "status", "headers"])', output: 'Structured API response', explanation: 'Type-safe response handling' }
    ],
    relatedMethods: ['dataclass', 'typing.NamedTuple'],
    notes: ['Immutable - good for data integrity', 'Consider dataclasses for mutable data with defaults']
  },
  {
    name: 'deque',
    category: 'collections',
    ...
    arguments: [
      ...
      { name: 'maxlen', type: 'int', description: 'Maximum size (auto-evict oldest)', optional: true }
    ],
    returns: { type: 'deque', description: 'Deque object' },
    timeComplexity: 'O(1) for append/pop on both ends',
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'd = deque([1, 2, 3]); d.appendleft(0); list(d)', output: '[0, 1, 2, 3]', explanation: 'O(1) left append' },
      { code: 'd.popleft()', output: '0', explanation: 'O(1) left pop' },
      { code: 'recent = deque(maxlen=100)\nrecent.append(event)  # Auto-evicts oldest', output: 'Bounded history', explanation: 'Recent activity buffer' }
    ],
  ...
  // ==================== ITERTOOLS MODULE ====================
  {
    ...
    arguments: [
      { name: 'iterables', type: 'iterable', description: 'Iterables to chain' }
    ],
    ...
    examples: [
      ...
      { code: 'list(chain.from_iterable([[1, 2], [3, 4]]))', output: '[1, 2, 3, 4]', explanation: 'Flatten nested iterables' },
      { code: 'all_items = chain(queryset1, queryset2, queryset3)', output: 'Combine querysets lazily', explanation: 'Memory-efficient queryset combination' }
    ],
    relatedMethods: ['chain.from_iterable', 'list.extend'],
    notes: ['Memory efficient - yields items lazily', 'Use chain.from_iterable for nested iterables']
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'for k, g in groupby("AAABBC"): print(k, list(g))', output: 'A [A,A,A], B [B,B], C [C]', explanation: 'Group consecutive' },
      { code: 'sorted_items = sorted(items, key=lambda x: x.category)\nfor cat, group in groupby(sorted_items, key=lambda x: x.category): ...', output: 'Group items by category', explanation: 'Category grouping - similar to Django annotate' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'list(islice(infinite_generator(), 100))', output: 'First 100 items', explanation: 'Limit infinite iterator' }
    ],
  ...
  // ==================== FUNCTOOLS MODULE ====================
  {
    ...
    arguments: [
      { name: 'maxsize', type: 'int | None', description: 'Maximum cache size', optional: true, defaultValue: '128' }
    ],
    returns: { type: 'function', description: 'Cached function' },
    timeComplexity: 'O(1) for cached calls',
    spaceComplexity: 'O(maxsize)',
    examples: [
      { code: '@lru_cache(maxsize=None)\ndef fibonacci(n): ...', output: 'Cached fibonacci', explanation: 'Memoization' },
      { code: '@lru_cache(maxsize=100)\ndef get_user_permissions(user_id): ...', output: 'Cached permissions', explanation: 'Cache expensive database lookups' },
      { code: '@lru_cache(maxsize=1000)\ndef expensive_computation(data_hash): ...', output: 'Cached computation', explanation: 'Cache heavy processing' }
    ],
    relatedMethods: ['cache', 'cached_property'],
    notes: ['Arguments must be hashable', 'Use maxsize=None for unlimited cache', 'In Django, consider django.core.cache for distributed caching']
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'add_ten = partial(add, 10); add_ten(5)', output: '15', explanation: 'Pre-fill first argument' },
      { code: 'json_encoder = partial(json.dumps, indent=2, ensure_ascii=False)', output: 'Configured JSON encoder', explanation: 'Pre-configure function' },
      { code: 'logger = partial(log_event, level="INFO", source="api")', output: 'Pre-configured logger', explanation: 'Logging with defaults' }
    ],
    relatedMethods: ['lambda', 'functools.partialmethod'],
    notes: ['Cleaner than lambda for simple partial application', 'Preserves function signature for introspection']
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'reduce(lambda a, b: a + b, [1, 2, 3, 4])', output: '10', explanation: 'Sum using reduce' },
      { code: 'reduce(operator.mul, [1, 2, 3, 4])', output: '24', explanation: 'Product using reduce' },
      { code: 'reduce(lambda acc, x: acc | x, permission_sets, set())', output: 'Union of permission sets', explanation: 'Merge multiple sets' }
    ],
    relatedMethods: ['sum', 'itertools.accumulate'],
    notes: ['Prefer sum(), min(), max() for common cases', 'Use accumulate() to see intermediate results']
  },
  ...
  // ==================== PATHLIB MODULE ====================
  {
    ...
    arguments: [
      { name: 'path_string', type: 'str', description: 'Path string' }
    ],
    returns: { type: 'Path', description: 'Path object' },
    timeComplexity: 'O(1)',
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'Path("/home/user/file.txt")', output: 'PosixPath object', explanation: 'Create path object' },
      { code: 'Path(__file__).parent / "config.json"', output: 'Path relative to script', explanation: 'Django settings pattern' },
      { code: 'BASE_DIR = Path(__file__).resolve().parent.parent', output: 'Django BASE_DIR', explanation: 'Standard Django settings pattern' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: 'if config_path.exists(): load_config(config_path)', output: 'Conditional load', explanation: 'Safe file loading pattern' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(n)',
    examples: [
      { code: 'Path("config.json").read_text()', output: 'File contents as string', explanation: 'Read file' },
      { code: 'config = json.loads(Path("config.json").read_text())', output: 'Load JSON config', explanation: 'One-liner config loading' }
    ],
  ...
  },
  {
    ...
    examples: [
      { code: 'list(Path(".").glob("*.py"))', output: 'All Python files', explanation: 'Find Python files' },
      { code: 'list(Path(".").glob("**/*.py"))', output: 'Recursive Python files', explanation: 'Recursive search' },
      { code: 'for migration in Path("migrations").glob("*.py"): ...', output: 'Process migrations', explanation: 'Django migration processing' }
    ],
    relatedMethods: ['rglob', 'iterdir'],
    notes: ['Use ** for recursive matching', 'rglob(pattern) is shorthand for glob("**/" + pattern)']
  },
  ...
  // ==================== TYPING MODULE ====================
  {
    ...
    description: 'Type annotations for functions and variables. Essential for maintainable Django code.',
    arguments: [],
    returns: { type: 'N/A', description: 'Type hints do not affect runtime' },
    examples: [
      { code: 'def greet(name: str) -> str:\n    return f"Hello, {name}"', output: 'Type annotated function', explanation: 'Basic type hints' },
      { code: 'def get_user(user_id: int) -> User | None:', output: 'Union type with None', explanation: 'Optional return type (Python 3.10+)' },
      { code: 'def process_items(items: list[dict[str, Any]]) -> list[int]:', output: 'Generic types', explanation: 'Complex type annotations' },
      { code: 'class UserService:\n    def __init__(self, repo: UserRepository) -> None:', output: 'Dependency injection', explanation: 'Django service pattern with types' }
    ],
    relatedMethods: ['Optional', 'Union', 'List', 'Dict'],
    notes: ['Use mypy for static type checking', 'Types are not enforced at runtime', 'Essential for large Django projects'],
    sinceVersion: '3.5'
  },
  {
    name: 'Optional',
    ...
    arguments: [
      { name: 'Type', type: 'type', description: 'The non-None type' }
    ],
    returns: { type: 'type alias', description: 'Type that allows None' },
    examples: [
      { code: 'def find_user(email: str) -> Optional[User]:', output: 'Returns User or None', explanation: 'Optional return type' },
      { code: 'def process(data: str | None = None) -> None:', output: 'Optional parameter', explanation: 'Python 3.10+ syntax' },
      { code: 'user: User | None = User.objects.filter(email=email).first()', output: 'Optional queryset result', explanation: 'Django first() returns None if not found' }
    ],
  ...
  // ==================== WEB/DJANGO PATTERNS ====================
  {
    ...
    description: 'Returns the value of a named attribute. Essential for dynamic attribute access in Django.',
    arguments: [
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'getattr(user, "email", "")', output: 'User email or empty string', explanation: 'Safe attribute access' },
      { code: 'fields = ["name", "email"]\n{f: getattr(obj, f) for f in fields}', output: 'Dynamic serialization', explanation: 'Building dict from fields list' },
      { code: 'method = getattr(self, f"handle_{action}", self.handle_default)', output: 'Dynamic method dispatch', explanation: 'Strategy pattern - Django view dispatch' }
    ],
  ...
  },
  {
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'hasattr(user, "profile")', output: 'True if user has profile', explanation: 'Check attribute existence' },
      { code: 'if hasattr(request, "user") and request.user.is_authenticated:', output: 'Safe auth check', explanation: 'Django request checking' },
      { code: 'duck_typing = hasattr(obj, "read") and hasattr(obj, "write")', output: 'Duck typing check', explanation: 'File-like object detection' }
    ],
    relatedMethods: ['getattr', 'setattr'],
    notes: ['Duck typing pattern - check capabilities, not types', 'May call __getattr__ which could have side effects']
  },
  {
    name: 'setattr',
    category: 'builtins',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'setattr(user, "email", new_email)', output: 'Set user email', explanation: 'Dynamic attribute setting' },
      { code: 'for field, value in data.items():\n    setattr(instance, field, value)', output: 'Bulk update from dict', explanation: 'Django model update pattern' },
      { code: 'setattr(settings, "DEBUG", False)', output: 'Modify settings', explanation: 'Runtime settings modification (testing)' }
    ],
    relatedMethods: ['getattr', 'hasattr'],
    notes: ['Use for dynamic attribute assignment', 'Common in Django model serialization/deserialization']
  },
  {
    name: 'isinstance',
    category: 'builtins',
    ...
    examples: [
      ...
      { code: 'isinstance(data, (list, tuple))', output: 'True for list or tuple', explanation: 'Multiple type check' },
      { code: 'if isinstance(error, ValidationError):\n    return JsonResponse({"errors": error.messages}, status=400)', output: 'Error type handling', explanation: 'Django exception handling' }
    ],
  ...
  },
  {
    ...
    category: 'builtins',
    syntax: 'repr(object)',
    description: 'Returns a string representation that could recreate the object. Essential for debugging.',
    arguments: [
      { name: 'object', type: 'any', description: 'Object to represent' }
    ],
    ...
    examples: [
      ...
      { code: 'repr(datetime.now())', output: "'datetime.datetime(2024, 1, 15, ...)'", explanation: 'Datetime repr' },
      { code: 'def __repr__(self):\n    return f"User(id={self.id}, email={self.email!r})"', output: 'Custom __repr__', explanation: 'Django model __repr__ pattern' }
    ],
    relatedMethods: ['str', '__repr__'],
    notes: ['Use !r in f-strings for repr: f"{var!r}"', 'Implement __repr__ for custom classes for better debugging']
  },
  {
    name: 'vars',
    category: 'builtins',
    ...
    spaceComplexity: 'O(1)',
    examples: [
      { code: 'vars(user)', output: "{'id': 1, 'name': 'Alice', ...}", explanation: 'Object attributes as dict' },
      { code: '{k: v for k, v in vars(obj).items() if not k.startswith("_")}', output: 'Public attributes only', explanation: 'Filter private attributes' },
      { code: 'json.dumps({k: v for k, v in vars(model).items() if not k.startswith("_")})', output: 'Simple serialization', explanation: 'Quick model serialization (careful with relations)' }
    ],
  ...
  },
  {
    ...
    examples: [
      ...
      { code: '[m for m in dir(obj) if not m.startswith("_")]', output: 'Public methods/attributes', explanation: 'Filter dunder methods' },
      { code: '[m for m in dir(User) if not m.startswith("_")]', output: 'Model fields and methods', explanation: 'Exploring Django model' }
    ],

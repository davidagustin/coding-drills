...
export const typescriptProblems: Problem[] = [
  ...
  },
  {
    ...
    setup: 'interface Item { name: string; active?: boolean; }\nconst items: Item[] = [{name: "a", active: true}, {name: "b"}, {name: "c", active: true}, {name: "d", active: false}];',
    setupCode: 'interface Item { name: string; active?: boolean; }\nconst items: Item[] = [{name: "a", active: true}, {name: "b"}, {name: "c", active: true}, {name: "d", active: false}];',
  ...
  },
  {
    ...
    setup: 'type Result = { type: "success"; data: string } | { type: "error"; message: string };\nconst results: Result[] = [{type: "success", data: "ok"}, {type: "error", message: "fail"}, {type: "success", data: "done"}];',
    setupCode: 'type Result = { type: "success"; data: string } | { type: "error"; message: string };\nconst results: Result[] = [{type: "success", data: "ok"}, {type: "error", message: "fail"}, {type: "success", data: "done"}];',
    expected: [{ type: 'success', data: 'ok' }, { type: 'success', data: 'done' }],
    sample: 'results.filter((r): r is Extract<Result, { type: "success" }> => r.type === "success")',
    hints: [
  ...
  },
  {
    ...
    setup: 'interface User { id: number; firstName: string; lastName: string; email: string; }\nconst users: User[] = [{id: 1, firstName: "John", lastName: "Doe", email: "john@test.com"}];',
    setupCode: 'interface User { id: number; firstName: string; lastName: string; email: string; }\nconst users: User[] = [{id: 1, firstName: "John", lastName: "Doe", email: "john@test.com"}];',
    expected: [{ id: 1, fullName: 'John Doe' }],
    sample: 'users.map((u): { id: number; fullName: string } => ({ id: u.id, fullName: `${u.firstName} ${u.lastName}` }))',
    hints: [
  ...
  },
  {
    ...
    expected: [{ value: 'a', index: 0 }, { value: 'b', index: 1 }, { value: 'c', index: 2 }],
  ...
  },
  {
    ...
    setup: 'interface Item { name: string; category: string; }\nconst items: Item[] = [{name: "a", category: "x"}, {name: "b", category: "y"}, {name: "c", category: "x"}];',
    setupCode: 'interface Item { name: string; category: string; }\nconst items: Item[] = [{name: "a", category: "x"}, {name: "b", category: "y"}, {name: "c", category: "x"}];',
    expected: { x: [{ name: 'a', category: 'x' }, { name: 'c', category: 'x' }], y: [{ name: 'b', category: 'y' }] },
    sample: 'items.reduce<Record<string, Item[]>>((acc, item) => ({ ...acc, [item.category]: [...(acc[item.category] || []), item] }), {})',
    hints: [
  ...
  },
  {
    ...
    setup: 'interface User { name: string; role: "admin" | "user"; }\nconst users: User[] = [{name: "Alice", role: "user"}, {name: "Bob", role: "admin"}];',
    setupCode: 'interface User { name: string; role: "admin" | "user"; }\nconst users: User[] = [{name: "Alice", role: "user"}, {name: "Bob", role: "admin"}];',
  ...
  },
  {
    ...
    setup: 'interface User { id: number; name: string; email: string; age: number; }\nconst user: User = { id: 1, name: "Alice", email: "a@test.com", age: 30 };',
    setupCode: 'interface User { id: number; name: string; email: string; age: number; }\nconst user: User = { id: 1, name: "Alice", email: "a@test.com", age: 30 };',
  ...
  },
  {
    ...
    setup: 'interface User { id: number; name: string; password: string; }\nconst user: User = { id: 1, name: "Alice", password: "secret123" };',
    setupCode: 'interface User { id: number; name: string; password: string; }\nconst user: User = { id: 1, name: "Alice", password: "secret123" };',
    expected: { id: 1, name: 'Alice' },
  ...
  },
  {
    ...
    validPatterns: [
      /Record\s*<\s*Status\s*,\s*boolean\s*>/,
  ...
  },
  {
    ...
    setup: 'interface Config { host?: string; port?: number; debug?: boolean; }\nconst partial: Partial<Config> = { host: "localhost" };',
    setupCode: 'interface Config { host?: string; port?: number; debug?: boolean; }\nconst partial: Partial<Config> = { host: "localhost" };',
    expected: { host: 'localhost', port: 3000, debug: false },
    sample: '({ host: partial.host || "localhost", port: partial.port || 3000, debug: partial.debug || false } as Required<Config>)',
    hints: [
      'Required<T> makes all properties mandatory',
      'Provide default values for missing properties',
    ],
    validPatterns: [
      /Required\s*<\s*Config\s*>/,
      /host.*port.*debug/,
    ],
  ...
  },
  {
    ...
    setup: 'interface Config { apiKey: string; timeout: number; }\nconst config: Config = { apiKey: "abc123", timeout: 5000 };',
    setupCode: 'interface Config { apiKey: string; timeout: number; }\nconst config: Config = { apiKey: "abc123", timeout: 5000 };',
  ...
  },
  {
    ...
    validPatterns: [
      /Extract\s*<\s*Mixed\s*,\s*string\s*>/,
    ],
  ...
  },
  {
    ...
    validPatterns: [
      /Exclude\s*<\s*MaybeString\s*,\s*null\s*>/,
    ],
  ...
  // ============================================================
  {
    ...
    hints: [
      ...
      'Cast to (keyof T)[] for type safety',
    ],
    validPatterns: [
      /Object\.keys\s*\(/,
      /as\s*\(\s*keyof\s+User\s*\)\s*\[\s*\]/,
    ],
  ...
  },
  {
    ...
    validPatterns: [
      /Object\.entries\s*\(/,
      /as\s*\[\s*string\s*,\s*number\s*\]\s*\[\s*\]/,
    ],
  ...
  },
  {
    ...
    hints: [
      'Object.values extracts all values',
      'Return type is inferred from Record value type',
    ],
    validPatterns: [
      /Object\.values\s*\(\s*config\s*\)/,
    ],
    tags: ['object-values'],
  },
  {
    ...
    hints: [
      ...
      'Cast to Record for better typing',
    ],
    validPatterns: [
  ...
  },
  {
    ...
    sample: 'nested.flat()',
    hints: [
      'flat() flattens by depth 1 by default',
      'Pass depth for deeper flattening',
    ],
    validPatterns: [
      /\.flat\s*\(/,
    ],
  ...
  },
  {
    ...
    hints: [
      ...
      'Spread first to create a copy',
    ],
    validPatterns: [
      /\[\s*\.\.\.arr\s*\]\.reverse\s*\(\)|arr\.slice\s*\(\s*\)\.reverse\s*\(\)/,
    ],
  ...
  },
  {
    ...
    hints: [
      ...
      'fill fills all elements with value',
    ],
    validPatterns: [
      /Array\s*\(\s*size\s*\)\.fill\s*\(\s*0\s*\)|Array\.from\s*\(/,
    ],
  ...
  },
  {
    ...
    hints: [
      'Array.from accepts array-like object',
      'Second argument is map function',
    ],
    validPatterns: [
      /Array\.from\s*\(\s*\{\s*length/,
    ],
  ...
  // ============================================================
  {
    ...
    hints: [
      ...
      'Separator can be string or RegExp',
    ],
    validPatterns: [
      /\.split\s*\(\s*["']\s*["']\s*\)/,
    ],
  ...
  },
  {
    ...
    validPatterns: [
      /\.replace\s*\(\s*["']World["']\s*,\s*["']TypeScript["']\s*\)/,
    ],
    tags: ['replace'],
  },
  {
  ...
  },
  {
    ...
    setup: 'const setA: Set<number> = new Set([1, 2, 3]);\nconst setB: Set<number> = new Set([2, 3, 4]);',
    setupCode: 'const setA: Set<number> = new Set([1, 2, 3]);\nconst setB: Set<number> = new Set([2, 3, 4]);',
    expected: [2, 3],
  ...
  },
  {
    ...
    sample: '[...setA].filter((x) => !setB.has(x))',
    hints: [
      'Filter elements not in the other set',
      'Use negation with has()',
    ],
    validPatterns: [
      /\.filter\s*\(/,
      /!\s*setB\.has|!setB\.has/,
  ...
  // ============================================================
  {
    ...
    hints: [
      'Map constructor accepts array of entries',
      'Convert to object for comparison',
    ],
    validPatterns: [
      /new\s+Map\s*\(\s*entries\s*\)/,
    ],
    tags: ['Map', 'create'],
  },
  {
    ...
    hints: [
      ...
      'Use has() to check existence first',
    ],
    validPatterns: [
      /\.get\s*\(\s*["']name["']\s*\)/,
    ],
  ...
  },
  {
    ...
    hints: [
      'Object.fromEntries works with Map directly',
      'Map is iterable as entries',
    ],
    validPatterns: [
      /Object\.fromEntries\s*\(\s*map\s*\)/,
    ],
    tags: ['Map', 'conversion'],
  },
  {
  ...
  // ============================================================
  {
    ...
    validPatterns: [
      /identity\s*<\s*string\s*>\s*\(\s*["']hello["']\s*\)/,
    ],
  ...
  },
  {
    ...
    setup: 'function getProperty<T, K extends keyof T>(obj: T, key: K): T[K] { return obj[key]; }\nconst user = { name: "Alice", age: 30 };',
    setupCode: 'function getProperty<T, K extends keyof T>(obj: T, key: K): T[K] { return obj[key]; }\nconst user = { name: "Alice", age: 30 };',
    expected: 'Alice',
    sample: 'getProperty(user, "name")',
    hints: [
      'K is constrained to keys of T',
      'Return type T[K] is the value type',
    ],
    validPatterns: [
      /getProperty\s*\(\s*user\s*,\s*["']name["']\s*\)/,
    ],
    tags: ['generics', 'keyof'],
  },
  ...
  },
  {
    ...
    hints: [
      ...
      'Preserves literal type information',
    ],
    validPatterns: [
      /satisfies\s+Colors/,
    ],
...
export function getTypescriptProblemsByDifficulty(

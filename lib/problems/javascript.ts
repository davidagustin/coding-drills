...
const promises = [operation, timeout];`,
  ...
  // ========================================
  {
    ...
    validPatterns: [/\.filter\s*\([^)]+\)\s*\.map\s*\([^)]+\)/],
  ...
  },
  {
    ...
    validPatterns: [/\.filter\s*\([^)]*indexOf[^)]*\)/],
  ...
  },
  {
    ...
    validPatterns: [/\.flat\s*\(\s*Infinity\s*\)/],
  ...
  { name: "D", price: 40, inStock: true }
    ...
    validPatterns: [/\.filter\s*\([^)]+\)\s*\.map\s*\([^)]+\)\s*\.reduce\s*\([^)]+\)/],
  ...
  return groups;
    ...
    validPatterns: [/\.reduce\s*\([^)]*department[^)]*\{[^}]*\}\s*\)/],
  ...
  return chunks;
    ...
    validPatterns: [/\.reduce\s*\([^)]*Math\.floor[^)]*\)/, /Array\.from\s*\([^)]*slice[^)]*\)/],
  ...
  },
  {
    ...
    validPatterns: [/\.reduce\s*\(\s*\(\s*\w+\s*,\s*\[\s*\w+\s*,\s*\w+\s*\]\s*\)/],
  ...
  (numbers[i] + numbers[i + 1] + numbers[i + 2]) / 3
    ...
    validPatterns: [/\.map\s*\([^)]*\+[^)]*\+[^)]*\/\s*3/, /\.slice\s*\([^)]*\)\s*\.map/],
  ...
  },
  {
    ...
    validPatterns: [/\.flatMap\s*\(\s*\w+\s*=>\s*\[\s*\w+\s*,\s*\w+\s*\]\s*\)/],
  ...
  return counts;
    ...
    hints: ['Use an object as the accumulator', 'Initialize count to 0 if fruit not seen before'],
    validPatterns: [/\.reduce\s*\([^)]*\|\|\s*0[^)]*\+\s*1/],
  ...
  .map(n => [n, 10 - n])`,
    ...
    validPatterns: [/\.filter\s*\([^)]*10\s*-\s*\w+[^)]*\)/, /\.flatMap\s*\([^)]*10\s*-/],
  ...
  [[], []]
    ...
    validPatterns: [/\.reduce\s*\(\s*\(\s*\[\s*\w+\s*,\s*\w+\s*\]\s*,\s*\w+\s*\)/],
...
const ages = [25, 30, 35];`,
  ...
  [customer]: (totals[customer] || 0) + amount
    ...
    validPatterns: [/\.reduce\s*\([^)]*customer[^)]*amount[^)]*\)/],
  ...
  .reduce((counts, { role }) => ({
    ...
    validPatterns: [/\.filter\s*\([^)]+\)\s*\.reduce\s*\([^)]*role[^)]*\)/],
...
const arrays = [arr1, arr2, arr3];`,
    ...
    validPatterns: [/\.reduce\s*\([^)]*\.filter\s*\([^)]*\.includes\s*\([^)]*\)\s*\)/],
  ...
  [7, 8, 9]
    ...
    hints: ['Map over column indices', 'For each column, collect elements from each row'],
    validPatterns: [
  ...
  return a.price - b.price || a.name.localeCompare(b.name);
    ...
    validPatterns: [/\.sort\s*\([^)]*null[^)]*\)/, /\.sort\s*\([^)]*===\s*null[^)]*localeCompare/],
  ...
  // ========================================
  {
    ...
    validPatterns: [/new\s+Map\s*\(\s*entries\s*\)\.get\s*\(\s*['"]apple['"]\s*\)/],
  ...
  },
  {
    ...
    validPatterns: [/fruitMap\.has\s*\(\s*['"]grape['"]\s*\)/],
  ...
  },
  {
    ...
    hints: [
      'delete() returns true if element was present',
      'Returns false if element was not in Set',
    ],
    validPatterns: [/fruits\.delete\s*\(\s*['"]banana['"]\s*\)/],
  ...
  },
  {
    ...
    hints: [
      ...
      'Spread or Array.from to convert to array',
    ],
    validPatterns: [
...
let total = 0;`,
  ...
  },
  {
    ...
    validPatterns: [/new\s+Map\s*\(\s*Object\.entries\s*\(\s*person\s*\)\s*\)/],
  ...
  },
  {
    ...
    hints: [
      'Object.fromEntries accepts iterables of [key, value] pairs',
...
const computeValue = () => 42;`,
    ...
    hints: [
      'Check with has() before getting',
      'set() returns the Map, allowing chaining with get()',
    ],
    validPatterns: [
...
const userScores = new Map([[user1, 100], [user2, 200]]);`,
    ...
    validPatterns: [/userScores\.get\s*\(\s*user1\s*\)/],
...
const set2 = new Set([3, 4, 5, 6]);`,
    ...
    validPatterns: [/\[\s*\.\.\.new\s+Set\s*\(\s*\[\s*\.\.\.set1\s*,\s*\.\.\.set2\s*\]\s*\)\s*\]/],
...
const setB = new Set([4, 5, 6, 7, 8]);`,
  ...
  },
  {
    ...
    hints: [
      ...
      'Map over entries and transform values',
      'Create new Map from result',
    ],
    validPatterns: [
...
privateData.set(user, { name: "Secret Alice" });`,
    ...
    validPatterns: [/privateData\.get\s*\(\s*user\s*\)\.name/],
  ...
  },
  {
    ...
    hints: [
      ...
      'Create new Map from filtered entries',
    ],
    validPatterns: [
  ...
  return map.set(item.category, group);
    ...
    hints: [
      'Use reduce with Map as accumulator',
...
const factorial = n => {
  ...
  return result;
    ...
    validPatterns: [/factorial\s*\(\s*5\s*\)/],
...
const user = { id: 1, name: "Alice" };`,
    ...
    hints: [
      'WeakSet tracks object references',
      'add() returns the WeakSet',
...
const map3 = new Map([["c", 5], ["d", 6]]);`,
    ...
    hints: [
      'Spread all Maps into a single array',
      'Later entries overwrite earlier ones',
      'Map constructor deduplicates by key',
    ],
    validPatterns: [/new\s+Map\s*\(\s*\[\s*\.\.\.map1\s*,\s*\.\.\.map2\s*,\s*\.\.\.map3\s*\]\s*\)/],
...
const setY = new Set([4, 5, 6, 7, 8]);`,
    ...
    sample:
      '[...new Set([...[...setX].filter(x => !setY.has(x)), ...[...setY].filter(x => !setX.has(x))])]',
...
Date: 2024-01-15`,
  ...
  },
  {
    ...
    hints: [
      ...
      'Spread or Array.from to convert',
      'Need global flag /g',
    ],
...
const city = "paris";`,
  ...
  },
  {
    ...
    sample:
      'const m = phone.match(/\\((\\d+)\\)\\s*(\\d+-\\d+)/); ({ areaCode: m[1], number: m[2] })',
...
const passingScore = 70;`,
  ...
  },
  {
    ...
    sample:
      '[...css.matchAll(/(?<property>[\\w-]+):\\s*(?<value>[^;]+)/g)].map(m => ({ property: m.groups.property, value: m.groups.value }))',
...
const userInput = '<script>alert("xss")</script>';`,
  ...
  // Easy (8 problems)
  {
    ...
    setup: 'const car = { make: "Toyota", model: "Camry", year: 2022, color: "blue" };',
    setupCode: 'const car = { make: "Toyota", model: "Camry", year: 2022, color: "blue" };',
    expected: 4,
    sample: 'Object.keys(car).length',
    hints: ['Object.keys returns array of keys', 'Arrays have a length property'],
    validPatterns: [
  ...
  },
  {
    ...
    setup: 'const scores = { math: 95, science: 88, english: 92, history: 87 };',
    setupCode: 'const scores = { math: 95, science: 88, english: 92, history: 87 };',
    expected: 362,
  ...
  },
  {
    ...
    expected: { a: 1, b: 2, c: 3 },
    sample: '{ ...original }',
    hints: ['Spread operator copies properties', 'Creates a new object reference'],
  ...
  },
  {
    ...
    hints: ['Later properties override earlier ones', 'Spread first, then override'],
  ...
  },
  {
    ...
    setup: 'const pairs = [["name", "Charlie"], ["role", "developer"], ["level", "senior"]];',
    setupCode: 'const pairs = [["name", "Charlie"], ["role", "developer"], ["level", "senior"]];',
    expected: { name: 'Charlie', role: 'developer', level: 'senior' },
    sample: 'Object.fromEntries(pairs)',
    hints: ['Object.fromEntries takes array of pairs', 'Each pair is [key, value]'],
...
const source = { b: 2, c: 3 };`,
    ...
    hints: ['Object.assign merges objects', 'First arg is target, rest are sources'],
    validPatterns: [
  ...
  // Medium (12 problems)
  {
    ...
    sample: 'Object.fromEntries(Object.entries(scores).filter(([_, v]) => v > 50))',
    hints: ['Use entries to get [key, value] pairs', 'Filter pairs, then fromEntries'],
    validPatterns: [/Object\.fromEntries\s*\(\s*Object\.entries\s*\(\s*scores\s*\)\.filter/],
  ...
  },
  {
    ...
    setup: 'const user = { first_name: "John", last_name: "Doe", user_age: 30 };',
    setupCode: 'const user = { first_name: "John", last_name: "Doe", user_age: 30 };',
    ...
    hints: ['Use entries and map to transform keys', 'Replace _x with uppercase X'],
    validPatterns: [/Object\.fromEntries\s*\(\s*Object\.entries\s*\(\s*user\s*\)\.map/],
  ...
  },
  {
    ...
    sample: 'Object.fromEntries(Object.entries(original).map(([k, v]) => [v, k]))',
    hints: ['Map entries to flip the pairs', 'Return [value, key] instead of [key, value]'],
    validPatterns: [
  ...
  },
  {
    ...
    setup: 'const user = { name: "Alice", email: "alice@example.com", age: 30, role: "admin" };',
    setupCode:
    ...
    hints: ['Destructure desired properties', 'Return new object with only those'],
    validPatterns: [
  ...
  },
  {
    ...
    setup: 'const user = { name: "Bob", email: "bob@example.com", password: "secret123" };',
    setupCode: 'const user = { name: "Bob", email: "bob@example.com", password: "secret123" };',
    expected: { name: 'Bob', email: 'bob@example.com' },
    sample: '(({ password, ...rest }) => rest)(user)',
    hints: ['Destructure the property to omit', 'Use rest operator for remaining'],
    validPatterns: [/\{\s*password\s*,\s*\.\.\.rest\s*\}/, /\{\s*password\s*,\s*\.\.\.(\w+)\s*\}/],
...
const sessionPrefs = { size: "large" };`,
  ...
  },
  {
    ...
    setup: 'const words = { greeting: "hello", farewell: "goodbye", question: "how" };',
    setupCode: 'const words = { greeting: "hello", farewell: "goodbye", question: "how" };',
    expected: { greeting: 'HELLO', farewell: 'GOODBYE', question: 'HOW' },
    sample: 'Object.fromEntries(Object.entries(words).map(([k, v]) => [k, v.toUpperCase()]))',
    hints: [
      'Use entries to iterate with keys',
      'Map and transform values',
      'Rebuild with fromEntries',
    ],
    validPatterns: [/Object\.fromEntries\s*\(\s*Object\.entries\s*\(\s*words\s*\)\.map/],
...
const extension = { z: 30 };`,
    ...
    hints: ['Object.assign copies enumerable properties', 'First arg is target object'],
    validPatterns: [/Object\.assign\s*\(\s*\{\s*\}\s*,\s*base\s*,\s*extension\s*\)/],
  ...
  },
  {
    ...
    setup: 'const prices = { apple: 1.5, banana: 0.75, mango: 2.5, orange: 1.25 };',
    setupCode: 'const prices = { apple: 1.5, banana: 0.75, mango: 2.5, orange: 1.25 };',
    expected: 'mango',
    sample: 'Object.entries(prices).reduce((a, b) => b[1] > a[1] ? b : a)[0]',
    hints: ['Use entries to get [key, value] pairs', 'Reduce to find max, then extract key'],
    validPatterns: [
  ...
  },
  {
    ...
    hints: ['Use entries to get key-value pairs', 'Map to "key=value" strings', 'Join with &'],
    validPatterns: [
...
const source = { user: { settings: { fontSize: 14 } } };`,
    ...
    hints: ['Spread alone does shallow merge', 'Manually spread nested objects'],
    validPatterns: [
  ...
  },
  {
    ...
    hints: ['Object.freeze prevents changes', 'Object.isFrozen checks freeze status'],
    validPatterns: [/Object\.freeze\s*\(\s*config\s*\)/, /Object\.isFrozen\s*\(\s*config\s*\)/],
  ...
  // Hard (5 problems)
  {
    ...
    hints: ['defineProperty adds/modifies properties', 'Use getter for computed value'],
    validPatterns: [/Object\.defineProperty\s*\(\s*person\s*,\s*['"]fullName['"]/],
...
Object.defineProperty(obj, "name", { value: "Immutable", writable: false, enumerable: true, configurable: false });`,
    ...
    hints: ['getOwnPropertyDescriptor returns descriptor object', 'Check the writable property'],
    validPatterns: [
  ...
  },
  {
    ...
    hints: ['set trap intercepts assignments', 'Validate before setting', 'Return true on success'],
...
const logs = [];`,
  ...
  },
  {
    ...
    hints: [
      'Use * quantifier for zero or more',
      'The pattern goo* matches g followed by one or more o',
    ],
    tags: ['regex', 'match', 'quantifier'],
  },
  {
  ...
  },
  {
    ...
    hints: [
      'Use lookahead to keep last 4 digits',
      'Replace each digit that has 4+ digits after it',
    ],
  ...
  },
  {
    ...
    hints: [
      'Match protocol, domain, optional port, path, query, hash',
      'Each part is optional except protocol and domain',
    ],
  ...
  },
  {
    ...
    hints: [
      'Match http:// or https:// followed by domain and optional path',
      'Use optional group for path',
    ],
  ...
  },
  {
    ...
    hints: [
      'Optional negative, optional integer part, optional decimal',
  ...
  },
  {
    ...
    sample:
      '[...md.matchAll(/\\[([^\\]]+)\\]\\(([^)]+)\\)/g)].map(m => ({ text: m[1], url: m[2] }))',
  ...
  },
  {
    ...
    expected: true,
    sample:
      '/^(?=(?:.*[A-Z]){2})(?=(?:.*[a-z]){2})(?=(?:.*\\d){2})(?!.*(.)\\1).{8,20}$/.test(password)',
  ...
  },
  {
    ...
    setup:
      'const fast = Promise.resolve("fast"); const slow = new Promise(r => setTimeout(() => r("slow"), 100));',
    setupCode:
      'const fast = Promise.resolve("fast"); const slow = new Promise(r => setTimeout(() => r("slow"), 100));',
    expected: 'fast',
  ...
  },
  {
    ...
    setup:
      'const promises = [Promise.reject("err1"), Promise.resolve("success"), Promise.reject("err2")];',
    setupCode:
      'const promises = [Promise.reject("err1"), Promise.resolve("success"), Promise.reject("err2")];',
    expected: 'success',
    sample: 'await Promise.any(promises)',
  ...
  },
  {
    ...
    setup:
      'const order = []; setTimeout(() => order.push("timeout"), 0); Promise.resolve().then(() => order.push("promise"));',
    setupCode:
      'const order = []; setTimeout(() => order.push("timeout"), 0); Promise.resolve().then(() => order.push("promise"));',
    ...
    hints: [
      ...
      'Promise.then is a microtask, setTimeout is macrotask',
    ],
  ...
  },
  {
    ...
    expected: [1, 2, 3],
    sample:
      'await tasks.reduce(async (acc, fn) => [...await acc, await fn()], Promise.resolve([]))',
    hints: [
      'Use reduce for sequential execution',
      'Await the accumulator before adding new result',
    ],
  ...
  },
  {
    ...
    setup:
      'const slow = new Promise(r => setTimeout(() => r("slow"), 100)); const timeout = new Promise((_, rej) => setTimeout(() => rej("timeout"), 50));',
    setupCode:
      'const slow = new Promise(r => setTimeout(() => r("slow"), 100)); const timeout = new Promise((_, rej) => setTimeout(() => rej("timeout"), 50));',
    expected: 'timeout',
  ...
  },
  {
    ...
    expected: 10,
    sample:
      'await new Promise((resolve, reject) => callbackFn(5, (err, result) => err ? reject(err) : resolve(result)))',
    hints: [
      'Wrap the callback function in new Promise',
      'Call resolve or reject based on callback args',
    ],
  ...
  },
  {
    ...
    sample:
      '(() => { order.push("sync"); queueMicrotask(() => order.push("microtask")); setTimeout(() => order.push("timeout"), 0); return new Promise(r => setTimeout(r, 20)); })().then(() => order)',
  ...
  },
  {
    ...
    expected: [1, 3],
    sample:
      'await Promise.allSettled(promises).then(results => results.filter(r => r.status === "fulfilled").map(r => r.value))',
  ...
  },
  {
    ...
    expected: [2, 4],
    sample:
      'await Promise.all(nums.map(async n => ({ n, keep: await asyncIsEven(n) }))).then(results => results.filter(r => r.keep).map(r => r.n))',
  ...
  },
  {
    ...
    hints: [
      ...
      'Access errors property for all rejection reasons',
    ],
  ...
  },
  {
    ...
    setup:
      'const order = []; Promise.resolve().then(() => order.push(1)).then(() => order.push(2)); Promise.resolve().then(() => order.push(3));',
    setupCode:
      'const order = []; Promise.resolve().then(() => order.push(1)).then(() => order.push(2)); Promise.resolve().then(() => order.push(3));',
    expected: [1, 3, 2],
    sample: 'await new Promise(r => setTimeout(r, 10)).then(() => order)',
  ...
  },
  {
    ...
    setup:
      'const chain = Promise.resolve(10).then(x => { throw "error"; }).catch(() => 0).then(x => x + 5);',
    setupCode:
      'const chain = Promise.resolve(10).then(x => { throw "error"; }).catch(() => 0).then(x => x + 5);',
    expected: 5,
    sample: 'await chain',
  ...
  },
  {
    ...
    setup:
      'class AsyncClass { constructor(data) { this.data = data; } static async create() { const data = await Promise.resolve("loaded"); return new AsyncClass(data); } }',
    setupCode:
      'class AsyncClass { constructor(data) { this.data = data; } static async create() { const data = await Promise.resolve("loaded"); return new AsyncClass(data); } }',
    expected: 'loaded',
  ...
  },
  {
    ...
    setup:
      'const tasks = [() => Promise.resolve(1), () => Promise.resolve(2), () => Promise.resolve(3), () => Promise.resolve(4)];',
    setupCode:
      'const tasks = [() => Promise.resolve(1), () => Promise.resolve(2), () => Promise.resolve(3), () => Promise.resolve(4)];',
    expected: [1, 2, 3, 4],
    sample:
      '(async () => { const results = []; for (let i = 0; i < tasks.length; i += 2) { results.push(...await Promise.all(tasks.slice(i, i + 2).map(t => t()))); } return results; })()',
  ...
  },
  {
    ...
    expected: ['promise', 'timeout'],
    sample:
      '(() => { setTimeout(() => order.push("timeout"), 0); Promise.resolve().then(() => order.push("promise")); return new Promise(r => setTimeout(r, 20)); })().then(() => order)',
  ...
  },
  {
    ...
    expected: [1, 2, 3],
    sample:
      '(async () => { const result = []; for await (const x of gen()) result.push(x); return result; })()',
  ...
  },
  {
    ...
    setup:
      'const p1 = new Promise(r => setTimeout(() => r("first"), 10)); const p2 = new Promise(r => setTimeout(() => r("second"), 50));',
    setupCode:
      'const p1 = new Promise(r => setTimeout(() => r("first"), 10)); const p2 = new Promise(r => setTimeout(() => r("second"), 50));',
    expected: 'first',
  ...
  },
  {
    ...
    expected: [1, 2, 3],
    sample:
      '(async () => { for (const item of items) { results.push(await Promise.resolve(item)); } return results; })()',
  ...
  },
  {
    ...
    sample:
      '(() => { setTimeout(() => { order.push("outer"); setTimeout(() => order.push("inner"), 0); }, 0); return new Promise(r => setTimeout(r, 50)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'let attempts = 0; const flaky = async () => { attempts++; if (attempts < 3) throw "fail"; return "success"; };',
    setupCode:
      'let attempts = 0; const flaky = async () => { attempts++; if (attempts < 3) throw "fail"; return "success"; };',
    expected: 'success',
    sample:
      '(async () => { for (let i = 0; i < 3; i++) { try { return await flaky(); } catch (e) { if (i === 2) throw e; } } })()',
  ...
  },
  {
    ...
    setup:
      'const p1 = new Promise((_, rej) => setTimeout(() => rej("fast reject"), 10)); const p2 = new Promise(r => setTimeout(() => r("slow"), 100)); let p2Resolved = false; p2.then(() => p2Resolved = true);',
    setupCode:
      'const p1 = new Promise((_, rej) => setTimeout(() => rej("fast reject"), 10)); const p2 = new Promise(r => setTimeout(() => r("slow"), 100)); let p2Resolved = false; p2.then(() => p2Resolved = true);',
  ...
  },
  {
    ...
    setup:
      'const asyncIterable = { [Symbol.asyncIterator]() { let i = 0; return { async next() { if (i < 3) return { value: ++i, done: false }; return { done: true }; } }; } };',
    setupCode:
      'const asyncIterable = { [Symbol.asyncIterator]() { let i = 0; return { async next() { if (i < 3) return { value: ++i, done: false }; return { done: true }; } }; } };',
    expected: [1, 2, 3],
    sample:
      '(async () => { const result = []; for await (const x of asyncIterable) result.push(x); return result; })()',
  ...
  },
  {
    ...
    sample:
      '(() => { order.push("sync"); Promise.resolve().then(() => { order.push("micro1"); Promise.resolve().then(() => order.push("micro2")); }); setTimeout(() => order.push("timeout"), 0); return new Promise(r => setTimeout(r, 20)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'const controller = new AbortController(); const cancellable = (signal) => new Promise((resolve, reject) => { const id = setTimeout(() => resolve("done"), 100); signal.addEventListener("abort", () => { clearTimeout(id); reject("cancelled"); }); });',
    setupCode:
      'const controller = new AbortController(); const cancellable = (signal) => new Promise((resolve, reject) => { const id = setTimeout(() => resolve("done"), 100); signal.addEventListener("abort", () => { clearTimeout(id); reject("cancelled"); }); });',
    expected: 'cancelled',
    sample:
      '(() => { const p = cancellable(controller.signal); setTimeout(() => controller.abort(), 10); return p.catch(e => e); })()',
  ...
  },
  {
    ...
    setup:
      'const promises = [Promise.resolve(1), Promise.reject("a"), Promise.resolve(2), Promise.reject("b"), Promise.resolve(3)];',
    setupCode:
      'const promises = [Promise.resolve(1), Promise.reject("a"), Promise.resolve(2), Promise.reject("b"), Promise.resolve(3)];',
    expected: { fulfilled: 3, rejected: 2 },
    sample:
      'await Promise.allSettled(promises).then(results => results.reduce((acc, r) => { acc[r.status]++; return acc; }, { fulfilled: 0, rejected: 0 }))',
  ...
  },
  {
    ...
    setup:
      'const delay = (ms, val) => new Promise(r => setTimeout(() => r(val), ms)); const tasks = [() => delay(30, 1), () => delay(20, 2), () => delay(10, 3), () => delay(40, 4)];',
    setupCode:
      'const delay = (ms, val) => new Promise(r => setTimeout(() => r(val), ms)); const tasks = [() => delay(30, 1), () => delay(20, 2), () => delay(10, 3), () => delay(40, 4)];',
    expected: [1, 2, 3, 4],
    sample:
      '(async () => { const results = []; const executing = []; for (const task of tasks) { const p = task().then(r => { executing.splice(executing.indexOf(p), 1); return r; }); results.push(p); executing.push(p); if (executing.length >= 2) await Promise.race(executing); } return Promise.all(results); })()',
  ...
  },
  {
    ...
    setup:
      'const reject = new Promise((_, r) => setTimeout(() => r("error"), 10)); const resolve = new Promise(r => setTimeout(() => r("success"), 50));',
    setupCode:
      'const reject = new Promise((_, r) => setTimeout(() => r("error"), 10)); const resolve = new Promise(r => setTimeout(() => r("success"), 50));',
    expected: 'error',
  ...
  },
  {
    ...
    sample:
      '(() => { setTimeout(() => { order.push("timeout1"); Promise.resolve().then(() => order.push("micro")); }, 0); setTimeout(() => order.push("timeout2"), 0); return new Promise(r => setTimeout(r, 30)); })().then(() => order)',
    hints: [
      'Each setTimeout callback gets its own microtask queue',
  ...
  },
  {
    ...
    setup:
      'class Mutex { constructor() { this.locked = false; this.queue = []; } async acquire() { if (this.locked) { await new Promise(r => this.queue.push(r)); } this.locked = true; } release() { this.locked = false; if (this.queue.length > 0) this.queue.shift()(); } }',
    setupCode:
      'class Mutex { constructor() { this.locked = false; this.queue = []; } async acquire() { if (this.locked) { await new Promise(r => this.queue.push(r)); } this.locked = true; } release() { this.locked = false; if (this.queue.length > 0) this.queue.shift()(); } }',
    expected: [1, 2, 3],
    sample:
      '(async () => { const mutex = new Mutex(); const results = []; const task = async (n) => { await mutex.acquire(); results.push(n); mutex.release(); }; await Promise.all([task(1), task(2), task(3)]); return results; })()',
  ...
  },
  {
    ...
    setup:
      'const p1 = Promise.resolve(1); const p2 = p1.then(x => x + 1); const p3 = p1.then(x => x + 2);',
    setupCode:
      'const p1 = Promise.resolve(1); const p2 = p1.then(x => x + 1); const p3 = p1.then(x => x + 2);',
    expected: [2, 3],
  ...
  },
  {
    ...
    setup:
      'class Emitter { constructor() { this.listeners = {}; } on(event, cb) { (this.listeners[event] ||= []).push(cb); } emit(event, data) { (this.listeners[event] || []).forEach(cb => cb(data)); } } const emitter = new Emitter();',
    setupCode:
      'class Emitter { constructor() { this.listeners = {}; } on(event, cb) { (this.listeners[event] ||= []).push(cb); } emit(event, data) { (this.listeners[event] || []).forEach(cb => cb(data)); } } const emitter = new Emitter();',
    expected: 'event data',
    sample:
      '(() => { const promise = new Promise(r => emitter.on("data", r)); setTimeout(() => emitter.emit("data", "event data"), 10); return promise; })()',
  ...
  },
  {
    ...
    sample:
      '(() => { Promise.resolve().then(() => { order.push("p1"); Promise.resolve().then(() => order.push("p1-inner")); }); Promise.resolve().then(() => { order.push("p2"); Promise.resolve().then(() => order.push("p2-inner")); }); return new Promise(r => setTimeout(r, 20)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'async function* delayedGen() { for (let i = 1; i <= 3; i++) { await new Promise(r => setTimeout(r, 10)); yield i; } }',
    setupCode:
      'async function* delayedGen() { for (let i = 1; i <= 3; i++) { await new Promise(r => setTimeout(r, 10)); yield i; } }',
    expected: [1, 2, 3],
    sample:
      '(async () => { const result = []; for await (const x of delayedGen()) result.push(x); return result; })()',
  ...
  },
  {
    ...
    setup:
      'const p1 = Promise.reject("err"); const p2 = new Promise(r => setTimeout(() => r("ok"), 20));',
    setupCode:
      'const p1 = Promise.reject("err"); const p2 = new Promise(r => setTimeout(() => r("ok"), 20));',
    expected: { race: 'err', any: 'ok' },
    sample:
      'await Promise.all([Promise.race([p1, p2]).catch(e => e), Promise.any([p1, p2])]).then(([race, any]) => ({ race, any }))',
  ...
  },
  {
    ...
    setup:
      'let cleanup = false; const createPromise = () => { const id = setTimeout(() => {}, 1000); return { promise: new Promise(r => setTimeout(r, 100)), cancel: () => { clearTimeout(id); cleanup = true; } }; };',
    setupCode:
      'let cleanup = false; const createPromise = () => { const id = setTimeout(() => {}, 1000); return { promise: new Promise(r => setTimeout(r, 100)), cancel: () => { clearTimeout(id); cleanup = true; } }; };',
    expected: true,
    sample:
      '(async () => { const { promise, cancel } = createPromise(); cancel(); return cleanup; })()',
  ...
  },
  {
    ...
    sample:
      '(() => { new Promise(async (resolve) => { order.push("constructor sync"); await Promise.resolve(); order.push("inner resolved"); resolve(); }); order.push("outer sync"); return new Promise(r => setTimeout(r, 20)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'class Semaphore { constructor(n) { this.n = n; this.queue = []; } async acquire() { if (this.n > 0) { this.n--; return; } await new Promise(r => this.queue.push(r)); } release() { if (this.queue.length > 0) { this.queue.shift()(); } else { this.n++; } } }',
    setupCode:
      'class Semaphore { constructor(n) { this.n = n; this.queue = []; } async acquire() { if (this.n > 0) { this.n--; return; } await new Promise(r => this.queue.push(r)); } release() { if (this.queue.length > 0) { this.queue.shift()(); } else { this.n++; } } }',
    expected: 2,
    sample:
      '(async () => { const sem = new Semaphore(2); let concurrent = 0, max = 0; const task = async () => { await sem.acquire(); concurrent++; max = Math.max(max, concurrent); await new Promise(r => setTimeout(r, 20)); concurrent--; sem.release(); }; await Promise.all([task(), task(), task(), task()]); return max; })()',
  ...
  },
  {
    ...
    sample:
      'await Promise.resolve().then(() => order.push("then")).finally(() => order.push("finally")).then(() => order.push("after")).then(() => order)',
  ...
  },
  {
    ...
    setup:
      'class AsyncQueue { constructor() { this.queue = []; this.processing = false; } async add(fn) { return new Promise((resolve, reject) => { this.queue.push({ fn, resolve, reject }); this.process(); }); } async process() { if (this.processing) return; this.processing = true; while (this.queue.length > 0) { const { fn, resolve, reject } = this.queue.shift(); try { resolve(await fn()); } catch (e) { reject(e); } } this.processing = false; } }',
    setupCode:
      'class AsyncQueue { constructor() { this.queue = []; this.processing = false; } async add(fn) { return new Promise((resolve, reject) => { this.queue.push({ fn, resolve, reject }); this.process(); }); } async process() { if (this.processing) return; this.processing = true; while (this.queue.length > 0) { const { fn, resolve, reject } = this.queue.shift(); try { resolve(await fn()); } catch (e) { reject(e); } } this.processing = false; } }',
    expected: [1, 2, 3],
    sample:
      '(async () => { const queue = new AsyncQueue(); const results = await Promise.all([queue.add(() => Promise.resolve(1)), queue.add(() => Promise.resolve(2)), queue.add(() => Promise.resolve(3))]); return results; })()',
  ...
  },
  {
    ...
    setup:
      'const inner = async () => { throw new Error("inner error"); }; const outer = async () => { await inner(); };',
    setupCode:
      'const inner = async () => { throw new Error("inner error"); }; const outer = async () => { await inner(); };',
  ...
  },
  {
    ...
    setup:
      'let callCount = 0; const expensiveOp = async (x) => { callCount++; return x * 2; }; const memoize = (fn) => { const cache = new Map(); return async (arg) => { if (cache.has(arg)) return cache.get(arg); const result = await fn(arg); cache.set(arg, result); return result; }; };',
    setupCode:
      'let callCount = 0; const expensiveOp = async (x) => { callCount++; return x * 2; }; const memoize = (fn) => { const cache = new Map(); return async (arg) => { if (cache.has(arg)) return cache.get(arg); const result = await fn(arg); cache.set(arg, result); return result; }; };',
    expected: { result: 10, calls: 1 },
    sample:
      '(async () => { const memoized = memoize(expensiveOp); await memoized(5); const result = await memoized(5); return { result, calls: callCount }; })()',
  ...
  },
  {
    ...
    sample:
      '(() => { Promise.resolve().then(() => order.push("resolve")); new Promise(r => r()).then(() => order.push("new")); order.push("sync"); return new Promise(r => setTimeout(r, 10)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'const pipe = (...fns) => async (initial) => fns.reduce(async (acc, fn) => fn(await acc), initial);',
    setupCode:
      'const pipe = (...fns) => async (initial) => fns.reduce(async (acc, fn) => fn(await acc), initial);',
    expected: 25,
  ...
  },
  {
    ...
    sample:
      '(() => { Promise.resolve().then(() => order.push("micro")); simulateRAF(() => order.push("raf")); setTimeout(() => order.push("timeout"), 20); return new Promise(r => setTimeout(r, 50)); })().then(() => order)',
  ...
  },
  {
    ...
    setup:
      'class AsyncContext { constructor() { this.storage = new Map(); } run(key, value, fn) { this.storage.set(key, value); return fn().finally(() => this.storage.delete(key)); } get(key) { return this.storage.get(key); } }',
    setupCode:
      'class AsyncContext { constructor() { this.storage = new Map(); } run(key, value, fn) { this.storage.set(key, value); return fn().finally(() => this.storage.delete(key)); } get(key) { return this.storage.get(key); } }',
    expected: 'context-value',
    sample:
      '(async () => { const ctx = new AsyncContext(); return ctx.run("key", "context-value", async () => { await Promise.resolve(); return ctx.get("key"); }); })()',
  ...
  },
  {
    ...
    setup:
      'const p1 = new Promise(r => setTimeout(() => r(1), 30)); const p2 = new Promise(r => setTimeout(() => r(2), 10)); const p3 = new Promise(r => setTimeout(() => r(3), 20));',
    setupCode:
      'const p1 = new Promise(r => setTimeout(() => r(1), 30)); const p2 = new Promise(r => setTimeout(() => r(2), 10)); const p3 = new Promise(r => setTimeout(() => r(3), 20));',
    expected: [1, 2, 3],
  ...
  },
  {
    ...
    sample:
      'await Promise.allSettled(promises).then(results => results.map(r => r.status === "fulfilled" ? { success: true, data: r.value } : { success: false, error: r.reason }))',
  ...
  },
  {
    ...
    expected: 4,
    sample:
      '(async () => { for (const item of items) { if (await asyncCheck(item)) return item; } })()',
  ...
  },
  {
    ...
    sample:
      '(() => { const race = Promise.race([]); let resolved = false; race.then(() => resolved = true); return new Promise(r => setTimeout(r, 50)).then(() => resolved ? "resolved" : "pending forever"); })()',
  ...
  },
  {
    ...
    expected: 3,
    sample:
      '(() => { return new Promise(resolve => { const id = setInterval(() => { count++; if (count >= 3) { clearInterval(id); resolve(count); } }, 10); }); })()',
  ...
  },
  {
    ...
    expected: 'timeout',
    sample:
      'await withTimeout(new Promise(r => setTimeout(() => r("slow"), 100)), 20).catch(e => e)',
  ...
  },
  {
    ...
    setup:
      'const debounceAsync = (fn, ms) => { let timeout; return (...args) => { clearTimeout(timeout); return new Promise(resolve => { timeout = setTimeout(() => resolve(fn(...args)), ms); }); }; };',
    setupCode:
      'const debounceAsync = (fn, ms) => { let timeout; return (...args) => { clearTimeout(timeout); return new Promise(resolve => { timeout = setTimeout(() => resolve(fn(...args)), ms); }); }; };',
    expected: 'final',
    sample:
      '(async () => { const debounced = debounceAsync(x => x, 30); debounced("first"); debounced("second"); return debounced("final"); })()',
  ...
  },
  {
    ...
    expected: true,
    sample:
      '(async () => { let eventLoopYielded = false; setTimeout(() => eventLoopYielded = true, 0); await yieldToEventLoop(); return eventLoopYielded; })()',
  ...
  },
  {
    ...
    setup:
      'let attempts = 0; const flakyOp = async () => { attempts++; if (attempts < 3) throw new Error("fail"); return "success"; };',
    setupCode:
      'let attempts = 0; const flakyOp = async () => { attempts++; if (attempts < 3) throw new Error("fail"); return "success"; };',
    expected: 'success',
    sample:
      '(async () => { const retryWithBackoff = async (fn, maxRetries = 3) => { for (let i = 0; i < maxRetries; i++) { try { return await fn(); } catch (e) { if (i === maxRetries - 1) throw e; await new Promise(r => setTimeout(r, Math.pow(2, i) * 10)); } } }; return retryWithBackoff(flakyOp); })()',
  ...
  },
  {
    ...
    setup:
      'const boundary = (fn) => (value) => Promise.resolve(value).then(fn).catch(e => ({ error: e.message }));',
    setupCode:
      'const boundary = (fn) => (value) => Promise.resolve(value).then(fn).catch(e => ({ error: e.message }));',
    expected: { error: 'boom' },
  ...
  },
  {
    ...
    sample:
      'try { if (value < 0) throw new Error("Invalid input"); } catch (e) { return e.message; }',
  ...
  },
  {
    ...
    expected: 'cleaned',
    sample:
      'try { throw new Error("fail"); } catch (e) {} finally { result = "cleaned"; } return result;',
  ...
  },
  {
    ...
    sample:
      'try { throw new Error("fail"); } catch (e) { logs.push("Error caught"); } return logs;',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'try { recurse(); } catch (e) { return e.message.toLowerCase().includes("stack") || e.name === "RangeError"; }',
  ...
  },
  {
    ...
    setupCode:
      'function throwWithCode() { const e = new Error("Failed"); e.code = "E001"; throw e; }',
    expected: 'E001',
  ...
  },
  {
    ...
    expected: 'handled',
    sample:
      'try { maybeThrow(); } catch (e) { if (!(e instanceof TypeError)) throw e; return "handled"; }',
  ...
  },
  {
    ...
    sample:
      'try { throw new Error("Operation failed", { cause: originalError }); } catch (e) { return e.cause.message; }',
  ...
  },
  {
    ...
    sample:
      'try { try { inner(); } catch (e) { return e.message + "-handled"; } } catch (e) { return "outer"; }',
  ...
  },
  {
    ...
    expected: 3,
    sample:
      'try { throw new AggregateError(errors, "Multiple errors"); } catch (e) { return e.errors.length; }',
  ...
  },
  {
    ...
    sample:
      'try { dbQuery(); } catch (e) { const wrapped = new Error(`Failed to fetch user: ${e.message}`); return wrapped.message; }',
  ...
  },
  {
    ...
    expected: 5,
    sample:
      'if (typeof value !== "string") throw new TypeError("Expected string"); return value.length;',
  ...
  },
  {
    ...
    expected: 'range',
    sample:
      'try { randomError(); } catch (e) { if (e instanceof RangeError) return "range"; if (e instanceof TypeError) return "type"; return "other"; }',
  ...
  },
  {
    ...
    setup:
      'function test() { try { throw new Error("try"); } finally { throw new Error("finally"); } }',
    setupCode:
      'function test() { try { throw new Error("try"); } finally { throw new Error("finally"); } }',
    expected: 'finally',
  ...
  },
  {
    ...
    sample:
      'try { if (!data.name) throw new Error("Name required"); if (data.age < 0) throw new Error("Invalid age"); return "valid"; } catch (e) { return e.message; }',
  ...
  },
  {
    ...
    setup:
      'function primary() { throw new Error("fail"); } function secondary() { return "backup"; }',
    setupCode:
      'function primary() { throw new Error("fail"); } function secondary() { return "backup"; }',
    expected: 'backup',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'try { original(); } catch (e) { const wrapped = new Error("wrapped", { cause: e }); return wrapped.cause.stack.includes("original"); }',
  ...
  },
  {
    ...
    setup:
      'class InternalError extends Error { constructor(msg) { super(msg); this.name = "InternalError"; } }',
    setupCode:
      'class InternalError extends Error { constructor(msg) { super(msg); this.name = "InternalError"; } }',
    expected: 'InternalError: Stack overflow',
    sample:
      'try { throw new InternalError("Stack overflow"); } catch (e) { return `${e.name}: ${e.message}`; }',
  ...
  },
  {
    ...
    setup:
      'function validate(user) { if (!user) return { error: "No user" }; if (!user.email) return { error: "No email" }; return { valid: true }; }',
    setupCode:
      'function validate(user) { if (!user) return { error: "No user" }; if (!user.email) return { error: "No email" }; return { valid: true }; }',
  ...
  },
  {
    ...
    setup:
      'function handleStatus(code) { switch(code) { case 200: return "OK"; case 404: return "Not Found"; default: throw new Error("Unknown: " + code); } }',
    setupCode:
      'function handleStatus(code) { switch(code) { case 200: return "OK"; case 404: return "Not Found"; default: throw new Error("Unknown: " + code); } }',
  ...
  },
  {
    ...
    setupCode:
      'const obj = { set value(v) { if (v < 0) throw new Error("Negative not allowed"); } };',
  ...
  },
  {
    ...
    setup:
      'const handler = { get(t, p) { throw new Error("Access denied: " + p); } }; const proxy = new Proxy({}, handler);',
    setupCode:
      'const handler = { get(t, p) { throw new Error("Access denied: " + p); } }; const proxy = new Proxy({}, handler);',
  ...
  },
  {
    ...
    setup:
      'const obj = { [Symbol.iterator]: () => ({ next: () => { throw new Error("Iterator error"); } }) };',
    setupCode:
      'const obj = { [Symbol.iterator]: () => ({ next: () => { throw new Error("Iterator error"); } }) };',
  ...
  },
  {
    ...
    sample:
      'const values = []; const g = gen(); try { values.push(g.next().value); values.push(g.next().value); } catch (e) { return { values, error: e.message }; }',
  ...
  },
  {
    ...
    setup:
      'function assertPositive(n) { if (n <= 0) throw new Error(`Expected positive, got ${n}`); return n; }',
    setupCode:
      'function assertPositive(n) { if (n <= 0) throw new Error(`Expected positive, got ${n}`); return n; }',
  ...
  },
  {
    ...
    setup:
      'class AppError extends Error { constructor(msg) { super(msg); this.name = "AppError"; } } class NetworkError extends AppError { constructor(msg) { super(msg); this.name = "NetworkError"; } }',
    setupCode:
      'class AppError extends Error { constructor(msg) { super(msg); this.name = "AppError"; } } class NetworkError extends AppError { constructor(msg) { super(msg); this.name = "NetworkError"; } }',
    expected: [true, true, true],
    sample:
      'const e = new NetworkError("timeout"); return [e instanceof NetworkError, e instanceof AppError, e instanceof Error];',
  ...
  },
  {
    ...
    setup:
      'const errorFactory = { validation: (msg) => { const e = new Error(msg); e.type = "VALIDATION"; return e; }, network: (msg) => { const e = new Error(msg); e.type = "NETWORK"; return e; } };',
    setupCode:
      'const errorFactory = { validation: (msg) => { const e = new Error(msg); e.type = "VALIDATION"; return e; }, network: (msg) => { const e = new Error(msg); e.type = "NETWORK"; return e; } };',
    expected: 'VALIDATION',
  ...
  },
  {
    ...
    setup:
      'let attempts = 0; function unreliable() { attempts++; if (attempts < 3) throw new Error(`Attempt ${attempts}`); return "success"; }',
    setupCode:
      'let attempts = 0; function unreliable() { attempts++; if (attempts < 3) throw new Error(`Attempt ${attempts}`); return "success"; }',
    expected: { result: 'success', errors: ['Attempt 1', 'Attempt 2'] },
    sample:
      'const errors = []; while (true) { try { return { result: unreliable(), errors }; } catch (e) { errors.push(e.message); } }',
  ...
  },
  {
    ...
    setup:
      'function createError() { const e = new TypeError("Invalid argument"); e.code = "ARG_001"; return e; }',
    setupCode:
      'function createError() { const e = new TypeError("Invalid argument"); e.code = "ARG_001"; return e; }',
  ...
  },
  {
    ...
    setup:
      'const api = { response: { data: { users: [{ profile: { email: "test@example.com" } }] } } };',
    setupCode:
      'const api = { response: { data: { users: [{ profile: { email: "test@example.com" } }] } } };',
  ...
  },
  {
    ...
    setup:
      'const middlewares = [(e) => { e.context = []; e.context.push("m1"); return e; }, (e) => { e.context.push("m2"); return e; }];',
    setupCode:
      'const middlewares = [(e) => { e.context = []; e.context.push("m1"); return e; }, (e) => { e.context.push("m2"); return e; }];',
    expected: ['m1', 'm2'],
    sample:
      'let err = new Error("test"); for (const m of middlewares) err = m(err); return err.context;',
  ...
  },
  {
    ...
    setup:
      'const promises = [Promise.resolve(1), Promise.reject(new Error("fail1")), Promise.reject(new Error("fail2"))];',
    setupCode:
      'const promises = [Promise.resolve(1), Promise.reject(new Error("fail1")), Promise.reject(new Error("fail2"))];',
    expected: ['fail1', 'fail2'],
    sample:
      'return Promise.allSettled(promises).then(results => results.filter(r => r.status === "rejected").map(r => r.reason.message));',
  ...
  },
  {
    ...
    setup:
      'let state = { count: 0, errors: [] }; function increment() { state.count++; if (state.count === 2) throw new Error("fail at 2"); }',
    setupCode:
      'let state = { count: 0, errors: [] }; function increment() { state.count++; if (state.count === 2) throw new Error("fail at 2"); }',
    expected: { count: 3, errors: ['fail at 2'] },
    sample:
      'for (let i = 0; i < 3; i++) { try { increment(); } catch (e) { state.errors.push(e.message); } } return state;',
  ...
  },
  {
    ...
    setup:
      'const ops = [() => Promise.reject(new Error("op1")), () => Promise.resolve("ok"), () => Promise.reject(new Error("op3"))];',
    setupCode:
      'const ops = [() => Promise.reject(new Error("op1")), () => Promise.resolve("ok"), () => Promise.reject(new Error("op3"))];',
    expected: ['op1', 'op3'],
    sample:
      'return Promise.all(ops.map(op => op().catch(e => ({ error: e.message })))).then(results => results.filter(r => r.error).map(r => r.error));',
  ...
  },
  {
    ...
    setup:
      'let data = [1, 2, 3]; function transaction(fn) { const backup = [...data]; try { fn(); } catch (e) { data = backup; throw e; } }',
    setupCode:
      'let data = [1, 2, 3]; function transaction(fn) { const backup = [...data]; try { fn(); } catch (e) { data = backup; throw e; } }',
    expected: [1, 2, 3],
    sample:
      'try { transaction(() => { data.push(4); throw new Error("abort"); }); } catch {} return data;',
  ...
  },
  {
    ...
    setup:
      'class ContextError extends Error { constructor(msg, context = {}) { super(msg); this.context = context; this.name = "ContextError"; } addContext(key, value) { this.context[key] = value; return this; } }',
    setupCode:
      'class ContextError extends Error { constructor(msg, context = {}) { super(msg); this.context = context; this.name = "ContextError"; } addContext(key, value) { this.context[key] = value; return this; } }',
    expected: { userId: 123, action: 'save' },
    sample:
      'const e = new ContextError("Failed").addContext("userId", 123).addContext("action", "save"); return e.context;',
  ...
  },
  {
    ...
    sample:
      'const results = [], errors = []; operations.forEach(op => { try { results.push(op()); } catch (e) { errors.push(e.message); } }); return { results, errors };',
  ...
  },
  {
    ...
    setup:
      'const errorTracker = { count: 0, limit: 3, track() { this.count++; if (this.count > this.limit) throw new Error("Too many errors"); } };',
    setupCode:
      'const errorTracker = { count: 0, limit: 3, track() { this.count++; if (this.count > this.limit) throw new Error("Too many errors"); } };',
    expected: 'Too many errors',
    sample:
      'try { for (let i = 0; i < 5; i++) errorTracker.track(); } catch (e) { return e.message; }',
  ...
  },
  {
    ...
    setup:
      'const items = [1, 2, 3]; function process(x) { if (x === 2) throw new Error("bad"); return x * 10; }',
    setupCode:
      'const items = [1, 2, 3]; function process(x) { if (x === 2) throw new Error("bad"); return x * 10; }',
    expected: [10, null, 30],
  ...
  },
  {
    ...
    setup:
      'let delays = []; let attempt = 0; function flaky() { attempt++; delays.push(attempt * 100); if (attempt < 3) throw new Error("retry"); return "done"; }',
    setupCode:
      'let delays = []; let attempt = 0; function flaky() { attempt++; delays.push(attempt * 100); if (attempt < 3) throw new Error("retry"); return "done"; }',
    expected: { result: 'done', delays: [100, 200, 300] },
    sample:
      'let result; while (!result) { try { result = flaky(); } catch {} } return { result, delays };',
  ...
  },
  {
    ...
    setup:
      'function withErrorHandler(fn, handler) { return (...args) => { try { return fn(...args); } catch (e) { return handler(e); } }; }',
    setupCode:
      'function withErrorHandler(fn, handler) { return (...args) => { try { return fn(...args); } catch (e) { return handler(e); } }; }',
    expected: 'Error: test error',
    sample:
      'const risky = () => { throw new Error("test error"); }; const safe = withErrorHandler(risky, e => "Error: " + e.message); return safe();',
  ...
  },
  {
    ...
    setup:
      'const emitter = { handlers: [], on(h) { this.handlers.push(h); }, emit(e) { this.handlers.forEach(h => h(e)); } };',
    setupCode:
      'const emitter = { handlers: [], on(h) { this.handlers.push(h); }, emit(e) { this.handlers.forEach(h => h(e)); } };',
    expected: ['Error: e1', 'Error: e2'],
    sample:
      'const errors = []; emitter.on(e => errors.push("Error: " + e)); emitter.emit("e1"); emitter.emit("e2"); return errors;',
  ...
  },
  {
    ...
    setup:
      'const schema = { name: v => v ? null : "Name required", age: v => v > 0 ? null : "Age must be positive" };',
    setupCode:
      'const schema = { name: v => v ? null : "Name required", age: v => v > 0 ? null : "Age must be positive" };',
    expected: ['Name required', 'Age must be positive'],
    sample:
      'const obj = { name: "", age: -5 }; return Object.entries(schema).map(([k, v]) => v(obj[k])).filter(Boolean);',
  ...
  },
  {
    ...
    setup:
      'const breaker = { failures: 0, open: false, call(fn) { if (this.open) throw new Error("Circuit open"); try { return fn(); } catch (e) { this.failures++; if (this.failures >= 2) this.open = true; throw e; } } };',
    setupCode:
      'const breaker = { failures: 0, open: false, call(fn) { if (this.open) throw new Error("Circuit open"); try { return fn(); } catch (e) { this.failures++; if (this.failures >= 2) this.open = true; throw e; } } };',
    expected: 'Circuit open',
    sample:
      'const fail = () => { throw new Error("fail"); }; try { breaker.call(fail); } catch {} try { breaker.call(fail); } catch {} try { breaker.call(fail); } catch (e) { return e.message; }',
  ...
  },
  {
    ...
    setup:
      'const boundary = { errors: [], wrap(fn) { return (...args) => { try { return { success: true, data: fn(...args) }; } catch (e) { this.errors.push(e.message); return { success: false, error: e.message }; } }; } };',
    setupCode:
      'const boundary = { errors: [], wrap(fn) { return (...args) => { try { return { success: true, data: fn(...args) }; } catch (e) { this.errors.push(e.message); return { success: false, error: e.message }; } }; } };',
    expected: { result: { success: false, error: 'boom' }, tracked: ['boom'] },
    sample:
      'const risky = () => { throw new Error("boom"); }; const safe = boundary.wrap(risky); const result = safe(); return { result, tracked: boundary.errors };',
  ...
  },
  {
    ...
    setup:
      'function strictEquals(a, b) { if (typeof a !== typeof b) throw new TypeError("Type mismatch"); return a === b; }',
    setupCode:
      'function strictEquals(a, b) { if (typeof a !== typeof b) throw new TypeError("Type mismatch"); return a === b; }',
  ...
  },
  {
    ...
    setup:
      'async function step1() { throw new Error("step1 failed"); } async function step2() { return "step2 ok"; }',
    setupCode:
      'async function step1() { throw new Error("step1 failed"); } async function step2() { return "step2 ok"; }',
  ...
  },
  {
    ...
    setup:
      'function sanitizeError(e) { const safe = new Error(e.message.replace(/password=\\w+/, "password=***")); safe.name = e.name; return safe; }',
    setupCode:
      'function sanitizeError(e) { const safe = new Error(e.message.replace(/password=\\w+/, "password=***")); safe.name = e.name; return safe; }',
    expected: 'Failed with password=***',
    sample:
      'const original = new Error("Failed with password=secret123"); return sanitizeError(original).message;',
  ...
  },
  {
    ...
    setup:
      'const items = [1, 2, 3, 4]; function addSafe(acc, x) { if (x === 3) throw new Error("skip"); return acc + x; }',
    setupCode:
      'const items = [1, 2, 3, 4]; function addSafe(acc, x) { if (x === 3) throw new Error("skip"); return acc + x; }',
    expected: 7,
    sample:
      'items.reduce((acc, x) => { try { return addSafe(acc, x); } catch { return acc; } }, 0)',
  ...
  },
  {
    ...
    setup:
      'const slow = new Promise(r => setTimeout(() => r("slow"), 100)); const fast = Promise.reject(new Error("fast error"));',
    setupCode:
      'const slow = new Promise(r => setTimeout(() => r("slow"), 100)); const fast = Promise.reject(new Error("fast error"));',
    expected: 'fast error',
  ...
  },
  {
    ...
    setup:
      'function getSafe(ref) { const obj = ref.deref(); if (!obj) throw new Error("Object collected"); return obj; }',
    setupCode:
      'function getSafe(ref) { const obj = ref.deref(); if (!obj) throw new Error("Object collected"); return obj; }',
    expected: { name: 'test' },
  ...
  },
  {
    ...
    expected: 'AbortError',
    sample:
      'controller.abort(); try { if (controller.signal.aborted) throw new DOMException("Aborted", "AbortError"); } catch (e) { return e.name; }',
  ...
  },
  {
    ...
    setup:
      'function captureStack() { const e = new Error("test"); return e.stack.split("\\n").length > 1; }',
    setupCode:
      'function captureStack() { const e = new Error("test"); return e.stack.split("\\n").length > 1; }',
    expected: true,
    sample: 'captureStack()',
  ...
  },
  {
    ...
    sample:
      'const gen = asyncGen(); return gen.next().then(() => gen.next()).catch(e => e.message);',
  ...
  },
  {
    ...
    text: 'Create an error boundary function that catches and logs errors',
    setup: 'function risky() { throw new Error("operation failed"); }',
    setupCode: 'function risky() { throw new Error("operation failed"); }',
    expected: 'handled',
    sample: 'try { risky(); } catch (e) { console.error(e); return "handled"; }',
    hints: ['Wrap risky operations in try-catch', 'Log errors for debugging'],
    tags: ['error', 'boundary', 'logging'],
  },
  {
    id: 'js-error-112',
    category: 'Error Handling',
    ...
    text: 'Implement retry logic with exponential backoff for failed operations',
    setup:
      'let attempts = 0; function mayFail() { attempts++; if (attempts < 3) throw new Error("retry"); return "success"; }',
    setupCode:
      'let attempts = 0; function mayFail() { attempts++; if (attempts < 3) throw new Error("retry"); return "success"; }',
    expected: 'success',
    sample:
      '(async () => { try { return mayFail(); } catch (e) { await new Promise(r => setTimeout(r, 100)); return mayFail(); } })()',
    hints: ['Retry on failure', 'Add delay between retries'],
    tags: ['error', 'retry', 'backoff', 'async'],
  },
  {
    id: 'js-error-113',
    category: 'Error Handling',
    difficulty: 'easy',
    title: 'Error Message Extraction',
    text: 'Extract the message from an Error object',
    setup: 'const error = new Error("Something went wrong");',
    setupCode: 'const error = new Error("Something went wrong");',
    expected: 'Something went wrong',
    sample: 'error.message',
    hints: ['Error objects have a message property', 'Access it directly'],
    tags: ['error', 'message', 'properties'],
  },
  {
    id: 'js-error-114',
    category: 'Error Handling',
    difficulty: 'easy',
    title: 'Error Name Property',
    text: 'Get the name of an Error type',
    setup: 'const error = new TypeError("Invalid type");',
    setupCode: 'const error = new TypeError("Invalid type");',
    expected: 'TypeError',
    sample: 'error.name',
    hints: ['Error.name contains the error type', 'Different error types have different names'],
    tags: ['error', 'name', 'type'],
  },
  {
    id: 'js-error-115',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Custom Error Class',
    text: 'Create and throw a custom error class',
    setup:
      'class ValidationError extends Error { constructor(msg) { super(msg); this.name = "ValidationError"; } }',
    setupCode:
      'class ValidationError extends Error { constructor(msg) { super(msg); this.name = "ValidationError"; } }',
    expected: 'ValidationError',
    sample: 'try { throw new ValidationError("invalid"); } catch (e) { return e.name; }',
    hints: ['Extend Error class', 'Set custom name property'],
    tags: ['error', 'class', 'custom', 'inheritance'],
  },
  {
    id: 'js-error-116',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error with Additional Properties',
    text: 'Create an error with custom properties',
    setup: 'const error = new Error("Failed"); error.code = 404; error.details = { path: "/api" };',
    setupCode:
      'const error = new Error("Failed"); error.code = 404; error.details = { path: "/api" };',
    expected: 404,
    sample: 'error.code',
    hints: ['Errors are objects', 'Can add custom properties'],
    tags: ['error', 'properties', 'custom'],
  },
  {
    id: 'js-error-117',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error Suppression Pattern',
    text: 'Suppress errors and return default value',
    setup: 'function risky() { throw new Error("fail"); }',
    setupCode: 'function risky() { throw new Error("fail"); }',
    expected: 'default',
    sample: 'try { risky(); } catch { return "default"; }',
    hints: ['Catch without binding error', 'Return fallback value'],
    tags: ['error', 'suppression', 'default'],
  },
  {
    id: 'js-error-118',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Finally Block Execution',
    text: 'Ensure cleanup code runs in finally block',
    setup: 'let cleaned = false; function cleanup() { cleaned = true; }',
    setupCode: 'let cleaned = false; function cleanup() { cleaned = true; }',
    expected: true,
    sample: 'try { throw new Error(); } catch { } finally { cleanup(); } cleaned',
    hints: ['Finally always executes', 'Runs after try/catch'],
    tags: ['error', 'finally', 'cleanup'],
  },
  {
    id: 'js-error-119',
    ...
    difficulty: 'medium',
    title: 'Nested Try-Catch',
    text: 'Handle errors in nested try-catch blocks',
    setup: 'function inner() { throw new Error("inner"); }',
    setupCode: 'function inner() { throw new Error("inner"); }',
    expected: 'inner',
    sample: 'try { try { inner(); } catch (e) { throw e; } } catch (e) { return e.message; }',
    hints: ['Inner catch can rethrow', 'Outer catch handles it'],
    tags: ['error', 'nested', 'rethrow'],
  },
  {
    id: 'js-error-120',
    category: 'Error Handling',
    difficulty: 'easy',
    title: 'Prevent Error Propagation',
    text: 'Catch error and prevent it from propagating',
    setup: 'function throws() { throw new Error("error"); }',
    setupCode: 'function throws() { throw new Error("error"); }',
    expected: 'caught',
    sample: 'try { throws(); } catch { return "caught"; }',
    hints: ['Catch block stops propagation', 'Return value instead of throwing'],
    tags: ['error', 'propagation', 'catch'],
  },
  {
    id: 'js-error-121',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Promise Chain',
    text: 'Handle error in middle of promise chain',
    setup:
      'const p = Promise.resolve(1).then(x => { throw new Error("middle"); }).then(x => x + 1);',
    setupCode:
      'const p = Promise.resolve(1).then(x => { throw new Error("middle"); }).then(x => x + 1);',
    expected: 'middle',
    sample: 'p.catch(e => e.message)',
    hints: ['Errors in chain skip subsequent thens', 'Catch handles all errors'],
    tags: ['error', 'promise', 'chain'],
  },
  {
    id: 'js-error-122',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error Recovery in Promise',
    text: 'Recover from error and continue promise chain',
    setup: 'const p = Promise.reject(new Error("failed"));',
    setupCode: 'const p = Promise.reject(new Error("failed"));',
    expected: 'recovered',
    sample: 'p.catch(() => "recovered")',
    hints: ['catch can return value', 'Continues chain with recovered value'],
    tags: ['error', 'promise', 'recovery'],
  },
  {
    id: 'js-error-123',
    category: 'Error Handling',
    ...
    text: 'Handle different error types differently',
    setup:
      'function mayThrow(type) { if (type === "type") throw new TypeError("type error"); if (type === "range") throw new RangeError("range error"); }',
    setupCode:
      'function mayThrow(type) { if (type === "type") throw new TypeError("type error"); if (type === "range") throw new RangeError("range error"); }',
    expected: 'type error',
    sample: 'try { mayThrow("type"); } catch (e) { if (e instanceof TypeError) return e.message; }',
    hints: ['Use instanceof to check error type', 'Handle each type differently'],
    tags: ['error', 'types', 'instanceof'],
  },
  {
    id: 'js-error-124',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Async Function',
    text: 'Handle error thrown in async function',
    setup: 'async function asyncError() { throw new Error("async error"); }',
    setupCode: 'async function asyncError() { throw new Error("async error"); }',
    expected: 'async error',
    sample: 'asyncError().catch(e => e.message)',
    hints: ['Async functions return promises', 'Errors become rejections'],
    tags: ['error', 'async', 'promise'],
  },
  {
    id: 'js-error-125',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error in forEach',
    text: 'Handle error thrown inside forEach callback',
    setup: 'const arr = [1, 2, 3]; let errorCaught = false;',
    setupCode: 'const arr = [1, 2, 3]; let errorCaught = false;',
    expected: true,
    sample:
      'try { arr.forEach(x => { if (x === 2) throw new Error(); }); } catch { errorCaught = true; } errorCaught',
    hints: ['forEach does not catch errors', 'Wrap in try-catch'],
    tags: ['error', 'forEach', 'callback'],
  },
  {
    id: 'js-error-126',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error in Map Callback',
    text: 'Handle error in map callback using try-catch',
    setup: 'const arr = [1, 2, 3];',
    setupCode: 'const arr = [1, 2, 3];',
    expected: [1, 'error', 3],
    sample:
      'arr.map(x => { try { if (x === 2) throw new Error(); return x; } catch { return "error"; } })',
    hints: ['Map callback can throw', 'Handle inside callback'],
    tags: ['error', 'map', 'callback'],
  },
  {
    id: 'js-error-127',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Generator',
    text: 'Handle error thrown in generator function',
    setup: 'function* gen() { yield 1; throw new Error("gen error"); }',
    setupCode: 'function* gen() { yield 1; throw new Error("gen error"); }',
    expected: 'gen error',
    sample: 'try { const g = gen(); g.next(); g.next(); } catch (e) { return e.message; }',
    hints: ['Generator errors propagate', 'Catch when calling next'],
    tags: ['error', 'generator', 'iteration'],
  },
  {
    id: 'js-error-128',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error in Setter Validation',
    text: 'Throw error in setter for invalid value',
    setup:
      'const obj = { _value: 0, set value(v) { if (v < 0) throw new Error("negative"); this._value = v; } };',
    setupCode:
      'const obj = { _value: 0, set value(v) { if (v < 0) throw new Error("negative"); this._value = v; } };',
    expected: 'negative',
    sample: 'try { obj.value = -1; } catch (e) { return e.message; }',
    hints: ['Setters can throw', 'Validate before setting'],
    tags: ['error', 'setter', 'validation'],
  },
  {
    id: 'js-error-129',
    category: 'Error Handling',
    difficulty: 'easy',
    title: 'ReferenceError Handling',
    text: 'Catch ReferenceError for undefined variable',
    setup: '// ReferenceError occurs',
    setupCode: '// ReferenceError occurs',
    expected: 'ReferenceError',
    sample: 'try { undefinedVar; } catch (e) { return e.name; }',
    hints: ['ReferenceError for undefined vars', 'Check error name'],
    tags: ['error', 'reference', 'undefined'],
  },
  {
    id: 'js-error-130',
    category: 'Error Handling',
    difficulty: 'easy',
    title: 'TypeError Handling',
    text: 'Catch TypeError for invalid operation',
    setup: 'const obj = null;',
    setupCode: 'const obj = null;',
    expected: 'TypeError',
    sample: 'try { obj.property; } catch (e) { return e.name; }',
    hints: ['TypeError for null/undefined access', 'Check error name'],
    tags: ['error', 'type', 'null'],
  },
  {
    id: 'js-error-131',
    category: 'Error Handling',
    ...
    text: 'Create error with cause chain',
    setup: 'const cause = new Error("root cause"); const error = new Error("outer", { cause });',
    setupCode:
      'const cause = new Error("root cause"); const error = new Error("outer", { cause });',
    expected: 'root cause',
    sample: 'error.cause.message',
    hints: ['Error can have cause property', 'Access via error.cause'],
    tags: ['error', 'cause', 'chain'],
  },
  {
    id: 'js-error-132',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Worker',
    text: 'Handle error from Web Worker',
    setup:
      'const worker = { postMessage: () => {}, onerror: null }; let errorMsg = ""; worker.onerror = (e) => { errorMsg = e.message; }; worker.onerror({ message: "worker error" });',
    setupCode:
      'const worker = { postMessage: () => {}, onerror: null }; let errorMsg = ""; worker.onerror = (e) => { errorMsg = e.message; }; worker.onerror({ message: "worker error" });',
    expected: 'worker error',
    sample: 'errorMsg',
    hints: ['Workers have onerror handler', 'Set handler before error'],
    tags: ['error', 'worker', 'async'],
  },
  {
    id: 'js-error-133',
    category: 'Error Handling',
    ...
    text: 'Handle error in event handler',
    setup: 'let errorCaught = false; const handler = () => { throw new Error("handler error"); };',
    setupCode:
      'let errorCaught = false; const handler = () => { throw new Error("handler error"); };',
    expected: true,
    sample: 'try { handler(); } catch { errorCaught = true; } errorCaught',
    hints: ['Event handlers can throw', 'Wrap in try-catch'],
    tags: ['error', 'event', 'handler'],
  },
  {
    id: 'js-error-134',
    category: 'Error Handling',
    ...
    text: 'Handle error in setTimeout callback',
    setup: 'let errorMsg = "";',
    setupCode: 'let errorMsg = "";',
    expected: 'timeout error',
    sample:
      'setTimeout(() => { try { throw new Error("timeout error"); } catch (e) { errorMsg = e.message; } }, 0); await new Promise(r => setTimeout(r, 10)); errorMsg',
    hints: ['setTimeout errors need internal catch', 'Async errors require await'],
    tags: ['error', 'setTimeout', 'async'],
  },
  {
    id: 'js-error-135',
    category: 'Error Handling',
    ...
    text: 'Extract stack trace from error',
    setup: 'const error = new Error("test");',
    setupCode: 'const error = new Error("test");',
    expected: true,
    sample: 'error.stack.includes("Error: test")',
    hints: ['Error.stack contains trace', 'Includes error message'],
    tags: ['error', 'stack', 'trace'],
  },
  {
    id: 'js-error-136',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Promise.all',
    text: 'Handle error when one promise in Promise.all rejects',
    setup:
      'const promises = [Promise.resolve(1), Promise.reject(new Error("fail")), Promise.resolve(3)];',
    setupCode:
      'const promises = [Promise.resolve(1), Promise.reject(new Error("fail")), Promise.resolve(3)];',
    expected: 'fail',
    sample: 'Promise.all(promises).catch(e => e.message)',
    hints: ['Promise.all fails fast', 'First rejection stops all'],
    tags: ['error', 'promise-all', 'rejection'],
  },
  {
    id: 'js-error-137',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Promise.allSettled',
    text: 'Handle results from Promise.allSettled including errors',
    setup: 'const promises = [Promise.resolve(1), Promise.reject(new Error("fail"))];',
    setupCode: 'const promises = [Promise.resolve(1), Promise.reject(new Error("fail"))];',
    expected: 'fail',
    sample:
      'Promise.allSettled(promises).then(results => results.find(r => r.status === "rejected").reason.message)',
    hints: ['allSettled never rejects', 'Check status for each result'],
    tags: ['error', 'promise-allSettled', 'results'],
  },
  {
    id: 'js-error-138',
    category: 'Error Handling',
    ...
    text: 'Wrap error with additional context',
    setup: 'function operation() { throw new Error("operation failed"); }',
    setupCode: 'function operation() { throw new Error("operation failed"); }',
    expected: 'Context: operation failed',
    sample: 'try { operation(); } catch (e) { throw new Error("Context: " + e.message); }',
    hints: ['Catch and rethrow with context', 'Preserve original message'],
    tags: ['error', 'wrapping', 'context'],
  },
  {
    id: 'js-error-139',
    ...
    text: 'Suppress error without handling',
    setup: 'function throws() { throw new Error("error"); }',
    setupCode: 'function throws() { throw new Error("error"); }',
    expected: undefined,
    sample: 'try { throws(); } catch { }',
    hints: ['Empty catch suppresses error', 'Returns undefined'],
    tags: ['error', 'suppression', 'silent'],
  },
  {
    id: 'js-error-140',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Async Iterator',
    text: 'Handle error in async iterator',
    setup: 'async function* asyncIter() { yield 1; throw new Error("iter error"); }',
    setupCode: 'async function* asyncIter() { yield 1; throw new Error("iter error"); }',
    expected: 'iter error',
    sample:
      '(async () => { const iter = asyncIter(); await iter.next(); try { await iter.next(); } catch (e) { return e.message; } })()',
    hints: ['Async iterators throw on error', 'Await next() to catch'],
    tags: ['error', 'async-iterator', 'iteration'],
  },
  {
    id: 'js-error-141',
    category: 'Error Handling',
    ...
    text: 'Handle error when destructuring fails',
    setup: 'const obj = null;',
    setupCode: 'const obj = null;',
    expected: 'default',
    sample: 'const { prop = "default" } = obj ?? {}; prop',
    hints: ['Nullish coalescing prevents error', 'Default values provide fallback'],
    tags: ['error', 'destructuring', 'nullish'],
  },
  {
    id: 'js-error-142',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in Proxy Handler',
    text: 'Handle error thrown in proxy get trap',
    setup:
      'const obj = {}; const proxy = new Proxy(obj, { get: () => { throw new Error("proxy error"); } });',
    setupCode:
      'const obj = {}; const proxy = new Proxy(obj, { get: () => { throw new Error("proxy error"); } });',
    expected: 'proxy error',
    sample: 'try { proxy.any; } catch (e) { return e.message; }',
    hints: ['Proxy traps can throw', 'Errors propagate normally'],
    tags: ['error', 'proxy', 'trap'],
  },
  {
    id: 'js-error-143',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error in JSON Parse',
    text: 'Handle error from invalid JSON parse',
    setup: 'const invalidJson = "{ invalid }";',
    setupCode: 'const invalidJson = "{ invalid }";',
    expected: 'default',
    sample: 'try { JSON.parse(invalidJson); } catch { return "default"; }',
    hints: ['JSON.parse throws on invalid', 'Catch and return default'],
    tags: ['error', 'json', 'parse'],
  },
  {
    id: 'js-error-144',
    category: 'Error Handling',
    difficulty: 'medium',
    title: 'Error in JSON Stringify',
    text: 'Handle circular reference error in JSON.stringify',
    setup: 'const obj = {}; obj.self = obj;',
    setupCode: 'const obj = {}; obj.self = obj;',
    expected: 'circular',
    sample:
      'try { JSON.stringify(obj); } catch (e) { return e.message.includes("circular") ? "circular" : "other"; }',
    hints: ['Circular refs cause errors', 'Catch and check message'],
    tags: ['error', 'json', 'circular'],
  },
  {
    id: 'js-error-145',
    category: 'Error Handling',
    difficulty: 'hard',
    title: 'Error in WeakMap/WeakSet',
    text: 'Handle error when using non-object key in WeakMap',
    setup: 'const wm = new WeakMap();',
    setupCode: 'const wm = new WeakMap();',
    expected: 'TypeError',
    sample: 'try { wm.set("string", 1); } catch (e) { return e.name; }',
    hints: ['WeakMap requires object keys', 'Primitives cause TypeError'],
    tags: ['error', 'weakmap', 'type'],
  },
  // ========================================
  ...
  // ========================================
  {
    ...
    hints: [
      'Pure functions always return the same output for same input',
  ...
  },
  {
    ...
    expected: [1, 2, 3],
    sample:
      'const makeCounter = () => { let count = 0; return () => ++count; }; const counter = makeCounter(); result = [counter(), counter(), counter()]',
    hints: [
      'Inner function captures outer variable',
      'Each call modifies the closed-over variable',
    ],
  ...
  },
  {
    ...
    sample:
      'const createAccount = (initial) => { let balance = initial; return { deposit: amt => balance += amt, withdraw: amt => balance -= amt, getBalance: () => balance }; }; const acc = createAccount(100); acc.deposit(100); acc.withdraw(50); result = { balance: acc.getBalance(), canWithdraw: acc.getBalance() > 0 }',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const myMap = (arr, fn) => arr.reduce((acc, x) => [...acc, fn(x)], []); myMap(nums, x => x * 2)',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const myFilter = (arr, pred) => arr.reduce((acc, x) => pred(x) ? [...acc, x] : acc, []); myFilter(nums, x => x % 2 === 0)',
  ...
  },
  {
    ...
    expected: [5, 7, 10],
    sample:
      'const add = a => b => a + b; const add3 = add(3); result.push(add3(2), add3(4), add(5)(5))',
    hints: [
      'Return a function that takes second arg',
  ...
  },
  {
    ...
    sample:
      'const makeGreeter = greeting => name => `${greeting}, ${name}!`; const sayHello = makeGreeter("Hello"); const sayHi = makeGreeter("Hi"); greetings = [sayHello("World"), sayHi("World"), sayHello("Alice")]',
  ...
  },
  {
    ...
    expected: 15,
    sample:
      'const partial = (fn, ...args) => (...more) => fn(...args, ...more); partial(add3, 5, 5)(5)',
  ...
  },
  {
    ...
    sample:
      'const memoize = fn => { const cache = {}; return x => x in cache ? cache[x] : cache[x] = fn(x); }; const memoized = memoize(expensive); const results = [memoized(5), memoized(5), memoized(10)]; ({ results, calls: callCount })',
  ...
  },
  {
    ...
    sample:
      'nums.reduce((groups, n) => { const key = n % 2 === 0 ? "even" : "odd"; groups[key] = [...(groups[key] || []), n]; return groups; }, {})',
  ...
  },
  {
    ...
    sample:
      'const once = fn => { let called = false, result; return (...args) => called ? result : (called = true, result = fn(...args)); }; const increment = once(() => ++counter); const results = [increment(), increment(), increment()]; ({ results, counter })',
  ...
  },
  {
    ...
    sample:
      'const tap = fn => x => { fn(x); return x; }; const result = nums.map(x => x * 2).map(tap(x => logged.push(x))); ({ result, logged })',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const map = (fn, [head, ...tail]) => head === undefined ? [] : [fn(head), ...map(fn, tail)]; map(x => x * 2, nums)',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const filter = (pred, [head, ...tail]) => head === undefined ? [] : pred(head) ? [head, ...filter(pred, tail)] : filter(pred, tail); filter(x => x % 2 === 0, nums)',
  ...
  },
  {
    ...
    sample:
      'const partition = (pred, arr) => arr.reduce(([pass, fail], x) => pred(x) ? [[...pass, x], fail] : [pass, [...fail, x]], [[], []]); partition(x => x % 2 === 0, nums)',
  ...
  },
  {
    ...
    expected: 9,
    sample:
      'const pipe = (...fns) => x => fns.reduce((v, fn) => fn(v), x); pipe(add1, mult2, sub3)(5)',
  ...
  },
  {
    ...
    expected: 5,
    sample:
      'const compose = (...fns) => x => fns.reduceRight((v, fn) => fn(v), x); compose(add1, mult2, sub3)(5)',
  ...
  },
  {
    ...
    sample:
      'const sortBy = key => arr => [...arr].sort((a, b) => a[key] > b[key] ? 1 : -1); sortBy("name")(users)',
  ...
  },
  {
    ...
    expected: [5, 15, 35],
    sample:
      'const makeAccumulator = () => { let total = 0; return n => total += n; }; const acc = makeAccumulator(); result = [acc(5), acc(10), acc(20)]',
  ...
  },
  {
    ...
    expected: 10,
    sample:
      'const curry = fn => { const arity = fn.length; return function curried(...args) { return args.length >= arity ? fn(...args) : (...more) => curried(...args, ...more); }; }; curry(add4)(1)(2)(3)(4)',
  ...
  },
  {
    ...
    expected: 6,
    sample:
      'const uncurry = fn => (...args) => args.reduce((f, arg) => f(arg), fn); uncurry(curriedAdd)(1, 2, 3)',
  ...
  },
  {
    ...
    sample:
      'const memoize = fn => { const cache = new Map(); return (...args) => { const key = JSON.stringify(args); if (cache.has(key)) return cache.get(key); const result = fn(...args); cache.set(key, result); return result; }; }; const memo = memoize(expensive); const results = [memo(1, 2), memo(1, 2), memo(3, 4)]; ({ results, calls })',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const debounce = (fn, delay) => { let timeout; return (...args) => { clearTimeout(timeout); timeout = setTimeout(() => fn(...args), delay); }; }; typeof debounce(() => {}, 100) === "function"',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const throttle = (fn, limit) => { let waiting = false; return (...args) => { if (!waiting) { fn(...args); waiting = true; setTimeout(() => waiting = false, limit); } }; }; result = typeof throttle(() => {}, 100) === "function"',
  ...
  },
  {
    ...
    expected: [1, 2, 3, 4, 5],
    sample:
      'const deepFlatten = arr => arr.reduce((acc, val) => acc.concat(Array.isArray(val) ? deepFlatten(val) : val), []); deepFlatten(nested)',
  ...
  },
  {
    ...
    expected: 10,
    sample:
      'const reduce = (fn, acc, [head, ...tail]) => head === undefined ? acc : reduce(fn, fn(acc, head), tail); reduce((a, b) => a + b, 0, nums)',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const mapT = fn => reducer => (acc, x) => reducer(acc, fn(x)); const push = (arr, x) => [...arr, x]; nums.reduce(mapT(x => x * 2)(push), [])',
  ...
  },
  {
    ...
    expected: [2, 4, 6],
    sample:
      'const filterT = pred => reducer => (acc, x) => pred(x) ? reducer(acc, x) : acc; const push = (arr, x) => [...arr, x]; nums.reduce(filterT(x => x % 2 === 0)(push), [])',
  ...
  },
  {
    ...
    expected: [4, 8, 12],
    sample:
      'const mapT = fn => r => (a, x) => r(a, fn(x)); const filterT = p => r => (a, x) => p(x) ? r(a, x) : a; const comp = (...fns) => fns.reduce((f, g) => x => f(g(x))); const push = (arr, x) => [...arr, x]; nums.reduce(comp(filterT(x => x % 2 === 0), mapT(x => x * 2))(push), [])',
  ...
  },
  {
    ...
    expected: 10,
    sample:
      'const Box = x => ({ map: fn => Box(fn(x)), fold: fn => fn(x) }); result = Box(5).map(x => x * 2).fold(x => x)',
  ...
  },
  {
    ...
    sample:
      'const Maybe = x => ({ map: fn => x == null ? Maybe(null) : Maybe(fn(x)), fold: (onNothing, onJust) => x == null ? onNothing() : onJust(x) }); const r1 = Maybe("hello").map(s => s.toUpperCase()).fold(() => "nothing", x => x); const r2 = Maybe(null).map(s => s.toUpperCase()).fold(() => "nothing", x => x); results = [r1, r2]',
  ...
  },
  {
    ...
    sample:
      'const Right = x => ({ map: fn => Right(fn(x)), fold: (f, g) => g(x) }); const Left = x => ({ map: fn => Left(x), fold: (f, g) => f(x) }); const safeDivide = (a, b) => b === 0 ? Left("Division by zero") : Right(a / b); const r1 = safeDivide(20, 2).fold(e => `Error: ${e}`, x => x); const r2 = safeDivide(20, 0).fold(e => `Error: ${e}`, x => x); results = [r1, r2]',
    hints: [
      'Right maps and folds with success handler',
      'Left ignores map and folds with error handler',
    ],
  ...
  },
  {
    ...
    expected: 'John Doe',
    sample:
      'const Maybe = x => ({ map: fn => x == null ? Maybe(null) : Maybe(fn(x)), chain: fn => x == null ? Maybe(null) : fn(x), fold: (n, j) => x == null ? n() : j(x) }); const user = { profile: { name: "John Doe" } }; result = Maybe(user).chain(u => Maybe(u.profile)).chain(p => Maybe(p.name)).fold(() => "No name", x => x)',
  ...
  },
  {
    ...
    expected: 120,
    sample:
      'const factorial = (n, acc = 1) => n <= 1 ? acc : factorial(n - 1, n * acc); result = factorial(5)',
  ...
  },
  {
    ...
    expected: 50005000,
    sample:
      'const trampoline = fn => (...args) => { let result = fn(...args); while (typeof result === "function") result = result(); return result; }; const sumTo = trampoline((n, acc = 0) => n === 0 ? acc : () => sumTo(n - 1, acc + n)); result = sumTo(10000)',
  ...
  },
  {
    ...
    expected: 8,
    sample:
      'const asyncCompose = (...fns) => x => fns.reduceRight((p, fn) => p.then(fn), Promise.resolve(x)); const add1 = async x => x + 1; const mult2 = async x => x * 2; result = await asyncCompose(mult2, add1, add1)(2)',
  ...
  },
  {
    ...
    expected: 7,
    sample:
      'const asyncPipe = (...fns) => x => fns.reduce((p, fn) => p.then(fn), Promise.resolve(x)); const add1 = async x => x + 1; const mult2 = async x => x * 2; result = await asyncPipe(add1, mult2, add1)(2)',
  ...
  },
  {
    ...
    expected: 'Alice',
    sample:
      'const lens = path => ({ get: obj => path.reduce((o, k) => o && o[k], obj) }); lens(["user", "profile", "name"]).get(data)',
  ...
  },
  {
    ...
    expected: { a: { b: { c: 2 } } },
    sample:
      'const setPath = (obj, [head, ...tail], val) => ({ ...obj, [head]: tail.length ? setPath(obj[head] || {}, tail, val) : val }); setPath(data, ["a", "b", "c"], 2)',
  ...
  },
  {
    ...
    expected: ['ALICE', 'BOB'],
    sample:
      'const compose = (...fns) => x => fns.reduceRight((v, f) => f(v), x); const prop = k => o => o[k]; const toUpper = s => s.toUpperCase(); const getName = compose(toUpper, prop("name")); users.map(getName)',
  ...
  },
  {
    ...
    expected: 3,
    sample:
      'const converge = (combiner, fns) => (...args) => combiner(...fns.map(fn => fn(...args))); const sum = arr => arr.reduce((a, b) => a + b, 0); const len = arr => arr.length; const average = converge((s, l) => s / l, [sum, len]); average(nums)',
  ...
  },
  {
    ...
    expected: 7,
    sample:
      'const useWith = (fn, transformers) => (...args) => fn(...args.map((arg, i) => transformers[i] ? transformers[i](arg) : arg)); useWith(fn, [x => x * 2, x => x + 1])(2, 2)',
  ...
  },
  {
    ...
    expected: 120,
    sample:
      'const Y = f => (x => f(v => x(x)(v)))(x => f(v => x(x)(v))); const factorial = Y(f => n => n <= 1 ? 1 : n * f(n - 1)); result = factorial(5)',
  ...
  },
  {
    ...
    expected: [1, 2, 3, 4, 5],
    sample:
      'const unfold = (fn, seed) => { const result = []; let val = fn(seed); while (val) { result.push(val[0]); val = fn(val[1]); } return result; }; result = unfold(n => n > 5 ? null : [n, n + 1], 1)',
  ...
  },
  {
    ...
    expected: 15,
    sample:
      'const Reader = run => ({ run, map: fn => Reader(env => fn(run(env))), chain: fn => Reader(env => fn(run(env)).run(env)) }); const getMultiplier = Reader(cfg => cfg.multiplier); const compute = x => getMultiplier.map(m => x * m); compute(5).run(config)',
  ...
  },
  {
    ...
    sample:
      'const createModule = () => { const items = []; return { add: item => items.push(item), getItems: () => [...items], count: () => items.length }; }; const mod = createModule(); mod.add("a"); mod.add("b"); mod.add("c"); result = { count: mod.count(), items: mod.getItems() }',
  ...
  },
  {
    ...
    sample:
      'const Stack = (items = []) => ({ push: x => Stack([...items, x]), pop: () => Stack(items.slice(0, -1)), peek: () => items[items.length - 1], size: () => items.length }); const stack = Stack().push(1).push(2).push(3); result = { top: stack.peek(), size: stack.size() }',
  ...
  },
  {
    ...
    sample:
      'const lazy = fn => { let cached, done = false; return () => done ? cached : (done = true, cached = fn()); }; const getValue = lazy(() => { computed++; return 10; }); getValue(); getValue(); getValue(); ({ value: getValue(), computedOnce: computed })',
  ...
  },
  {
    ...
    expected: 5,
    sample:
      'const times = (n, fn) => { for (let i = 0; i < n; i++) fn(i); }; times(5, () => count++); count',
  ...
  },
  {
    ...
    expected: [[1, 2, 3], [4, 5, 6], [7]],
    sample:
      'const chunk = (arr, size) => arr.reduce((chunks, item, i) => { const idx = Math.floor(i / size); chunks[idx] = [...(chunks[idx] || []), item]; return chunks; }, []); chunk(nums, 3)',
  ...
  },
  {
    ...
    sample:
      'const clamp = (min, max) => val => Math.min(Math.max(val, min), max); values.map(clamp(0, 10))',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const rateLimit = (fn, limit) => { const calls = []; return (...args) => { const now = Date.now(); while (calls.length && calls[0] < now - 1000) calls.shift(); if (calls.length < limit) { calls.push(now); return fn(...args); } }; }; result = typeof rateLimit(() => {}, 5) === "function"',
  ...
  },
  {
    ...
    sample:
      'const deepMerge = (a, b) => { const result = { ...a }; for (const key in b) { result[key] = a[key] && typeof a[key] === "object" && typeof b[key] === "object" ? deepMerge(a[key], b[key]) : b[key]; } return result; }; deepMerge(a, b)',
  ...
  },
  {
    ...
    expected: 60,
    sample:
      'const sumBy = (arr, key) => arr.reduce((sum, obj) => sum + obj[key], 0); sumBy(items, "val")',
  ...
  },
  {
    ...
    expected: { a: 1, c: 3 },
    sample:
      'const pick = (obj, keys) => Object.fromEntries(keys.filter(k => k in obj).map(k => [k, obj[k]])); pick(obj, ["a", "c"])',
  ...
  },
  {
    ...
    expected: { b: 2, d: 4 },
    sample:
      'const omit = (obj, keys) => Object.fromEntries(Object.entries(obj).filter(([k]) => !keys.includes(k))); omit(obj, ["a", "c"])',
  ...
  },
  {
    ...
    expected: { a: 2, b: 4, c: 6 },
    sample:
      'const mapValues = (obj, fn) => Object.fromEntries(Object.entries(obj).map(([k, v]) => [k, fn(v)])); mapValues(obj, x => x * 2)',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const deepFreeze = obj => { Object.freeze(obj); Object.values(obj).forEach(v => v && typeof v === "object" && deepFreeze(v)); return obj; }; deepFreeze(obj); Object.isFrozen(obj) && Object.isFrozen(obj.b)',
  ...
  },
  {
    ...
    sample:
      'const State = run => ({ run, map: fn => State(s => { const [a, s2] = run(s); return [fn(a), s2]; }), chain: fn => State(s => { const [a, s2] = run(s); return fn(a).run(s2); }) }); const increment = State(s => [s, s + 1]); const compute = increment.chain(a => increment.chain(b => increment.map(c => a + b + c))); result = { value: compute.run(0)[0], state: compute.run(0)[1] }',
  ...
  },
  {
    ...
    sample:
      'const IO = run => ({ run, map: fn => IO(() => fn(run())), chain: fn => IO(() => fn(run()).run()) }); const before = sideEffect; const io = IO(() => { sideEffect = 5; return 10; }); const result = io.run(); ({ before, after: sideEffect, result })',
  ...
  },
  {
    ...
    expected: [1, 3, 6, 10, 15],
    sample:
      'const scan = (fn, init, arr) => arr.reduce((acc, x) => [...acc, fn(acc.length ? acc[acc.length - 1] : init, x)], []); scan((a, b) => a + b, 0, nums)',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const f = x => Box(x + 1); const a = 5; const leftSide = Box(a).chain(f).fold(x => x); const rightSide = f(a).fold(x => x); leftSide === rightSide',
  ...
  // ========================================
  {
    ...
    setup:
      'const document = { querySelector: (sel) => sel === "#main-title" ? { id: "main-title", tagName: "H1" } : null };',
    setupCode:
      'const document = { querySelector: (sel) => sel === "#main-title" ? { id: "main-title", tagName: "H1" } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelector: (sel) => sel === ".button" ? { className: "button", tagName: "BUTTON" } : null };',
    setupCode:
      'const document = { querySelector: (sel) => sel === ".button" ? { className: "button", tagName: "BUTTON" } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelector: (sel) => sel === "p" ? { tagName: "P", textContent: "Hello" } : null };',
    setupCode:
      'const document = { querySelector: (sel) => sel === "p" ? { tagName: "P", textContent: "Hello" } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelectorAll: (sel) => sel === ".item" ? { length: 5 } : { length: 0 } };',
    setupCode:
      'const document = { querySelectorAll: (sel) => sel === ".item" ? { length: 5 } : { length: 0 } };',
    expected: 5,
  ...
  },
  {
    ...
    setup:
      'const document = { querySelector: (sel) => sel === \'[data-type="primary"]\' ? { dataset: { type: "primary" } } : null };',
    setupCode:
      'const document = { querySelector: (sel) => sel === \'[data-type="primary"]\' ? { dataset: { type: "primary" } } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelectorAll: (sel) => sel === "nav li" ? [{ tagName: "LI" }, { tagName: "LI" }, { tagName: "LI" }] : [] };',
    setupCode:
      'const document = { querySelectorAll: (sel) => sel === "nav li" ? [{ tagName: "LI" }, { tagName: "LI" }, { tagName: "LI" }] : [] };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelectorAll: (sel) => sel === "article > p" ? [{ tagName: "P", direct: true }] : [] };',
    setupCode:
      'const document = { querySelectorAll: (sel) => sel === "article > p" ? [{ tagName: "P", direct: true }] : [] };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelectorAll: (sel) => sel === "h1, h2, h3" ? [{ tagName: "H1" }, { tagName: "H2" }, { tagName: "H3" }] : [] };',
    setupCode:
      'const document = { querySelectorAll: (sel) => sel === "h1, h2, h3" ? [{ tagName: "H1" }, { tagName: "H2" }, { tagName: "H3" }] : [] };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelector: (sel) => sel === "ul li:first-child" ? { textContent: "First Item" } : null };',
    setupCode:
      'const document = { querySelector: (sel) => sel === "ul li:first-child" ? { textContent: "First Item" } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { querySelectorAll: (sel) => sel === "#settings input[type=checkbox]:checked" ? [{ checked: true }, { checked: true }] : [] };',
    setupCode:
      'const document = { querySelectorAll: (sel) => sel === "#settings input[type=checkbox]:checked" ? [{ checked: true }, { checked: true }] : [] };',
    expected: [{ checked: true }, { checked: true }],
    sample: 'document.querySelectorAll("#settings input[type=checkbox]:checked")',
    hints: [
      'Combine ID, attribute, and pseudo-class selectors',
      'Use :checked for selected checkboxes',
    ],
  ...
  // ========================================
  {
    ...
    setup:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), children: [] }) };',
    setupCode:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), children: [] }) };',
  ...
  },
  {
    ...
    setup:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), textContent: "" }) };',
    setupCode:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), textContent: "" }) };',
  ...
  },
  {
    ...
    setup:
      'const parent = { tagName: "DIV", children: [], appendChild(child) { this.children.push(child); return child; } }; const document = { createElement: (tag) => ({ tagName: tag.toUpperCase() }) };',
    setupCode:
      'const parent = { tagName: "DIV", children: [], appendChild(child) { this.children.push(child); return child; } }; const document = { createElement: (tag) => ({ tagName: tag.toUpperCase() }) };',
  ...
  },
  {
    ...
    setup:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), setAttribute(name, value) { this[name] = value; } }) };',
    setupCode:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), setAttribute(name, value) { this[name] = value; } }) };',
    expected: { tagName: 'A', href: 'https://example.com' },
    sample:
      'const a = document.createElement("a"); a.setAttribute("href", "https://example.com"); a',
    hints: [
      'Use setAttribute to add attributes',
      'First argument is attribute name, second is value',
    ],
  ...
  },
  {
    ...
    setup:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), textContent: "", children: [], appendChild(child) { this.children.push(child); return child; } }) };',
    setupCode:
      'const document = { createElement: (tag) => ({ tagName: tag.toUpperCase(), textContent: "", children: [], appendChild(child) { this.children.push(child); return child; } }) };',
    expected: {
      tagName: 'UL',
      textContent: '',
      children: [{ tagName: 'LI', textContent: 'Item 1', children: [] }],
    },
    sample:
      'const ul = document.createElement("ul"); const li = document.createElement("li"); li.textContent = "Item 1"; ul.appendChild(li); ul',
  ...
  },
  {
    ...
    setup:
      'const existing = { id: "existing" }; const parent = { children: [existing], insertBefore(newEl, ref) { const idx = this.children.indexOf(ref); this.children.splice(idx, 0, newEl); return newEl; } }; const newEl = { id: "new" };',
    setupCode:
      'const existing = { id: "existing" }; const parent = { children: [existing], insertBefore(newEl, ref) { const idx = this.children.indexOf(ref); this.children.splice(idx, 0, newEl); return newEl; } }; const newEl = { id: "new" };',
    expected: { children: [{ id: 'new' }, { id: 'existing' }] },
    sample: 'parent.insertBefore(newEl, existing); parent',
    hints: [
      'insertBefore takes new element and reference element',
      'New element is inserted before the reference',
    ],
  ...
  },
  {
    ...
    setup:
      'const child = { id: "child" }; const parent = { children: [child], removeChild(el) { const idx = this.children.indexOf(el); this.children.splice(idx, 1); return el; } };',
    setupCode:
      'const child = { id: "child" }; const parent = { children: [child], removeChild(el) { const idx = this.children.indexOf(el); this.children.splice(idx, 1); return el; } };',
    expected: { id: 'child' },
  ...
  },
  {
    ...
    setup:
      'const oldEl = { id: "old" }; const newEl = { id: "new" }; const parent = { children: [oldEl], replaceChild(newChild, oldChild) { const idx = this.children.indexOf(oldChild); this.children[idx] = newChild; return oldChild; } };',
    setupCode:
      'const oldEl = { id: "old" }; const newEl = { id: "new" }; const parent = { children: [oldEl], replaceChild(newChild, oldChild) { const idx = this.children.indexOf(oldChild); this.children[idx] = newChild; return oldChild; } };',
  ...
  },
  {
    ...
    setup:
      'const el = { id: "original", children: [{ id: "child" }], cloneNode(deep) { return deep ? { id: this.id, children: [...this.children], cloned: true } : { id: this.id, children: [], cloned: true }; } };',
    setupCode:
      'const el = { id: "original", children: [{ id: "child" }], cloneNode(deep) { return deep ? { id: this.id, children: [...this.children], cloned: true } : { id: this.id, children: [], cloned: true }; } };',
  ...
  },
  {
    ...
    setup:
      'const el = { content: ["existing"], insertAdjacentHTML(pos, html) { if(pos === "beforeend") this.content.push(html); } };',
    setupCode:
      'const el = { content: ["existing"], insertAdjacentHTML(pos, html) { if(pos === "beforeend") this.content.push(html); } };',
  ...
  },
  {
    ...
    setupCode:
      'const el = { textContent: "" }; const userInput = "<script>alert(\\"xss\\")</script>";',
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { classes: ["visible", "hidden"], remove(c) { this.classes = this.classes.filter(x => x !== c); } } };',
    setupCode:
      'const el = { classList: { classes: ["visible", "hidden"], remove(c) { this.classes = this.classes.filter(x => x !== c); } } };',
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { classes: [], toggle(c) { if(this.classes.includes(c)) { this.classes = this.classes.filter(x => x !== c); return false; } else { this.classes.push(c); return true; } } } };',
    setupCode:
      'const el = { classList: { classes: [], toggle(c) { if(this.classes.includes(c)) { this.classes = this.classes.filter(x => x !== c); return false; } else { this.classes.push(c); return true; } } } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { contains(c) { return ["item", "selected", "blue"].includes(c); } } };',
    setupCode:
      'const el = { classList: { contains(c) { return ["item", "selected", "blue"].includes(c); } } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { classes: [], add(...args) { this.classes.push(...args); } } };',
    setupCode:
      'const el = { classList: { classes: [], add(...args) { this.classes.push(...args); } } };',
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { classes: ["old-style", "other"], replace(old, newC) { const idx = this.classes.indexOf(old); if(idx !== -1) { this.classes[idx] = newC; return true; } return false; } } };',
    setupCode:
      'const el = { classList: { classes: ["old-style", "other"], replace(old, newC) { const idx = this.classes.indexOf(old); if(idx !== -1) { this.classes[idx] = newC; return true; } return false; } } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'const el = { classList: { classes: ["visible"], toggle(c, force) { if(force === true && !this.classes.includes(c)) { this.classes.push(c); } else if(force === false) { this.classes = this.classes.filter(x => x !== c); } return force; } } };',
    setupCode:
      'const el = { classList: { classes: ["visible"], toggle(c, force) { if(force === true && !this.classes.includes(c)) { this.classes.push(c); } else if(force === false) { this.classes = this.classes.filter(x => x !== c); } return force; } } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'const value = "test"; const el = { classList: { classes: [], add(c) { this.classes.push(c); } } };',
    setupCode:
      'const value = "test"; const el = { classList: { classes: [], add(c) { this.classes.push(c); } } };',
  ...
  // ========================================
  {
    ...
    setup:
      'let clicked = false; const el = { addEventListener(type, handler) { if(type === "click") handler(); } };',
    setupCode:
      'let clicked = false; const el = { addEventListener(type, handler) { if(type === "click") handler(); } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'let hovered = false; const el = { addEventListener(type, handler) { if(type === "mouseenter") handler(); } };',
    setupCode:
      'let hovered = false; const el = { addEventListener(type, handler) { if(type === "mouseenter") handler(); } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'let pressedKey = ""; const el = { addEventListener(type, handler) { if(type === "keydown") handler({ key: "Enter" }); } };',
    setupCode:
      'let pressedKey = ""; const el = { addEventListener(type, handler) { if(type === "keydown") handler({ key: "Enter" }); } };',
    expected: 'Enter',
  ...
  },
  {
    ...
    setup:
      'let prevented = false; const form = { addEventListener(type, handler) { if(type === "submit") handler({ preventDefault() { prevented = true; } }); } };',
    setupCode:
      'let prevented = false; const form = { addEventListener(type, handler) { if(type === "submit") handler({ preventDefault() { prevented = true; } }); } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'let inputValue = ""; const input = { addEventListener(type, handler) { if(type === "input") handler({ target: { value: "Hello" } }); } };',
    setupCode:
      'let inputValue = ""; const input = { addEventListener(type, handler) { if(type === "input") handler({ target: { value: "Hello" } }); } };',
    expected: 'Hello',
  ...
  },
  {
    ...
    setup:
      'const handler = () => {}; let removed = false; const el = { addEventListener() {}, removeEventListener(type, fn) { if(fn === handler) removed = true; } };',
    setupCode:
      'const handler = () => {}; let removed = false; const el = { addEventListener() {}, removeEventListener(type, fn) { if(fn === handler) removed = true; } };',
    expected: true,
    sample:
      'el.addEventListener("click", handler); el.removeEventListener("click", handler); removed',
  ...
  },
  {
    ...
    setup:
      'let count = 0; const el = { addEventListener(type, handler, options) { if(options?.once) { handler(); } else { handler(); handler(); } } };',
    setupCode:
      'let count = 0; const el = { addEventListener(type, handler, options) { if(options?.once) { handler(); } else { handler(); handler(); } } };',
    expected: 1,
  ...
  },
  {
    ...
    setup:
      'let focused = false; const input = { addEventListener(type, handler) { if(type === "focus") handler(); if(type === "blur") setTimeout(() => handler(), 0); } };',
    setupCode:
      'let focused = false; const input = { addEventListener(type, handler) { if(type === "focus") handler(); if(type === "blur") setTimeout(() => handler(), 0); } };',
    expected: true,
  ...
  },
  {
    ...
    setup:
      'let lastCall = 0; const debounce = (fn, delay) => { let timeout; return (...args) => { clearTimeout(timeout); timeout = setTimeout(() => fn(...args), delay); return delay; }; };',
    setupCode:
      'let lastCall = 0; const debounce = (fn, delay) => { let timeout; return (...args) => { clearTimeout(timeout); timeout = setTimeout(() => fn(...args), delay); return delay; }; };',
    expected: 100,
  ...
  },
  {
    ...
    setup:
      'const throttle = (fn, limit) => { let inThrottle = false; return (...args) => { if (!inThrottle) { fn(...args); inThrottle = true; setTimeout(() => inThrottle = false, limit); return true; } return false; }; };',
    setupCode:
      'const throttle = (fn, limit) => { let inThrottle = false; return (...args) => { if (!inThrottle) { fn(...args); inThrottle = true; setTimeout(() => inThrottle = false, limit); return true; } return false; }; };',
    expected: true,
  ...
  // ========================================
  {
    ...
    setup:
      'let clickedItem = ""; const ul = { addEventListener(type, handler) { if(type === "click") handler({ target: { tagName: "LI", textContent: "Item 2" } }); } };',
    setupCode:
      'let clickedItem = ""; const ul = { addEventListener(type, handler) { if(type === "click") handler({ target: { tagName: "LI", textContent: "Item 2" } }); } };',
    expected: 'Item 2',
    sample:
      'ul.addEventListener("click", (e) => { if(e.target.tagName === "LI") clickedItem = e.target.textContent; }); clickedItem',
  ...
  },
  {
    ...
    setup:
      'let matched = false; const container = { addEventListener(type, handler) { if(type === "click") handler({ target: { matches: (sel) => sel === ".btn" } }); } };',
    setupCode:
      'let matched = false; const container = { addEventListener(type, handler) { if(type === "click") handler({ target: { matches: (sel) => sel === ".btn" } }); } };',
    expected: true,
    sample:
      'container.addEventListener("click", (e) => { if(e.target.matches(".btn")) matched = true; }); matched',
  ...
  },
  {
    ...
    setup:
      'let foundCard = null; const container = { addEventListener(type, handler) { if(type === "click") handler({ target: { closest: (sel) => sel === ".card" ? { id: "card-1" } : null } }); } };',
    setupCode:
      'let foundCard = null; const container = { addEventListener(type, handler) { if(type === "click") handler({ target: { closest: (sel) => sel === ".card" ? { id: "card-1" } : null } }); } };',
    expected: { id: 'card-1' },
    sample:
      'container.addEventListener("click", (e) => { foundCard = e.target.closest(".card"); }); foundCard',
  ...
  },
  {
    ...
    setup:
      'const clicks = []; const parent = { addEventListener(type, handler) { handler({ target: { dataset: { action: "delete" }, closest: () => ({ id: "item-5" }) } }); } };',
    setupCode:
      'const clicks = []; const parent = { addEventListener(type, handler) { handler({ target: { dataset: { action: "delete" }, closest: () => ({ id: "item-5" }) } }); } };',
    expected: [{ action: 'delete', itemId: 'item-5' }],
    sample:
      'parent.addEventListener("click", (e) => { if(e.target.dataset.action) clicks.push({ action: e.target.dataset.action, itemId: e.target.closest("[id]").id }); }); clicks',
  ...
  },
  {
    ...
    setup:
      'const actions = { edit: 0, delete: 0, view: 0 }; const container = { addEventListener(type, handler) { handler({ target: { dataset: { action: "edit" } } }); handler({ target: { dataset: { action: "delete" } } }); } };',
    setupCode:
      'const actions = { edit: 0, delete: 0, view: 0 }; const container = { addEventListener(type, handler) { handler({ target: { dataset: { action: "edit" } } }); handler({ target: { dataset: { action: "delete" } } }); } };',
    expected: { edit: 1, delete: 1, view: 0 },
    sample:
      'container.addEventListener("click", (e) => { const action = e.target.dataset.action; if(action && actions[action] !== undefined) actions[action]++; }); actions',
  ...
  // ========================================
  {
    ...
    setup:
      'let bubbled = true; const child = { addEventListener(type, handler) { handler({ stopPropagation() { bubbled = false; } }); } };',
    setupCode:
      'let bubbled = true; const child = { addEventListener(type, handler) { handler({ stopPropagation() { bubbled = false; } }); } };',
    expected: false,
  ...
  },
  {
    ...
    setup:
      'let phase = ""; const el = { addEventListener(type, handler, options) { phase = options === true || options?.capture ? "capture" : "bubble"; } };',
    setupCode:
      'let phase = ""; const el = { addEventListener(type, handler, options) { phase = options === true || options?.capture ? "capture" : "bubble"; } };',
    expected: 'capture',
  ...
  },
  {
    ...
    setup:
      'const PHASES = { CAPTURING: 1, AT_TARGET: 2, BUBBLING: 3 }; const event = { eventPhase: 3 };',
    setupCode:
      'const PHASES = { CAPTURING: 1, AT_TARGET: 2, BUBBLING: 3 }; const event = { eventPhase: 3 };',
    expected: 'bubbling',
    sample:
      'event.eventPhase === PHASES.BUBBLING ? "bubbling" : event.eventPhase === PHASES.CAPTURING ? "capturing" : "at_target"',
  ...
  },
  {
    ...
    setup:
      'let count = 0; const el = { handlers: [], addEventListener(type, handler) { this.handlers.push(handler); }, fire(e) { for(const h of this.handlers) { h(e); if(e.stopped) break; } } };',
    setupCode:
      'let count = 0; const el = { handlers: [], addEventListener(type, handler) { this.handlers.push(handler); }, fire(e) { for(const h of this.handlers) { h(e); if(e.stopped) break; } } };',
    expected: 1,
    sample:
      'el.addEventListener("click", (e) => { count++; e.stopImmediatePropagation(); e.stopped = true; }); el.addEventListener("click", () => { count++; }); el.fire({}); count',
  ...
  // ========================================
  {
    ...
    setupCode:
      'const localStorage = { data: {}, setItem(key, value) { this.data[key] = value; } };',
  ...
  },
  {
    ...
    setup:
      'const localStorage = { data: { token: "abc123", user: "john" }, removeItem(key) { delete this.data[key]; } };',
    setupCode:
      'const localStorage = { data: { token: "abc123", user: "john" }, removeItem(key) { delete this.data[key]; } };',
  ...
  },
  {
    ...
    setup:
      'const localStorage = { data: { a: "1", b: "2", c: "3" }, clear() { this.data = {}; } };',
    setupCode:
      'const localStorage = { data: { a: "1", b: "2", c: "3" }, clear() { this.data = {}; } };',
    expected: { data: {} },
  ...
  },
  {
    ...
    setup:
      'const localStorage = { data: {}, setItem(key, value) { this.data[key] = value; } }; const user = { name: "Alice", age: 25 };',
    setupCode:
      'const localStorage = { data: {}, setItem(key, value) { this.data[key] = value; } }; const user = { name: "Alice", age: 25 };',
  ...
  },
  {
    ...
    setup:
      'const localStorage = { getItem(key) { return key === "settings" ? \'{"theme":"dark","fontSize":14}\' : null; } };',
    setupCode:
      'const localStorage = { getItem(key) { return key === "settings" ? \'{"theme":"dark","fontSize":14}\' : null; } };',
    expected: { theme: 'dark', fontSize: 14 },
  ...
  },
  {
    ...
    setup:
      'let changed = null; const window = { addEventListener(type, handler) { if(type === "storage") handler({ key: "theme", newValue: "light", oldValue: "dark" }); } };',
    setupCode:
      'let changed = null; const window = { addEventListener(type, handler) { if(type === "storage") handler({ key: "theme", newValue: "light", oldValue: "dark" }); } };',
    expected: { key: 'theme', from: 'dark', to: 'light' },
    sample:
      'window.addEventListener("storage", (e) => { changed = { key: e.key, from: e.oldValue, to: e.newValue }; }); changed',
  ...
  },
  {
    ...
    setup:
      'const fetch = () => Promise.resolve({ json: () => Promise.resolve({ id: 1, name: "Test" }) });',
    setupCode:
      'const fetch = () => Promise.resolve({ json: () => Promise.resolve({ id: 1, name: "Test" }) });',
  ...
  },
  {
    ...
    setup:
      'const fetch = (url, options) => Promise.resolve({ url, method: options?.method, body: options?.body, headers: options?.headers });',
    setupCode:
      'const fetch = (url, options) => Promise.resolve({ url, method: options?.method, body: options?.body, headers: options?.headers });',
    expected: {
    ...
    sample:
      'await fetch("https://api.example.com/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "John" }) })',
  ...
  },
  {
    ...
    setup:
      'const fetch = (url, options) => Promise.resolve({ authHeader: options?.headers?.Authorization });',
    setupCode:
      'const fetch = (url, options) => Promise.resolve({ authHeader: options?.headers?.Authorization });',
    expected: { authHeader: 'Bearer token123' },
    sample:
      'await fetch("https://api.example.com/protected", { headers: { Authorization: "Bearer token123" } })',
  ...
  },
  {
    ...
    expected: true,
    sample:
      'const controller = new AbortController(); controller.abort(); controller.signal.aborted',
  ...
  },
  {
    ...
    setup:
      'const AbortController = class { constructor() { this.signal = { aborted: false }; } abort() { this.signal.aborted = true; } }; const fetchWithTimeout = (url, timeout) => { const controller = new AbortController(); setTimeout(() => controller.abort(), timeout); return { signal: controller.signal }; };',
    setupCode:
      'const AbortController = class { constructor() { this.signal = { aborted: false }; } abort() { this.signal.aborted = true; } }; const fetchWithTimeout = (url, timeout) => { const controller = new AbortController(); setTimeout(() => controller.abort(), timeout); return { signal: controller.signal }; };',
    expected: false,
  ...
  },
  {
    ...
    setup:
      'const fetch = (url) => Promise.resolve({ url, data: url.split("/").pop() }); const urls = ["https://api.example.com/a", "https://api.example.com/b", "https://api.example.com/c"];',
    setupCode:
      'const fetch = (url) => Promise.resolve({ url, data: url.split("/").pop() }); const urls = ["https://api.example.com/a", "https://api.example.com/b", "https://api.example.com/c"];',
    expected: ['a', 'b', 'c'],
    sample:
      'await Promise.all(urls.map(url => fetch(url))).then(responses => responses.map(r => r.data))',
  ...
  // ========================================
  {
    ...
    setup:
      'const FormData = class { constructor() { this.data = {}; } append(key, value) { this.data[key] = value; } };',
    setupCode:
      'const FormData = class { constructor() { this.data = {}; } append(key, value) { this.data[key] = value; } };',
  ...
  },
  {
    ...
    setup:
      'const formData = { get(key) { const data = { email: "test@example.com" }; return data[key] || null; } };',
    setupCode:
      'const formData = { get(key) { const data = { email: "test@example.com" }; return data[key] || null; } };',
    expected: 'test@example.com',
  ...
  },
  {
    ...
    setup:
      'const form = { elements: [{ name: "user", value: "alice" }, { name: "pass", value: "secret" }] }; const FormData = class { constructor(f) { this.data = {}; if(f) f.elements.forEach(e => this.data[e.name] = e.value); } };',
    setupCode:
      'const form = { elements: [{ name: "user", value: "alice" }, { name: "pass", value: "secret" }] }; const FormData = class { constructor(f) { this.data = {}; if(f) f.elements.forEach(e => this.data[e.name] = e.value); } };',
  ...
  },
  {
    ...
    setup:
      'const formData = { data: { a: "1", b: "2", c: "3" }, delete(key) { delete this.data[key]; } };',
    setupCode:
      'const formData = { data: { a: "1", b: "2", c: "3" }, delete(key) { delete this.data[key]; } };',
  ...
  },
  {
    ...
    setup:
      'const formData = { entries() { return [["name", "John"], ["age", "30"], ["city", "NYC"]][Symbol.iterator](); } };',
    setupCode:
      'const formData = { entries() { return [["name", "John"], ["age", "30"], ["city", "NYC"]][Symbol.iterator](); } };',
    expected: [
      ['name', 'John'],
  ...
  },
  {
    ...
    setup:
      'const formData = { entries() { return [["firstName", "Jane"], ["lastName", "Doe"]][Symbol.iterator](); } };',
    setupCode:
      'const formData = { entries() { return [["firstName", "Jane"], ["lastName", "Doe"]][Symbol.iterator](); } };',
  ...
  // ========================================
  {
    ...
    setup:
      'const URLSearchParams = class { constructor(str) { this.params = {}; str.replace("?", "").split("&").forEach(p => { const [k, v] = p.split("="); this.params[k] = v; }); } get(k) { return this.params[k]; } };',
    setupCode:
      'const URLSearchParams = class { constructor(str) { this.params = {}; str.replace("?", "").split("&").forEach(p => { const [k, v] = p.split("="); this.params[k] = v; }); } get(k) { return this.params[k]; } };',
    expected: 'bar',
  ...
  },
  {
    ...
    setup:
      'const params = { data: { page: "1" }, append(key, value) { this.data[key] = value; } };',
    setupCode:
      'const params = { data: { page: "1" }, append(key, value) { this.data[key] = value; } };',
  ...
  },
  {
    ...
    setup:
      'const params = { data: { sort: "asc", filter: "active" }, set(key, value) { this.data[key] = value; } };',
    setupCode:
      'const params = { data: { sort: "asc", filter: "active" }, set(key, value) { this.data[key] = value; } };',
  ...
  },
  {
    ...
    setup:
      'const params = { getAll(key) { return key === "tag" ? ["javascript", "web", "frontend"] : []; } };',
    setupCode:
      'const params = { getAll(key) { return key === "tag" ? ["javascript", "web", "frontend"] : []; } };',
  ...
  },
  {
    ...
    setup:
      'const params = { data: { page: "1", sort: "asc", filter: "all" }, delete(key) { delete this.data[key]; } };',
    setupCode:
      'const params = { data: { page: "1", sort: "asc", filter: "all" }, delete(key) { delete this.data[key]; } };',
  ...
  },
  {
    ...
    setup:
      'const base = "https://api.example.com/search"; const URLSearchParams = class { constructor(obj) { this.obj = obj; } toString() { return Object.entries(this.obj).map(([k,v]) => `${k}=${encodeURIComponent(v)}`).join("&"); } };',
    setupCode:
      'const base = "https://api.example.com/search"; const URLSearchParams = class { constructor(obj) { this.obj = obj; } toString() { return Object.entries(this.obj).map(([k,v]) => `${k}=${encodeURIComponent(v)}`).join("&"); } };',
  ...
  },
  {
    ...
    setup:
      'const URL = class { constructor(url) { this.searchParams = { data: { id: "123" }, set(k, v) { this.data[k] = v; }, toString() { return Object.entries(this.data).map(([k,v]) => `${k}=${v}`).join("&"); } }; this.origin = "https://example.com"; this.pathname = "/api"; } toString() { return `${this.origin}${this.pathname}?${this.searchParams}`; } };',
    setupCode:
      'const URL = class { constructor(url) { this.searchParams = { data: { id: "123" }, set(k, v) { this.data[k] = v; }, toString() { return Object.entries(this.data).map(([k,v]) => `${k}=${v}`).join("&"); } }; this.origin = "https://example.com"; this.pathname = "/api"; } toString() { return `${this.origin}${this.pathname}?${this.searchParams}`; } };',
    expected: 'https://example.com/api?id=123&token=abc',
    sample:
      'const url = new URL("https://example.com/api?id=123"); url.searchParams.set("token", "abc"); url.toString()',
  ...
  // ========================================
  {
    ...
    setup:
      'const IntersectionObserver = class { constructor(callback, options) { this.callback = callback; this.options = options; this.observing = []; } observe(el) { this.observing.push(el); } };',
    setupCode:
      'const IntersectionObserver = class { constructor(callback, options) { this.callback = callback; this.options = options; this.observing = []; } observe(el) { this.observing.push(el); } };',
    expected: true,
    sample:
      'const observer = new IntersectionObserver((entries) => {}); typeof observer.observe === "function"',
  ...
  },
  {
    ...
    setup:
      'const el = { id: "target" }; const observer = { observing: [], observe(element) { this.observing.push(element); } };',
    setupCode:
      'const el = { id: "target" }; const observer = { observing: [], observe(element) { this.observing.push(element); } };',
  ...
  },
  {
    ...
    setup:
      'const el = { id: "target" }; const observer = { observing: [{ id: "target" }, { id: "other" }], unobserve(element) { this.observing = this.observing.filter(e => e.id !== element.id); } };',
    setupCode:
      'const el = { id: "target" }; const observer = { observing: [{ id: "target" }, { id: "other" }], unobserve(element) { this.observing = this.observing.filter(e => e.id !== element.id); } };',
  ...
  },
  {
    ...
    setup:
      'const IntersectionObserver = class { constructor(callback, options) { this.threshold = options?.threshold; } };',
    setupCode:
      'const IntersectionObserver = class { constructor(callback, options) { this.threshold = options?.threshold; } };',
    expected: 0.5,
  ...
  },
  {
    ...
    setup:
      'const entry = { target: { id: "lazy-img" }, isIntersecting: true, intersectionRatio: 0.75 };',
    setupCode:
      'const entry = { target: { id: "lazy-img" }, isIntersecting: true, intersectionRatio: 0.75 };',
    expected: { visible: true, ratio: 0.75, elementId: 'lazy-img' },
    sample:
      '({ visible: entry.isIntersecting, ratio: entry.intersectionRatio, elementId: entry.target.id })',
  ...
  },
  {
    ...
    setup:
      'const images = [{ dataset: { src: "image1.jpg" }, src: "" }, { dataset: { src: "image2.jpg" }, src: "" }]; const loadImage = (entry) => { if(entry.isIntersecting) entry.target.src = entry.target.dataset.src; }; const entries = [{ isIntersecting: true, target: images[0] }];',
    setupCode:
      'const images = [{ dataset: { src: "image1.jpg" }, src: "" }, { dataset: { src: "image2.jpg" }, src: "" }]; const loadImage = (entry) => { if(entry.isIntersecting) entry.target.src = entry.target.dataset.src; }; const entries = [{ isIntersecting: true, target: images[0] }];',
    expected: [
      { dataset: { src: 'image1.jpg' }, src: 'image1.jpg' },
      { dataset: { src: 'image2.jpg' }, src: '' },
    ],
  ...
  // ========================================
  {
    ...
    setup:
      'const MutationObserver = class { constructor(callback) { this.callback = callback; } observe(target, options) { this.target = target; this.options = options; } };',
    setupCode:
      'const MutationObserver = class { constructor(callback) { this.callback = callback; } observe(target, options) { this.target = target; this.options = options; } };',
    expected: true,
    sample:
      'const observer = new MutationObserver((mutations) => {}); typeof observer.observe === "function"',
  ...
  },
  {
    ...
    setup:
      'const target = { id: "container" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
    setupCode:
      'const target = { id: "container" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
  ...
  },
  {
    ...
    setup:
      'const target = { id: "element" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
    setupCode:
      'const target = { id: "element" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
    expected: { config: { attributes: true, attributeFilter: ['class', 'style'] } },
    sample:
      'observer.observe(target, { attributes: true, attributeFilter: ["class", "style"] }); observer',
  ...
  },
  {
    ...
    setup:
      'const target = { id: "root" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
    setupCode:
      'const target = { id: "root" }; const observer = { config: null, observe(el, options) { this.config = options; } };',
  ...
  },
  {
    ...
    setup:
      'const mutations = [{ type: "childList", addedNodes: [{ id: "new1" }, { id: "new2" }], removedNodes: [] }, { type: "childList", addedNodes: [{ id: "new3" }], removedNodes: [{ id: "old1" }] }];',
    setupCode:
      'const mutations = [{ type: "childList", addedNodes: [{ id: "new1" }, { id: "new2" }], removedNodes: [] }, { type: "childList", addedNodes: [{ id: "new3" }], removedNodes: [{ id: "old1" }] }];',
  ...
  // ========================================
  {
    ...
    setup:
      'const document = { getElementById: (id) => id === "header" ? { id: "header", tagName: "HEADER" } : null };',
    setupCode:
      'const document = { getElementById: (id) => id === "header" ? { id: "header", tagName: "HEADER" } : null };',
  ...
  },
  {
    ...
    setup:
      'const document = { getElementsByClassName: (cls) => cls === "card" ? [{ className: "card" }, { className: "card" }] : [] };',
    setupCode:
      'const document = { getElementsByClassName: (cls) => cls === "card" ? [{ className: "card" }, { className: "card" }] : [] };',
    expected: 2,
  ...
  },
  {
    ...
    setup:
      'const el = { id: "box" }; const getComputedStyle = (element) => ({ color: "rgb(255, 0, 0)", fontSize: "16px" });',
    setupCode:
      'const el = { id: "box" }; const getComputedStyle = (element) => ({ color: "rgb(255, 0, 0)", fontSize: "16px" });',
  ...
  },
  {
    ...
    setup:
      'const el = { getBoundingClientRect: () => ({ top: 100, left: 50, width: 200, height: 150, bottom: 250, right: 250 }) };',
    setupCode:
      'const el = { getBoundingClientRect: () => ({ top: 100, left: 50, width: 200, height: 150, bottom: 250, right: 250 }) };',
    expected: { top: 100, left: 50, width: 200, height: 150 },
    sample:
      'const rect = el.getBoundingClientRect(); ({ top: rect.top, left: rect.left, width: rect.width, height: rect.height })',
  ...
  },
  {
    ...
    setup:
      'const document = { createDocumentFragment: () => ({ children: [], appendChild(el) { this.children.push(el); return el; } }), createElement: (tag) => ({ tagName: tag.toUpperCase() }) };',
    setupCode:
      'const document = { createDocumentFragment: () => ({ children: [], appendChild(el) { this.children.push(el); return el; } }), createElement: (tag) => ({ tagName: tag.toUpperCase() }) };',
    expected: { children: [{ tagName: 'DIV' }, { tagName: 'DIV' }, { tagName: 'DIV' }] },
    sample:
      'const frag = document.createDocumentFragment(); for(let i = 0; i < 3; i++) frag.appendChild(document.createElement("div")); frag',
  ...
  },
  {
    ...
    setup:
      'const CustomEvent = class { constructor(type, options) { this.type = type; this.detail = options?.detail; } }; let received = null; const el = { dispatchEvent(event) { received = event; return true; } };',
    setupCode:
      'const CustomEvent = class { constructor(type, options) { this.type = type; this.detail = options?.detail; } }; let received = null; const el = { dispatchEvent(event) { received = event; return true; } };',
    expected: { type: 'userAction', detail: { action: 'click', id: 42 } },
    sample:
      'el.dispatchEvent(new CustomEvent("userAction", { detail: { action: "click", id: 42 } })); received',
  ...
  },
  {
    ...
    setup:
      'let scheduled = null; const requestAnimationFrame = (callback) => { scheduled = callback; return 123; };',
    setupCode:
      'let scheduled = null; const requestAnimationFrame = (callback) => { scheduled = callback; return 123; };',
    expected: 123,
  ...
  },
  {
    ...
    setup:
      'const frames = { 123: true, 456: true }; const cancelAnimationFrame = (id) => { delete frames[id]; };',
    setupCode:
      'const frames = { 123: true, 456: true }; const cancelAnimationFrame = (id) => { delete frames[id]; };',
    expected: { '456': true },

...
db.articles.insertOne({title: "MongoDB Guide", content: "Learn MongoDB"});`,
  ...
  {
    id: 'mongo-beginner-sort-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Sort Ascending',
    text: 'Find all employees sorted by name in ascending order (A to Z).',
    setup: 'Collection: employees with name and salary fields',
    setupCode: `db.employees.insertMany([
  ...
  {name: "Bob", salary: 55000}
    ...
    sample: 'db.employees.find().sort({name: 1})',
    hints: [
      'Use sort() after find() to order results',
      '1 means ascending order (A to Z, lowest to highest)',
      'You can sort by multiple fields: sort({field1: 1, field2: -1})',
    ],
    validPatterns: [/db\.employees\.find\s*\(\s*\)\s*\.sort\s*\(\s*\{\s*name\s*:\s*1\s*\}\s*\)/i],
    tags: ['find', 'beginner', 'sort', 'ascending'],
  },
  {
    id: 'mongo-beginner-sort-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Sort Descending',
    text: 'Find all employees sorted by salary from highest to lowest.',
    setup: 'Collection: employees with name and salary fields',
    setupCode: `db.employees.insertMany([
  {name: "Alice", salary: 75000},
  {name: "Bob", salary: 55000},
  {name: "Carol", salary: 90000}
    ...
    sample: 'db.employees.find().sort({salary: -1})',
    hints: [
      '-1 means descending order (Z to A, highest to lowest)',
      'Descending is useful for "top N" queries',
    ],
    validPatterns: [
      /db\.employees\.find\s*\(\s*\)\s*\.sort\s*\(\s*\{\s*salary\s*:\s*-1\s*\}\s*\)/i,
    ],
    tags: ['find', 'beginner', 'sort', 'descending'],
  },
  {
    id: 'mongo-beginner-limit-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Limit Results',
    text: 'Find the first 3 employees from the collection.',
    setup: 'Collection: employees with multiple documents',
    setupCode: `db.employees.insertMany([
  {name: "Alice"},
  {name: "Bob"},
  ...
  {name: "Eve"}
    ...
    sample: 'db.employees.find().limit(3)',
    hints: [
      'Use limit() to restrict the number of documents returned',
      'Often combined with sort() for top-N queries',
      'Example: find().sort({score: -1}).limit(10) for top 10',
    ],
    validPatterns: [/db\.employees\.find\s*\(\s*\)\s*\.limit\s*\(\s*3\s*\)/i],
    tags: ['find', 'beginner', 'limit'],
  },
  {
    id: 'mongo-beginner-skip-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Skip for Pagination',
    text: 'Find 3 employees starting from the 4th document (skip the first 3).',
    setup: 'Collection: employees with multiple documents',
    setupCode: `db.employees.insertMany([
  {name: "Alice"},
  {name: "Bob"},
  ...
  {name: "Frank"}
    ...
    sample: 'db.employees.find().skip(3).limit(3)',
    hints: [
      'Use skip() to skip a number of documents',
      'Useful for pagination: page N = skip((N-1) * pageSize).limit(pageSize)',
      'For large collections, consider using range queries instead of skip()',
    ],
    validPatterns: [
      /db\.employees\.find\s*\(\s*\)\s*\.skip\s*\(\s*3\s*\)\s*\.limit\s*\(\s*3\s*\)/i,
    ],
    tags: ['find', 'beginner', 'skip', 'pagination'],
  },
  {
    id: 'mongo-beginner-count-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Count Documents',
    text: 'Count the total number of employees in the collection.',
    setup: 'Collection: employees with multiple documents',
    setupCode: `db.employees.insertMany([
  {name: "Alice", department: "Engineering"},
  {name: "Bob", department: "Sales"},
  {name: "Carol", department: "Engineering"}
    ...
    expected: '3',
    sample: 'db.employees.countDocuments()',
    hints: [
      'Use countDocuments() to count all documents',
      'You can pass a query to count matching documents: countDocuments({department: "Engineering"})',
      'countDocuments() is more accurate than the deprecated count()',
    ],
    validPatterns: [/db\.employees\.countDocuments\s*\(\s*\)/i],
    tags: ['collection', 'beginner', 'count'],
  },
  {
    id: 'mongo-beginner-delete-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Delete Single Document',
    text: 'Delete the employee named "Bob" from the collection.',
    setup: 'Collection: employees with multiple documents',
    setupCode: `db.employees.insertMany([
  {name: "Alice", department: "Engineering"},
  {name: "Bob", department: "Sales"},
  {name: "Carol", department: "Engineering"}
    ...
    expected: '1 document deleted',
    sample: 'db.employees.deleteOne({name: "Bob"})',
    hints: [
      'Use deleteOne() to delete a single matching document',
      'Pass a query object to specify which document to delete',
      'Use deleteMany() to delete all matching documents',
    ],
    validPatterns: [/db\.employees\.deleteOne\s*\(\s*\{\s*name\s*:\s*"Bob"\s*\}\s*\)/i],
    tags: ['delete', 'beginner', 'deleteOne'],
  },
];

...
export const clojureProblems: Problem[] = [
  ...
  },
  {
    ...
    setup:
      '(def items [{:category "B" :name "x"} {:category "A" :name "y"} {:category "A" :name "x"}])',
    setupCode:
      '(def items [{:category "B" :name "x"} {:category "A" :name "y"} {:category "A" :name "x"}])',
    expected: [
      { category: 'A', name: 'x' },
  ...
  },
  {
    ...
    setup:
      '(defn configure [{:keys [host port] :or {host "localhost" port 8080}}]\n  (str host ":" port))',
    setupCode:
      '(defn configure [{:keys [host port] :or {host "localhost" port 8080}}]\n  (str host ":" port))',
  ...
  },
  {
    ...
    setup:
      '(defprotocol Describable (describe [this]))\n(defrecord Person [name age]\n  Describable\n  (describe [_] (str name " is " age)))',
    setupCode:
      '(defprotocol Describable (describe [this]))\n(defrecord Person [name age]\n  Describable\n  (describe [_] (str name " is " age)))',
    expected: 'Alice is 30',
  ...
  },
  {
    ...
    setup:
      '(defprotocol Printable (to-str [this]))\n(extend-type String\n  Printable\n  (to-str [s] (str "String: " s)))',
    setupCode:
      '(defprotocol Printable (to-str [this]))\n(extend-type String\n  Printable\n  (to-str [s] (str "String: " s)))',
  ...
  },
  {
    ...
    expected: [1, 3, 6, 10, 15],
    sample:
      '(into [] (fn [rf] (let [sum (volatile! 0)] (fn ([] (rf)) ([result] (rf result)) ([result input] (rf result (vswap! sum + input)))))) [1 2 3 4 5])',
  ...
  },
  {
    ...
    expected: 'hello',
    sample:
      '(do (async/sub p :greeting sub-ch) (async/>!! c {:topic :greeting :msg "hello"}) (:msg (async/<!! sub-ch)))',
  ...
  // -- Strings (2 problems) --
  {
    id: 'clj-beginner-string-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'String Concatenation',
    text: 'Use the str function to concatenate the two strings with a space between them.',
    setup: '(def greeting "Hello")\n(def name "World")',
    setupCode: '(def greeting "Hello")\n(def name "World")',
    expected: 'Hello World',
    sample: '(str greeting " " name)',
    hints: [
      'str concatenates any number of arguments',
      'Converts non-strings automatically',
      'Returns a new string',
    ],
    validPatterns: [/\(\s*str\s+greeting\s+"\s*"\s+name\s*\)/, /str\s+greeting.*name/],
    tags: ['string', 'beginner', 'concatenation'],
  },
  {
    id: 'clj-beginner-string-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'String Length',
    text: 'Use the count function to find the number of characters in the string.',
    setup: '(def message "Clojure")',
    setupCode: '(def message "Clojure")',
    expected: 7,
    sample: '(count message)',
    hints: [
      'count works on strings, collections, etc.',
      'Returns the number of characters for strings',
      'Also works on vectors, lists, maps',
    ],
    validPatterns: [/\(\s*count\s+message\s*\)/],
    tags: ['string', 'beginner', 'count', 'length'],
  },

  // -- Let Bindings (2 problems) --
  {
    id: 'clj-beginner-let-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Basic Let Binding',
    text: 'Use let to bind x to 10 and return its double.',
    setup: '-- Use let to create local bindings',
    setupCode: '-- Use let to create local bindings',
    expected: 20,
    sample: '(let [x 10] (* x 2))',
    hints: [
      'let binds names to values locally',
      'Syntax: (let [name value] body)',
      'Bindings are only available in the body',
    ],
    validPatterns: [/\(\s*let\s+\[\s*x\s+10\s*\]\s*\(\s*\*\s+x\s+2\s*\)\s*\)/],
    tags: ['let', 'beginner', 'binding'],
  },
  {
    id: 'clj-beginner-let-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Multiple Let Bindings',
    text: 'Use let to bind width to 5 and height to 3, then calculate the area.',
    setup: '-- Calculate area using let bindings',
    setupCode: '-- Calculate area using let bindings',
    expected: 15,
    sample: '(let [width 5\n      height 3]\n  (* width height))',
    hints: [
      'Multiple bindings in one let',
      'Later bindings can reference earlier ones',
      'Bindings are processed in order',
    ],
    validPatterns: [/\(\s*let\s+\[.*width.*height.*\]\s*\(\s*\*\s+width\s+height\s*\)\s*\)/s],
    tags: ['let', 'beginner', 'binding', 'multiple'],
  },

  // -- Advanced Sequence Operations (2 problems) --
  {
    id: 'clj-beginner-seq-adv-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Find Maximum',
    text: 'Use apply with max to find the largest element in the vector.',
    setup: '(def scores [72 85 91 68 79])',
    setupCode: '(def scores [72 85 91 68 79])',
    expected: 91,
    sample: '(apply max scores)',
    hints: [
      'apply spreads collection as arguments',
      'max takes multiple arguments, not a collection',
      'apply max converts [a b c] to (max a b c)',
    ],
    validPatterns: [/\(\s*apply\s+max\s+scores\s*\)/, /\(\s*reduce\s+max\s+scores\s*\)/],
    tags: ['sequence', 'beginner', 'apply', 'max'],
  },
  {
    id: 'clj-beginner-seq-adv-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Check Contains with some',
    text: 'Use some with a set to check if the value exists in the vector.',
    setup: '(def fruits ["apple" "banana" "cherry"])\n(def search-for "banana")',
    setupCode: '(def fruits ["apple" "banana" "cherry"])\n(def search-for "banana")',
    expected: 'banana',
    sample: '(some #{search-for} fruits)',
    hints: [
      'some returns first truthy value',
      'Sets can be used as predicates',
      'Returns nil if not found, the value if found',
    ],
    validPatterns: [/\(\s*some\s+#\{search-for\}\s+fruits\s*\)/, /some.*search-for.*fruits/],
    tags: ['sequence', 'beginner', 'some', 'contains'],
  },
];

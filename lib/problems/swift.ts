...
export const swiftProblems: Problem[] = [
...

  {
    ...
    hints: ['reduce takes an initial value and a combining closure', 'You can use the + operator directly'],
...

  {
    ...
    hints: ['reduce(into:) allows mutation of the accumulator', 'More efficient for building collections'],
...

  {
    ...
    hints: ['sorted(by:) takes a comparison closure', 'Return true if first element should come before second'],
    validPatterns: [/\.sorted\s*\{.*\.count\s*<.*\.count.*\}/, /\.sorted\s*\(\s*by:\s*\{.*\.count.*\}\s*\)/],
...

  {
    ...
    validPatterns: [/\.split\s*\(\s*separator:\s*","\s*\)/, /\.components\s*\(\s*separatedBy:\s*","\s*\)/],
...

  {
    ...
    hints: ['hasPrefix checks the beginning of the string', 'Returns true if string starts with the prefix'],
...

  {
    ...
    hints: ['hasSuffix checks the end of the string', 'Returns true if string ends with the suffix'],
...

  {
    ...
    hints: ['Swift strings use String.Index, not integers', 'Use index(_:offsetBy:) to calculate positions'],
...

  {
    ...
    hints: ['removeValue returns the removed value as optional', 'Returns nil if key did not exist'],
...

  {
    ...
    validPatterns: [/\.keys\.contains\s*\(\s*"Alice"\s*\)/, /scores\s*\[\s*"Alice"\s*\]\s*!=\s*nil/],
...

  {
    ...
    hints: ['merge takes a dictionary and a closure for conflicts', 'The closure receives (current, new) values'],
...

  {
    ...
    validPatterns: [/\.filter\s*\{.*\.value\s*>\s*90.*\}/, /\.filter\s*\{.*\$0\.value\s*>\s*90.*\}/],
...

  {
    ...
    hints: ['insert adds an element to the set', 'Returns a tuple with (inserted, memberAfterInsert)'],
...

  {
    ...
    hints: ['subtracting removes elements present in the other set', 'Order matters: A - B is different from B - A'],
...

  {
    ...
    hints: ['symmetricDifference returns elements unique to each set', 'Excludes elements present in both'],
...

  {
    ...
    hints: ['isSubset returns true if all elements are in the other set', 'A set is a subset of itself'],
...

  {
    ...
    hints: ['isSuperset returns true if it contains all elements of the other set', 'Opposite of isSubset'],
...

  {
    ...
    hints: ['isDisjoint returns true if sets have no common elements', 'Empty intersection means disjoint'],
...

  {
    ...
    hints: ['if let creates a new constant with the unwrapped value', 'Code inside the block only runs if optional has a value'],
...

  {
    ...
    hints: ['guard let requires an else clause with exit', 'The unwrapped value is available after the guard'],
...

  {
    ...
    expected: 'Unknown',
    sample: 'name ?? "Unknown"',
    hints: ['?? is the nil coalescing operator', 'Returns the left value if non-nil, otherwise the right'],
...

  {
    ...
    hints: ['flatMap unwraps nested optionals', 'Useful when the transform also returns an optional'],
...

  {
    ...
    hints: ['Chain multiple if let conditions with commas', 'All must be non-nil for the block to execute'],
...

  {
    ...
    setup: 'struct Address { var city: String? }\nstruct Person { var address: Address? }\nlet person: Person? = Person(address: Address(city: "NYC"))',
    setupCode: 'struct Address { var city: String? }\nstruct Person { var address: Address? }\nlet person: Person? = Person(address: Address(city: "NYC"))',
    expected: 'NYC',
    sample: 'person?.address?.city',
    hints: ['Chain multiple ?. for nested optionals', 'Returns nil if any part of the chain is nil'],
...

  {
    ...
    hints: ['Add conditions after optional binding with commas', 'Conditions are checked after successful unwrap'],

...
export const cProblems: Problem[] = [
  ...
  // -- Loops (4 problems) --
  {
    id: 'c-beginner-loop-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Basic For Loop',
    text: 'Write a for loop to print numbers 1 through 5.',
    setup: '#include <stdio.h>',
    setupCode: '#include <stdio.h>',
    expected: 'for (int i = 1; i <= 5; i++) printf("%d\\n", i);',
    sample: 'for (int i = 1; i <= 5; i++) {\n    printf("%d\\n", i);\n}',
    hints: [
      'for(init; condition; increment)',
      'Use i <= 5 for inclusive',
      'i++ increments after each iteration',
    ],
    validPatterns: [/for\s*\(\s*int\s+i\s*=\s*1\s*;\s*i\s*<=\s*5\s*;\s*i\s*\+\+\s*\)/],
    tags: ['loop', 'for', 'beginner'],
  },
  {
    id: 'c-beginner-loop-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'While Loop Counter',
    text: 'Write a while loop to count down from 5 to 1.',
    setup: '#include <stdio.h>\nint n = 5;',
    setupCode: '#include <stdio.h>\nint n = 5;',
    expected: 'while (n > 0) { printf("%d\\n", n); n--; }',
    sample: 'while (n > 0) {\n    printf("%d\\n", n);\n    n--;\n}',
    hints: [
      'while(condition) executes while true',
      'n-- decrements n by 1',
      'Make sure to modify n to avoid infinite loop',
    ],
    validPatterns: [/while\s*\(\s*n\s*>\s*0\s*\)/, /n\s*--/],
    tags: ['loop', 'while', 'beginner'],
  },
  {
    id: 'c-beginner-loop-003',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Sum with For Loop',
    text: 'Use a for loop to calculate the sum of numbers 1 to 10.',
    setup: '#include <stdio.h>\nint sum = 0;',
    setupCode: '#include <stdio.h>\nint sum = 0;',
    expected: 'for (int i = 1; i <= 10; i++) sum += i;',
    sample: 'for (int i = 1; i <= 10; i++) {\n    sum += i;\n}\nprintf("%d", sum);',
    hints: ['Initialize sum to 0', 'sum += i adds i to sum', 'Loop from 1 to 10 inclusive'],
    validPatterns: [/for\s*\([^)]+\).*sum\s*\+=\s*i/s, /sum\s*=\s*sum\s*\+\s*i/],
    tags: ['loop', 'for', 'sum', 'beginner'],
  },
  {
    id: 'c-beginner-loop-004',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Do-While Loop',
    text: 'Write a do-while loop that prints at least once, then continues while n > 0.',
    setup: '#include <stdio.h>\nint n = 3;',
    setupCode: '#include <stdio.h>\nint n = 3;',
    expected: 'do { printf("%d\\n", n); n--; } while (n > 0);',
    sample: 'do {\n    printf("%d\\n", n);\n    n--;\n} while (n > 0);',
    hints: [
    ...
    validPatterns: [/do\s*\{/, /\}\s*while\s*\(\s*n\s*>\s*0\s*\)\s*;/],
    tags: ['loop', 'do-while', 'beginner'],
  },

  // -- Conditionals (3 problems) --
  {
    id: 'c-beginner-cond-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Basic If-Else',
    text: 'Write an if-else to print "positive" if n > 0, else "non-positive".',
    setup: '#include <stdio.h>\nint n = 5;',
    setupCode: '#include <stdio.h>\nint n = 5;',
    expected: 'if (n > 0) printf("positive"); else printf("non-positive");',
    sample: 'if (n > 0) {\n    printf("positive");\n} else {\n    printf("non-positive");\n}',
    hints: [
      'if (condition) { } else { }',
    ...
    validPatterns: [/if\s*\(\s*n\s*>\s*0\s*\)/, /else.*non-positive/s],
    tags: ['conditional', 'if-else', 'beginner'],
  },
  {
    id: 'c-beginner-cond-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'If-Else-If Chain',
    text: 'Write an if-else-if chain to classify n as "negative", "zero", or "positive".',
    setup: '#include <stdio.h>\nint n = 0;',
    setupCode: '#include <stdio.h>\nint n = 0;',
    expected:
      'if (n < 0) printf("negative"); else if (n == 0) printf("zero"); else printf("positive");',
    sample:
      'if (n < 0) {\n    printf("negative");\n} else if (n == 0) {\n    printf("zero");\n} else {\n    printf("positive");\n}',
    hints: [
      'Chain else if for multiple conditions',
      'Use == for equality check',
      'Final else catches remaining cases',
    ],
    validPatterns: [
      /if\s*\(\s*n\s*<\s*0\s*\)/,
      /else\s+if\s*\(\s*n\s*==\s*0\s*\)/,
      /else.*positive/s,
    ],
    tags: ['conditional', 'if-else-if', 'beginner'],
  },
  {
    id: 'c-beginner-cond-003',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Switch Statement',
    text: 'Use a switch statement to print the day name for day number 1-7.',
    setup: '#include <stdio.h>\nint day = 3;',
    setupCode: '#include <stdio.h>\nint day = 3;',
    expected: 'switch(day) { case 1: printf("Monday"); break; ... }',
    sample:
    ...
    validPatterns: [/switch\s*\(\s*day\s*\)/, /case\s+3\s*:/, /break\s*;/],
    tags: ['conditional', 'switch', 'beginner'],
  },

  // -- Arrays (3 problems) --
  {
    id: 'c-beginner-array-001',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Declare and Initialize Array',
    text: 'Declare an array of 5 integers and initialize with values 1-5.',
    setup: '#include <stdio.h>',
    setupCode: '#include <stdio.h>',
    expected: 'int arr[5] = {1, 2, 3, 4, 5};',
    sample: 'int arr[5] = {1, 2, 3, 4, 5};',
    hints: [
      'type name[size] = {values}',
      'Size can be omitted if initializing',
      'Array indices start at 0',
    ],
    validPatterns: [/int\s+arr\s*\[\s*5?\s*\]\s*=\s*\{\s*1\s*,\s*2\s*,\s*3\s*,\s*4\s*,\s*5\s*\}/],
    tags: ['array', 'initialization', 'beginner'],
  },
  {
    id: 'c-beginner-array-002',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Loop Through Array',
    text: 'Use a for loop to print all elements of the array.',
    setup: '#include <stdio.h>\nint arr[5] = {10, 20, 30, 40, 50};',
    setupCode: '#include <stdio.h>\nint arr[5] = {10, 20, 30, 40, 50};',
    expected: 'for (int i = 0; i < 5; i++) printf("%d\\n", arr[i]);',
    sample: 'for (int i = 0; i < 5; i++) {\n    printf("%d\\n", arr[i]);\n}',
    hints: [
      'Array indices go from 0 to size-1',
      'Use i < 5 not i <= 5',
      'Access elements with arr[i]',
    ],
    validPatterns: [/for\s*\(\s*int\s+i\s*=\s*0\s*;\s*i\s*<\s*5\s*;/, /arr\s*\[\s*i\s*\]/],
    tags: ['array', 'loop', 'iteration', 'beginner'],
  },
  {
    id: 'c-beginner-array-003',
    category: 'Beginner Fundamentals',
    difficulty: 'easy',
    title: 'Find Array Sum',
    text: 'Write a loop to calculate the sum of all array elements.',
    setup: '#include <stdio.h>\nint arr[5] = {1, 2, 3, 4, 5};\nint sum = 0;',
    setupCode: '#include <stdio.h>\nint arr[5] = {1, 2, 3, 4, 5};\nint sum = 0;',
    expected: 'for (int i = 0; i < 5; i++) sum += arr[i];',
    sample: 'for (int i = 0; i < 5; i++) {\n    sum += arr[i];\n}\nprintf("Sum: %d", sum);',
    hints: [
      'Initialize sum to 0 before loop',
      'Add each element: sum += arr[i]',
      'Result should be 15',
    ],
    validPatterns: [
      /for\s*\([^)]+\).*sum\s*\+=\s*arr\s*\[\s*i\s*\]/s,
      /sum\s*=\s*sum\s*\+\s*arr\s*\[\s*i\s*\]/,
    ],

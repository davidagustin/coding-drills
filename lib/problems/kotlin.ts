...
export const kotlinProblems: Problem[] = [
...

  {
    ...
    expected: [1, 2, 3],
    sample: 'listOf(1, 2, 3)',
    hints: ['listOf() creates an immutable List', 'Cannot add or remove elements from immutable lists'],
...

  {
    ...
    hints: ['Use getOrNull() for safe access', 'Returns null instead of throwing IndexOutOfBoundsException'],
...

  {
    ...
    hints: ['Use flatten() for simple flattening', 'flatMap allows transformation while flattening'],
...

  {
    ...
    hints: ['Use max() for maximum value', 'Returns null on empty list in older Kotlin, throws in 1.7+'],
...

  {
    ...
    hints: ['partition returns Pair<List, List>', 'First list contains matches, second contains non-matches'],
...

  {
    ...
    hints: ['asSequence() creates lazy sequence', 'Sequences are lazy - process one element at a time'],
...

  {
    ...
    hints: ['generateSequence creates infinite sequence', 'Must use terminal operation like take()'],
...

  {
    ...
    hints: ['data class auto-generates equals, hashCode, toString, copy', 'Properties in primary constructor'],
    validPatterns: [/data\s+class\s+Person\s*\(\s*val\s+name:\s*String\s*,\s*val\s+age:\s*Int\s*\)/],
...

  {
    ...
    sample: 'sealed class Result { data class Success(val data: String): Result(); data class Error(val msg: String): Result() }',
    hints: ['Sealed classes restrict inheritance', 'Useful for representing restricted hierarchies'],

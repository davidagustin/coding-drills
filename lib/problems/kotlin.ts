...
export const kotlinProblems: Problem[] = [
...

  {
    ...
    hints: [
      'fold allows building any result type',
      'First parameter is initial accumulator value',
    ],
...

  {
    ...
    hints: [
      'partition returns Pair of matching and non-matching',
      'Can destructure the result',
    ],
...

  {
    ...
    sample: 'generateSequence(Pair(1, 1)) { Pair(it.second, it.first + it.second) }.map { it.first }.take(8).toList()',
    hints: [
...

  {
    ...
    setup: 'data class Address(val city: String?)\ndata class Person(val address: Address?)\nval person: Person? = Person(Address("NYC"))',
    setupCode: 'data class Address(val city: String?)\ndata class Person(val address: Address?)\nval person: Person? = Person(Address("NYC"))',
    expected: 3,
...

  {
    ...
    setup: 'data class User(val name: String, val age: Int, val email: String)\nval user = User("Alice", 30, "alice@example.com")',
    setupCode: 'data class User(val name: String, val age: Int, val email: String)\nval user = User("Alice", 30, "alice@example.com")',
...

  {
    ...
    hints: ['removeSurrounding removes prefix and suffix if both match', 'Only removes if both ends match'],
...

  {
    ...
    hints: ['removePrefix only removes if string starts with prefix', 'Returns original if no match'],
...

  {
    ...
    sample: 'Regex("(\\\\d{4})-(\\\\d{2})-(\\\\d{2})").find(date)!!.destructured.let { (y, m, d) -> mapOf("year" to y, "month" to m, "day" to d) }',
...

  {
    ...
    hints: [
      'associateBy uses the lambda result as key',
      'Element itself becomes the value',
    ],
...

  {
    ...
    expected: [[1, 3, 2, 4], [7, 9, 6, 8]],
    sample: 'nums.partition { it < 5 }',
    hints: [
      'partition returns Pair<List, List>',
      'First list matches predicate, second does not',
    ],
...

  {
    ...
    hints: [
      'chunked divides list into fixed-size sublists',
...

  {
    ...
    hints: [
      'Destructure in lambda parameters with (a, b)',
      'Works with Pairs and data classes',
    ],
...

  {
    ...
    hints: [
      'yieldAll emits all elements from an iterable',
      'Can mix yield and yieldAll',
    ],
...

  {
    ...
    hints: [
      'partialWindows = true includes incomplete windows',
...

  {
    ...
    hints: [
      'zipWithNext accepts a transform lambda',
      'Lambda receives consecutive pairs',
    ],
...

  {
    ...
    setup: 'data class Person(val name: String, val age: Int)\nval people = listOf(Person("Alice", 30), Person("Bob", 25))',
    setupCode:
...

  {
    ...
    hints: [
      'chunked accepts a transform lambda',
      'Lambda receives each chunk as a list',
    ],
...

  {
    ...
    hints: [
      'reified allows runtime type access',
...

  {
    ...
    hints: [
      'runningFold returns all intermediate results',
...

  {
    ...
    hints: [
      'Chain groupBy with mapValues',
      'Use sumOf and average for aggregations',
    ],

...
test.describe('Drill Mode - Setup Phase', () => {
  ...
  test('should display setup screen with category chips', async ({ page }) => {
    ...
    const categoryChips = page
      .locator('[data-testid="category-chip"]')
  ...
  test('should toggle category chips on click', async ({ page }) => {
    ...
    const categoryChip = page
      .locator('[data-testid="category-chip"]')
      .first()
      .or(page.locator('button').filter({ hasText: /array/i }).first());
  ...
  test('should have question count selector', async ({ page }) => {
    ...
    const countSelector = page.locator('[data-testid="question-count"]').or(
      page
        ...
        .or(page.locator('select').filter({ hasText: /5|10|15|20/ }))
        .or(page.getByRole('slider'))
        .or(page.locator('button').filter({ hasText: /^(5|10|15|20)$/ })),
    );

    await expect(countSelector.first())
  ...
  test('should have "All" category option', async ({ page }) => {
    const allButton = page
      .locator('[data-testid="category-all"]')
      .or(
        page
          .getByRole('button', { name: /^all$/i })
          .or(page.locator('button').filter({ hasText: /^all$/i })),
      );
  ...
  test('should have start button that launches drill', async ({ page }) => {
    ...
    const drillScreen = page
      .locator('[data-testid="drill-problem"]')
      .or(
        page
...
test.describe('Drill Mode - Gameplay', () => {
  ...
  test('should display setup code', async ({ page }) => {
    ...
    const setupCode = page
      .locator('[data-testid="setup-code"]')
      .or(page.locator('pre').or(page.locator('code').or(page.locator('.code-block'))));
  ...
  test('should display expected output', async ({ page }) => {
    ...
    const expectedOutput = page
      ...
      .or(page.locator('text=/expected|output|result/i'));
    await expect(expectedOutput.first())
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
  ...
  test('should submit answer on Enter key', async ({ page }) => {
    ...
    const feedback = page
      .locator('[data-testid="feedback"]')
      .or(
        page
          .locator('.feedback')
          .or(page.locator('text=/correct|incorrect|error|success|wrong|right/i')),
      );
  ...
  test('should have skip button that works', async ({ page }) => {
    ...
    const counterBefore = await page
      .locator('[data-testid="problem-counter"]')
      .or(page.locator('text=/\\d+\\s*\\/\\s*\\d+/'))
      .first()
      .textContent()
      .catch(() => null);
    ...
    const counter = page
      ...
      .or(
        page
          ...
          .or(page.locator('text=/(question|problem)\\s*\\d+/i')),
      );
...
test.describe('Drill Mode - Answer Validation', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const startButton = page
      ...
      .or(page.getByRole('button', { name: /start|begin|go/i }));
    await startButton.first().click({ timeout: 10000 });
    await page.waitForSelector('[data-testid="code-input"], input[type="text"], textarea', {
      timeout: 10000,
    });
  });
  ...
  test('should show success feedback for correct answers', async ({ page }) => {
    ...
    const _sampleSolution = await page
    ...
    const successFeedback = page
      ...
      .or(page.locator('.success').or(page.locator('text=/correct|right|good|nice/i')));

    const errorFeedback = page
      .locator('[data-testid="error-feedback"]')
      .or(page.locator('.error').or(page.locator('text=/incorrect|wrong|error|try again/i')));

    // Either success or error should appear
    await expect(successFeedback.first().or(errorFeedback.first())).toBeVisible({ timeout: 5000 });
  });

  test('should show error feedback with sample solution for incorrect answers', async ({
    page,
  }) => {
    const inputField = page
      .locator('[data-testid="code-input"]')
    ...
    const errorFeedback = page
      .locator('[data-testid="error-feedback"]')
      .or(page.locator('.error').or(page.locator('text=/incorrect|wrong|error/i')));
    ...
    const sampleSolution = page
      .locator('[data-testid="sample-solution"]')
      .or(page.locator('text=/solution|correct answer|expected/i').or(page.locator('pre').nth(1)));
  ...
  test('should display user output', async ({ page }) => {
    ...
    const userOutput = page
      .locator('[data-testid="user-output"]')
      .or(page.locator('text=/your (output|result|answer)/i').or(page.locator('.user-output')));

    await expect(userOutput.first())
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
  ...
  test('should display expected output for comparison', async ({ page }) => {
    ...
    const expectedOutput = page
      .locator('[data-testid="expected-output"]')
      .or(page.locator('text=/expected/i'));
...
test.describe('Drill Mode - Anti-Hardcode Validation', () => {
  ...
  test('should reject hardcoded array values', async ({ page }) => {
    ...
    const errorMessage = page
      ...
      .or(
        page
          .locator('text=/hardcode|must use|provided variables|method/i')
  ...
  test('should reject literal output without method call', async ({ page }) => {
    ...
    const error = page
      .locator('[data-testid="error-feedback"]')
      .or(page.locator('.error').or(page.locator('text=/incorrect|error|wrong|must use/i')));
...
test.describe('Drill Mode - Progress & Stats', () => {
  ...
  test('should display score counter', async ({ page }) => {
    const scoreDisplay = page
      ...
      .or(page.locator('text=/score|points|correct/i').or(page.locator('.score')));
  ...
  test('should display streak counter', async ({ page }) => {
    const streakDisplay = page
      ...
      .or(page.locator('text=/streak/i').or(page.locator('.streak')));
  ...
  test('should display timer during drill', async ({ page }) => {
    const timer = page
      ...
      .or(page.locator('text=/\\d+:\\d+/').or(page.locator('.timer').or(page.locator('time'))));
  ...
  test('should increment score on correct answer', async ({ page }) => {
    const scoreDisplay = page
      .locator('[data-testid="score"]')
      .or(page.locator('text=/(score|correct).*\\d/i'));

    const _initialScore = await scoreDisplay
      .first()
      .textContent()
      .catch(() => '0');
  ...
  test('should save progress to localStorage', async ({ page }) => {
    ...
    const _progress = await getLocalStorageProgress(page);
...
test.describe('Drill Mode - Results Screen', () => {
  test('should show results after completing all questions', async ({ page }) => {
    ...
    const resultsScreen = page
      .locator('[data-testid="results-screen"]')
      .or(page.locator('text=/results|completed|finished|summary|final/i'));
  ...
  test('should display correct count on results', async ({ page }) => {
    ...
    const correctCount = page
      .locator('[data-testid="correct-count"]')
      .or(page.locator('text=/\\d+.*correct/i').or(page.locator('text=/\\d+\\s*\\/\\s*\\d+/')));
  ...
  test('should display time taken on results', async ({ page }) => {
    ...
    const timeTaken = page
      .locator('[data-testid="time-taken"]')
      .or(page.locator('text=/time|duration|\\d+:\\d+|\\d+\\s*(sec|min)/i'));
  ...
  test('should have try again button that works', async ({ page }) => {
    ...
    const tryAgainButton = page
      ...
      .or(page.getByRole('button', { name: /try again|restart|play again|new drill/i }));
    ...
    const setupOrDrill = page
      .locator('[data-testid="drill-setup"]')
      .or(
        page
          .locator('[data-testid="drill-problem"]')
          .or(page.locator('[data-testid="code-input"]')),
      );
